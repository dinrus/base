/*******************************************************************************
    ...
    $(UL
      ...
      $(LI  sample usage:
            ...
            // as the исток generates numbers of its тип that then are simply cast в_
            // the тип needed.
    ...
    flexibility:
    $(UL
      $(LI  easily свопpable basic исток
            ---
      ...
      $(LI  Нормальный и Exp sources allocated only upon request: no память waste, but
            a (*very* small) скорость hit, that can be avoопрed by storing the исток in
...
/// or with рандомируй
///   r.рандомируй(i); r.рандомируй(j); r.рандомируй(ключ);
...
///   auto r2=r.НормальныйИсточник!(плав)(); r2(i)(j)(ключ);
...
final class СлуччисГ(ТИсток=ДефолтныйДвижок)
{
    ...
    /// if исток.можноСеять seeds the generator using the совместно используемый случ generator
    /// (use urandom directly if available?)
    СлуччисГ сей ()
    {
        static if(исток.можноСеять)
        {
            исток.сей(&случ.униформа!(бцел));
        }
        return this;
    }
    /// if исток.можноСеять seeds the generator using the given исток of бцелs
    ...
    T униформа(T,бул проверкаГраниц=да)()
    {
        ...
        {
            return cast(T)исток.следщД;
        }
        else static if (is(T==бул))
        {
            return cast(бул)(исток.следщ & 1u); // проверь lowest bit
        }
        else static if (is(T==плав)||is(T==дво)||is(T==реал))
        {
            static if (T.mant_dig<30)
            {
                ...
                else     // probability 0.00390625 for 24 bit mantissa
                {
                    ...
                    T рез=nV*шкала+исток.следщ*шкала*факт32;
                    static if (проверкаГраниц)
                    {
            ...
            else static if (T.mant_dig<62)
            {
                ...
                else     // probability 0.00048828125 for 53 bit mantissa
                {
                    ...
                    if (nV>=minV2)
                    {
                        return ((cast(T)nV)+(cast(T)исток.следщ)*факт32)*факт64;
                    }
                    else     // probability 1.1368683772161603e-13 for 53 bit mantissa
                    {
                        ...
                        T рез=шкала*((cast(T)nV)+(cast(T)исток.следщД)*факт64);
                        static if (проверкаГраниц)
                        {
                            if (рез!=cast(T)0) return рез;
            ...
            else static if (T.mant_dig<=64)
            {
                ...
                ббайт nB=исток.следщБ;
                if (nB!=0)
                {
                    T рез=nB*факт8+исток.следщД*факт72;
                    static if (проверкаГраниц)
                    {
                        if (рез!=cast(T)1) return рез;
                        // 1 due в_ rounding (<1.e-16), 0 impossible
                ...
                else     // probability 0.00390625
                {
                    ...
                    T рез=((cast(T)nB)+(cast(T)исток.следщД)*факт64)*шкала;
                    static if (проверкаГраниц)
                    {
                        if (рез!=cast(T)0) return рез;
    ...
    /// (it could be worked around using дол aritmethic for цел, и doing a перенос by hand for дол,
        ...
        static if (is(U S:S[]))
        {
            ...
            else static if (is(T==бцел)||is(T==цел))
            {
                ...
                for (T* aPtr=a.ptr; aPtr!=aEnd; ++aPtr)
                    *aPtr=cast(T)(исток.следщ);
            }
            else static if (is(T==бдол)||is(T==дол))
            {
                T* aEnd=a.ptr+a.length;
                for (T* aPtr=a.ptr; aPtr!=aEnd; ++aPtr)
                    *aPtr=cast(T)(исток.следщД);
            }
            else static if (is(T==плав)|| is(T==дво)|| is(T==реал))
            {
    ...
    /// randomizes the given Массив и returns it (for some типы this is potentially
    /// ещё efficient, Всё из_ the use of random numbers и speedwise)
    U рандомируйУниформуР(U,З,бул проверкаГраниц=да)(ref U a,З в_)
    in { assert((cast(БазТипМассивов!(U))в_)>0,"пустой диапазон");}
    body
    {
        alias БазТипМассивов!(U) T;
        static assert(is(З:T),"несовместимые типы "~U.stringof~" и "~З.stringof);
        static if (is(U S:S[]))
        {
    ...
    /// ещё efficient, Всё из_ the use of random numbers и speedwise)
    U рандомируйУниформуР2(U,З,W,бул проверкаГраниц=да)(ref U a,З из_, W в_)
    in
    {
        alias БазТипМассивов!(U) T;
    ...
    U рандомируйУниформуРСимм(U,З,бул проверкаГраниц=да, бул исключитьНоль=плав_ли!(БазТипМассивов!(U)))
    (ref U a,З в_)
        ...
        body
        {
            ...
            for (;;)
            {
                do
                {
                    T x=n.дайСлучайный();
                    T знач=c*x+cast(T)1;
                    знач=знач*знач*знач; // might недобор (in extreme situations) so it is in the loop
                }
                while (знач<=0)
                    T u=r.униформа!(T)();
                if (u<1-(cast(T)0.331)*(x*x)*(x*x)) return t*d*знач;
                if (лог(u)< x*x/2+d*(1-знач+лог(знач))) return t*d*знач;
            }
        }
    ...
    ЭкспИсточник!(СлуччисГ,T) экспИсточник(T)()
    {
        ...
        else static assert(0,T.stringof~" эксп исток не реализован");
    }
    ...
    body
    {
        ...
        for (;;)
        {
            ...
            while (знач<=0)
                T u=униформа!(T)();
            if (u<1-(cast(T)0.331)*(x*x)*(x*x)) return сигма*d*знач;
            if (лог(u)< x*x/2+d*(1-знач+лог(знач))) return сигма*d*знач;
        }
    }
    // ---------------
...
/// совместно используемый locked (threadsafe) random число generator
/// инициализован with urandom if available, with время иначе
