 ...
 *
 * For detailed information, refer to MSDN's documentation for the $(LINK2 http://msdn2.microsoft.com/en-us/library/system.drawing.imaging.aspx, Система.Drawing.Imaging) namespace.
 *
 * Copyright: (c) 2009 John Chapman
 *
 * License: See $(LINK2 ..\..\licence.txt, licence.txt) for use and distribution terms.
 */
module win32.media.imaging;
...
/**
 * Specifies the атрибуты of a битмап image.
 */
final class BitmapData {

  цел ширина;               /// Gets or sets the pixel _width of the Bitmap object.
  цел высота;              /// Gets of sets the pixel _height of the Bitmap object.
  цел stride;              /// Gets or sets the _stride ширина of the Bitmap object.
  PixelFormat pixelFormat; /// Gets or sets the format of the pixel information in the Bitmap object.
  ук scan0;             /// Gets or sets the адрес of the first pixel данные in the Bitmap object.
  цел reserved;            /// Reserved. Do not use.
...
final class ImageCodecInfo {
  ...
  ткст codecName;               ///
  ткст dllName;                 ///
  ткст formatDescription;       ///
  ткст filenameExtension;       ///
  ткст mimeType;                ///
  ImageCodecFlags флаги;          ///
  ббайт[][] signaturePatterns;    ///
  ббайт[][] signatureMasks;       ///

  /**
   */
  static ImageCodecInfo[] getImageEncoders() {
    цел numEncoders, размер;

    Status status = GdipGetImageEncodersSize(numEncoders, размер);
    if (status != Status.ОК)
      throw statusException(status);

    auto pCodecs = cast(GpImageCodecInfo*)LocalAlloc(LMEM_FIXED, размер);

    status = GdipGetImageEncoders(numEncoders, размер, pCodecs);
    if (status != Status.ОК)
      throw statusException(status);

    auto codecs = new ImageCodecInfo[numEncoders];
    for (цел i = 0; i < numEncoders; i++) {
      with (codecs[i] = new ImageCodecInfo) {
        with (pCodecs[i]) {
          clsid = Clsid;
          formatId = FormatID;
          if (CodecName) codecName = .вУтф8(CodecName);
          if (DllName) dllName = .вУтф8(DllName);
          if (FormatDescription) formatDescription = .вУтф8(FormatDescription);
          if (FilenameExtension) filenameExtension = .вУтф8(FilenameExtension);
          if (MimeType) mimeType = .вУтф8(MimeType);
          флаги = cast(ImageCodecFlags)Flags;

          signaturePatterns.length = SigCount;
          signatureMasks.length = SigCount;
          for (цел j = 0; j < SigCount; j++) {
            signaturePatterns[j].length = SigSize;
            signatureMasks[j].length = SigSize;
            cidrus.memcpy(signaturePatterns[j].ptr, SigPattern + (j * SigSize), SigSize);
            cidrus.memcpy(signatureMasks[j].ptr, SigMask + (j * SigSize), SigSize);
          }
        }
      }
    }

    LocalFree(cast(Укз)pCodecs);
  ...
  static ImageCodecInfo[] getImageDecoders() {
    цел numDecoders, размер;

    Status status = GdipGetImageDecodersSize(numDecoders, размер);
    if (status != Status.ОК)
      throw statusException(status);

    auto pCodecs = cast(GpImageCodecInfo*)LocalAlloc(LMEM_FIXED, размер);

    status = GdipGetImageDecoders(numDecoders, размер, pCodecs);
    if (status != Status.ОК)
      throw statusException(status);

    auto codecs = new ImageCodecInfo[numDecoders];
    for (цел i = 0; i < numDecoders; i++) {
      with (codecs[i] = new ImageCodecInfo) {
        with (pCodecs[i]) {
          clsid = Clsid;
          formatId = FormatID;
          codecName = .вУтф8(CodecName);
          if (DllName) dllName = .вУтф8(DllName);
          if (FormatDescription) formatDescription = .вУтф8(FormatDescription);
          if (FilenameExtension) filenameExtension = .вУтф8(FilenameExtension);
          if (MimeType) mimeType = .вУтф8(MimeType);
...
final class EncoderParameter {
  ...
  private GUID guid_;
  private цел numberOfValues_;
  private цел type_;
  private ук value_;

  /**
   */
  this(Encoder encoder, цел numberOfValues, цел тип, ук значение) {
    guid_ = encoder.гуид;
...
final class EncoderParameters {
  ...
  /*package*/ this(GpEncoderParameters* p) {
    ...
    for (цел i = 0; i < p.Count; i++) {
      param[i] = new EncoderParameter(new Encoder(p.Параметр[i].Guid), p.Параметр[i].NumberOfValues, p.Параметр[i].Тип, p.Параметр[i].Value);
    }
  }
  ...
  /*package*/ GpEncoderParameters* forGDIplus() {
    ...
    for (цел i = 0; i < param.length; i++) {
      p.Параметр[i].Guid = param[i].guid_;
      p.Параметр[i].NumberOfValues = param[i].numberOfValues_;
      p.Параметр[i].Тип = param[i].type_;
      p.Параметр[i].Value = param[i].value_;
    }
    return p;
  }
...
final class ColorMatrix {
  ...
  /**
   */
  this(плав[][] newColorMatrix) {
    matrix_ = newColorMatrix;
  }

  /**
   */
  проц opIndexAssign(плав значение, цел row, цел column) {
    matrix_[row][column] = значение;
  }

  /**
   * ditto
   */
  плав opIndex(цел row, цел column) {
