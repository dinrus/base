 ...
 */
module win32.media.consts;

/// Specifies the known system цвета.
enum KnownColor {
  ActiveBorder = 1,        /// A system-defined цвет.
  ActiveCaption,           /// ditto
...
enum CombineMode {
  ...
  Союз,      /// Two clipping regions are combined by taking the union of both.
  Xor,        /// Two clipping regions are combined by taking only the areas enclosed by one or другой region.
...
enum FlushIntention {
  ...
  Sync   /// Все graphics operations on the stack are executed as soon as possible, thus synchronizing the graphics state.
}
...
/// Specifies how данные is interpolated between endpoints.
...
/// Specifies how the исток цвета are combined with the background цвета.
enum CompositingMode {
  SourceOver, /// Specifies that the цвет is blended with the background цвет.
  SourceCopy  /// Specifies that the цвет overwrites the background цвет.
}

/// Specifies the quality уровень to use during compositing.
enum CompositingQuality {
  ...
  AssumeLinear                    /// Assume linear значения.
}

/// Specifies how pixels are смещение during rendering.
enum PixelOffsetMode {
  ...
  None,                           /// Specifies no pixel смещение.
  Half                            /// Specifies that pixels are смещение by -.5 units for high speed antialiasing.
}

/// Specifies the format of the цвет данные for each pixel in the image.
enum PixelFormat {
  Undefined = 0,                                                    /// The pixel format is undefined.
  DontCare = 0,                                                     /// No pixel format is specified.
  Indexed = 0x00010000,                                             /// The pixel данные содержит цвет-indexed значения.
  Gdi = 0x00020000,                                                 /// The pixel данные содержит GDI цвета.
  Alpha = 0x00040000,                                               /// The pixel данные содержит alpha значения that are not premultiplied.
  PAlpha = 0x00080000,                                              /// The pixel данные содержит premultiplied alpha значения.
  Extended = 0x00100000,                                            /// Reserved.
  Canonical = 0x00200000,                                           /// Reserved.
  Format1bppIndexed = 1 | (1 << 8) | Indexed | Gdi,                 /// Specifies that the pixel format is 1 bit per pixel and that it uses indexed цвет.
  ...
  Format16bppGrayScale = 4 | (16 << 8) | Extended,                  /// The pixel format is 16 bits per pixel. The цвет information specifies 65536 shades of gray.
  Format16bppRgb555 = 5 | (16 << 8) | Gdi,                          /// Specifies that the format is 16 bits per pixel; 5 bits each are used for the red, green, and blue компоненты.
  ...
  Format16bppArgb1555 = 7 | (16 << 8) | Alpha | Gdi,                /// The pixel format is 16 bits per pixel. The цвет information specifies 32,768 shades of цвет, of which 5 bits are red, 5 bits are green, 5 bits are blue, and 1 bit is alpha.
  Format24bppRgb = 8 | (24 << 8) | Gdi,                             /// Specifies that the format is 24 bits per pixel; 8 bits each are used for the red, green, and blue компоненты.
  Format32bppRgb = 9 | (32 << 8) | Gdi,                             /// Specifies that the format is 32 bits per pixel; 8 bits each are used for the red, green, and blue компоненты.
  Format32bppArgb = 10 | (32 << 8) | Alpha | Gdi | Canonical,       /// Specifies that the format is 32 bits per pixel; 8 bits each are used for the alpha, red, green, and blue компоненты.
  Format32bppPArgb = 11 | (32 << 8) | Alpha | PAlpha | Gdi,         /// Specifies that the format is 32 bits per pixel; 8 bits each are used for the alpha, red, green, and blue компоненты. The red, green, and blue компоненты are premultiplied according to the alpha component.
  Format48bppRgb = 12 | (48 << 8) | Extended,                       /// Specifies that the format is 48 bits per pixel; 16 bits each are used for the red, green, and blue компоненты.
  Format64bppArgb = 13 | (64 << 8) | Alpha | Canonical | Extended,  /// Specifies that the format is 64 bits per pixel; 16 bits each are used for the alpha, red, green, and blue компоненты.
  Format64bppPArgb = 14 | (64 << 8) | Alpha | PAlpha | Extended     /// Specifies that the format is 64 bits per pixel; 16 bits each are used for the alpha, red, green, and blue компоненты. The red, green, and blue компоненты are premultiplied according to the alpha component.
}
