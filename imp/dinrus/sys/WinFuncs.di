...
extern(C):
	...
	*/
    цел ВСАДайАсинхрРезультат (ук сокет, АСИНХРОН* оверлап, бцел* трансфер, бул ждать, бцел* флаги);
	/**
	*WSAIoctl
	*/
    цел ВСАВВКонтрл (ук s, бцел op, ук inBuf, бцел cbIn, ук outBuf, бцел cbOut, бцел* результат, АСИНХРОН* оверлап, ук фция);
	/**
	*WSARecv
	*/
    цел ВСАПрими (ук сок, ВИНСОКБУФ* буф, бцел см, бцел* ссм, бцел* ссмм, АСИНХРОН* ас, ук ф);
	/**
	*WSASend
	*/
    цел ВСАШли (ук сок, ВИНСОКБУФ* буф, бцел лл, бцел* олл, бцел оллл, АСИНХРОН* ас, ук ффф);
	/**
	*GetDiskFreeSpaceExA
	*/
...
//ВРЕМЯ
    ...
    */
    ФВРЕМЯ СистВремяВФВремя(in СИСТВРЕМЯ *систВрем);
    /**
...
///КОМ
    ...
    * CoCreateInstance
    */
	цел СоздайЭкземплярКо(КЛСИД клсид, Инкогнито анонВнешн, бцел контекстКл, ИИД иид, ук* ув);
    /**
    * CoCreateInstanceEx
    */
