...
*    ПФлагКУДоп, ППриоритетНити, ППраваПроцесса, ПРежим_Адресации, ПИдЧП, ПКомРез,
...
*          ПЯзык, ППодъяз, ПКодСтр, ПКонтекст, ПОкСооб, ПСозданиеПроцесса,
*           ПКонтекстВып, ППолитикаИсключений, ПДиспачФлаг, ПСредство, ПВар,
*            ПАспектЦУ, ТИМЕД, ПВидТипа, ПКонвВызова, ПВидФунк, ПВидВызова,
*             ПВидПерем, ПФлагТипа, ПФлагФунк, ПФлагиПерем, ПВидДескр, ПКлассКонтекста,
...
///////////////////////////////////////
/*** Преобразует текстовый литерал в ббайт*=PCHAR
***/
template _усим( ткст a )
{
    const PCHAR _усим = cast(PCHAR)a.ptr;
}
/////////////////////////////////////
/***
Перечень констант  в русском аналоге
***/
...
extern(Windows):
 ...
 enum : бцел
{
    ЕЩЁ_АКТИВНА = (0x103),//STILL_ALIVE
}

/***
Имя: Константа ИСХДЛИНТОК
Описание:
	Исходная длина токена.
***/
const т_мера ИСХДЛИНТОК = 8;//TOKEN_SOURCE_LENGTH
alias ИСХДЛИНТОК  ДЛИНА_ИСТОКА_ТОКЕНА;
/***
Имя: Константа СОКОШИБ
Описание:
	Ошибка сокета.
***/
const цел СОКОШИБ = -1;//SOCKET_ERROR
/***
Имя: Константа РАЗМНАБ_УД
Описание:
	Размер набора удалёных данных.
***/
const бцел РАЗМНАБ_УД = 64;//FD_SETSIZE
/***
Имя: Константа НЕВЕРНСОК
Описание:
	Неверный сокет.
***/
const СОКЕТ НЕВЕРНСОК = cast(СОКЕТ)~0;//INVALID_SOCKET
/***
Имя: Константа ДЛИНА_ВСАОПИСАНИЯ
Описание:
	Длина описания ВСА.
...
alias ДЛИНА_ВСАОПИСАНИЯ WSADESCRIPTION_LEN;
/***
Имя:
Описание:
...
alias ДЛИНА_ВСАСИС_СТАТУСА WSASYS_STATUS_LEN;
/***
Имя:
Описание:
	Ещё не документировано.
...
/***
Имя: Константа НЕВЕРНХЭНДЛ
Описание:
	Значение неверного хэндла.
***/
...
/***
Имя: Константа МАКС_ПУТЬ
Описание:
	Максимальное число символов в определении файлового пути.
***/
const бцел	МАКС_ПУТЬ 			= 260;
//alias МАКС_ПУТЬ MAX_PATH;
/***
Имя:
Описание:
	Ещё не документировано.
...
//alias МАКС_ИСКЛ_ПАРАМЫ EXCEPTION_MAXIMUM_PARAMETERS;
/***
Имя:
Описание:
	Ещё не документировано.
...
//alias ИСКЛ_НЕПРОДОЛЖИТЕЛЬНОЕ EXCEPTION_NONCONTINUABLE;
/***
Имя:
Описание:
	Ещё не документировано.
...
Описание:
	Ещё не документировано.
***/	
enum ПТипЗКД: ббайт //ACE_HEADER.ACE_TYPE
 {
...
***/	
const МАСКА_ДОСТУПА
	УДАЛИТЬ                   = 0x00010000,//DELETE
	ЧИТАТЬ_КОНТРОЛЬ             = 0x00020000,//READ_CONTROL
	ЗАПИСАТЬ_ДКД                = 0x00040000,//WRITE_DAC
	ЗАПИСАТЬ_ВЛАДЕЛЬЦА              = 0x00080000,//WRITE_OWNER
	СИНХРОНИЗИРОВАТЬ              = 0x00100000,//SYNCHRONIZE
	ДОСТУП_С_СИСТЕМНОЙ_БЕЗОПАСНОСТЬЮ   = 0x01000000,//ACCESS_SYSTEM_SECURITY
	МАКСИМАЛЬНО_ДОПУСТИМЫЙ          = 0x02000000,//MAXIMUM_ALLOWED
	ГЕНЕРНОЕ_ЧТЕНИЕ             = 0x80000000,//GENERIC_READ
	ГЕНЕРНАЯ_ЗАПИСЬ            = 0x40000000,//GENERIC_WRITE
	ГЕНЕРНОЕ_ВЫПОЛНЕНИЕ         = 0x20000000,//GENERIC_EXECUTE
	ГЕНЕРНОЕ_ВСЁ              = 0x10000000,//GENERIC_ALL
	ТРЕБУЮТСЯСП = 0x000F0000,//STANDARD_RIGHTS_REQUIRED
	СПЧ     = 0x00020000,//STANDARD_RIGHTS_READ
	СПЗ    = 0x00020000,//STANDARD_RIGHTS_WRITE
	СПВ  = 0x00020000,//STANDARD_RIGHTS_EXECUTE
	СП_ВСЕ      = 0x001F0000,//STANDARD_RIGHTS_ALL
	ОП_ВСЕ      = 0x0000FFFF;//SPECIFIC_RIGHTS_ALL
//////////////////////////////////
/***
***/	
...
const ДИСПИД_НЕИЗВЕСТЕН = -1;//DISPID_UNKNOWN
const ДИСПИД_ЗНАЧЕНИЕ = 0;//DISPID_VALUE
const ДИСПИД_ПОМЕСТИТЬ_СВОЙСТВО = -3;//DISPID_PROPERTYPUT
const ДИСПИД_НОВПЕРЕЧЕНЬ = -4;//DISPID_NEWENUM
const ДИСПИД_ОЦЕНИТЬ = -5;//DISPID_EVALUATE
const ДИСПИД_КОНСТРУКТОР = -6;//DISPID_CONSTRUCTOR
const ДИСПИД_ДЕСТРУКТОР = -7;//DISPID_DESTRUCTOR
const ДИСПИД_КОЛЛЕКТ = -8; //DISPID_COLLECT
//////////////////////////////////
/***
***/
enum ПМайлСлот
...
// STARTUPINFO, ИНФОСТАРТА.флаги
enum ПСтартФ: бцел
{
	ИспПоказОкна    = 0x0001,//STARTF_USESHOWWINDOW
	ИспРазмер          = 0x0002,//STARTF_USESIZE
	ИспПозицию      = 0x0004,//STARTF_USEPOSITION
	ИспСчётСимв    = 0x0008,//STARTF_USECOUNTCHARS
	ИспЗапАотрибут = 0x0010,//STARTF_USEFILLATTRIBUTE
	ЗапускПолнЭкр    = 0x0020,//STARTF_RUNFULLSCREEN
	ВклФорсФидбэк  = 0x0040,//STARTF_FORCEONFEEDBACK
	ВыклФорсФидбэк = 0x0080,//STARTF_FORCEOFFFEEDBACK
	ИспСтдХэндлы    = 0x0100,//STARTF_USESTDHANDLES
	ИспХотки        = 0x0200//STARTF_USEHOTKEY
	}
...
enum ПСтраница: бцел {
	НетДоступа          = 0x0001,//PAGE_NOACCESS
	ТолькоЧтен          = 0x0002,//PAGE_READONLY
	ЧтенЗап         = 0x0004,//PAGE_READWRITE
	ЗапКоп         = 0x0008,//PAGE_WRITECOPY
	Вып           = 0x0010,//PAGE_EXECUTE
	ВыпЧтен      = 0x0020,//PAGE_EXECUTE_READ
	ВыпЧтенЗап = 0x0040,//PAGE_EXECUTE_READWRITE
	ВыпЗапКоп = 0x0080,//PAGE_EXECUTE_WRITECOPY
	Гард             = 0x0100,//PAGE_GUARD
	НетКэша           = 0x0200//PAGE_NOCACHE
}

const МАСКА_ДОСТУПА
	СЕКЦИЯ_ЗАПРОС       = 0x000001,//SECTION_QUERY
	СЕКЦИЯ_КАРТА_ЗАПИСЬ   = 0x000002,//SECTION_MAP_WRITE
	СЕКЦИЯ_КАРТА_ЧТЕНИЕ    = 0x000004,//SECTION_MAP_READ
	СЕКЦИЯ_КАРТА_ВЫПОЛНЕНИЕ = 0x000008,//SECTION_MAP_EXECUTE
	СЕКЦИЯ_РАСШИРИТЬ_РАЗМЕР = 0x000010,//SECTION_EXTEND_SIZE
	СЕКЦИЯ_ПОЛНЫЙ_ДОСТУП  = 0x0F001F;//SECTION_ALL_ACCESS

const МАСКА_ДОСТУПА
        ФАЙЛ_КАРТА_КОПИРОВАТЬ =       СЕКЦИЯ_ЗАПРОС,//FILE_MAP_COPY
        ФАЙЛ_КАРТА_ЗАПИСЬ =      СЕКЦИЯ_КАРТА_ЗАПИСЬ,//FILE_MAP_WRITE
        ФАЙЛ_КАРТА_ЧТЕНИЕ =       СЕКЦИЯ_КАРТА_ЧТЕНИЕ,//FILE_MAP_READ
        ФАЙЛ_КАРТА_ПОЛНЫЙ_ДОСТУП = СЕКЦИЯ_ПОЛНЫЙ_ДОСТУП;//FILE_MAP_ALL_ACCESS
...
const бцел LOCALE_NEUTRAL     = (SORT_DEFAULT << 16)
...
// ???
//MACRO #define HEAP_MAKE_TAG_FLAGS(b,o) ((бцел)((b)+(o)<<16)))
...
const бцел MAXIMUM_PROCESSORS = 32;
...
+/
/////////////////////////
enum ППаритетСКУ: ббайт
{
	Нет = 0,//NOPARITY
	Нечётный,//ODDPARITY
	Чётный,//EVENPARITY
	Метка,//MARKPARITY
	Пространство//SPACEPARITY
}

// DCB/СКУ
enum ПСтопбитыСКУ : ббайт {
	Один = 0,
	ОдинИПять,
	Два
}
...
/////////////////////////
	// for MoveFileEx()
	const бцел
		ФПЕРЕМЕСТ_ЗАМЕНИ_СУЩ   = 1,//MOVEFILE_REPLACE_EXISTING
		ФПЕРЕМЕСТ_КОПИР_РАЗРЕШЕНО       = 2,//MOVEFILE_COPY_ALLOWED
		ФПЕРЕМЕСТ_ОТЛОЖИ_ДО_ПЕРЕЗАГРУЗКИ = 4,//MOVEFILE_DELAY_UNTIL_REBOOT
		ФПЕРЕМЕСТ_ПЕРЕПИШИ      = 8;//MOVEFILE_WRITE_THROUGH
...
const бцел PRIVILEGE_SET_ALL_NECESSARY = 1;
...
const бцел TLS_MINIMUM_AVAILABLE = 64;
...
****/
enum ПТипТокена//TOKEN_TYPE
	{
	Первичный = 1,//TokenPrimary
	Имперсонация//TokenImpersonation
	}	
...
//////////////////////////////
/** Уровень имперсонализации безопасности (УИБ)
**/
...
const ПУровеньИмперсонацииБезопасности
	МАКСУРИМП = ПУровеньИмперсонацииБезопасности.Делегация,
	ДЕФУРИМП      = ПУровеньИмперсонацииБезопасности.Имперсонация;	
...
enum ПХэндлФ: бцел
{
	Наследовать            = 0x01,//HANDLE_FLAG_INHERIT
	ЗащитаОтЗакрытия = 0x02//HANDLE_FLAG_PROTECT_FROM_CLOSE
}
...
/***
Имя: Перечень Определений Прав Доступа к Реестру
Описание:
	Определение право доступа к рестру;
...
//СИ = Сообщение об Изменении
    СИ_ИмяФайла		= 0x00000001,//FILE_NOTIFY_CHANGE_FILE_NAME
    СИ_ИмяПапки		= 0x00000002,//FILE_NOTIFY_CHANGE_DIR_NAME
	СИ_Имя         = 0x00000003,//
    СИ_АтрыФайла			= 0x00000004,//FILE_NOTIFY_CHANGE_ATTRIBUTES
    СИ_РазмФайла					= 0x00000008,//FILE_NOTIFY_CHANGE_SIZE
    СИ_ПоследнЗап			= 0x00000010,//FILE_NOTIFY_CHANGE_LAST_WRITE
    СИ_ПоследнДост			= 0x00000020,//FILE_NOTIFY_CHANGE_LAST_ACCESS
    СИ_ДатаСозд				= 0x00000040,//FILE_NOTIFY_CHANGE_CREATION
	СИ__ЭА			          = 0x00000080,//
    СИ_Безоп				= 0x00000100,//FILE_NOTIFY_CHANGE_SECURITY	
...
////////////////////////////////////	
//	Действие над файлом
    Добавлен						= 0x00000001,//FILE_ACTION_ADDED
    Удалён				= 0x00000002,//FILE_ACTION_REMOVED
    Изменён				= 0x00000003,//FILE_ACTION_MODIFIED
    ПереименованОбратно			= 0x00000004,//FILE_ACTION_RENAMED_OLD_NAME
    Переименован			= 0x00000005,//FILE_ACTION_RENAMED_NEW_NAME
...
///////////////////////////////////////////////////	
    РегчувствПоиск				= 0x00000001,//FILE_CASE_SENSITIVE_SEARCH
    СохраненыРегистрыИмён					= 0x00000002,//
    ЮникодНаДиске							= 0x00000004,//FILE_UNICODE_ON_DISK
    НеизменныеСКД								= 0x00000008,//FILE_PERSISTENT_ACLS
    Сжатие								= 0x00000010,//FILE_FILE_COMPRESSION
    ...
    ТомСжат									= 0x00008000,//FILE_VOLUME_IS_COMPRESSED
...
//Флаги и Атрибуты	///FILE_ATTRIBUTE_
    ПервыйПайпЭкземпляр    = 0x00080000,//
	ПереписатьЧерез					= 0x80000000,//FILE_FLAG_WRITE_THROUGH
    Асинхронно					= 0x40000000,//FILE_FLAG_OVERLAPPED
    БезБуф						= 0x20000000,//FILE_FLAG_NO_BUFFERING
    СлучДоступ					= 0x10000000,//FILE_FLAG_RANDOM_ACCESS
    ПоследоватСкан				= 0x08000000,//FILE_FLAG_SEQUENTIAL_SCAN
    УдалитьПриЗакрытии			= 0x04000000,//FILE_FLAG_DELETE_ON_CLOSE
    РезервнСохрСемантики		= 0x02000000,//FILE_FLAG_BACKUP_SEMANTICS
    СемантикаПосикс				= 0x01000000,//FILE_FLAG_POSIX_SEMANTICS
	ТолькоЧтение				 = 0x00000001,//FILE_ATTRIBUTE_READONLY
    Скрытый 				= 0x00000002,//FILE_ATTRIBUTE_HIDDEN
    Системный			 = 0x00000004,//FILE_ATTRIBUTE_SYSTEM
    Папка 				= 0x00000010,//FILE_ATTRIBUTE_DIRECTORY
    Архив 				= 0x00000020,//FILE_ATTRIBUTE_ARCHIVE
	Устройство     		 = 0x00000040,//
    Нормальный			= 0x00000080,//FILE_ATTRIBUTE_NORMAL
    Временный			= 0x00000100,//FILE_ATTRIBUTE_TEMPORARY
	РедкийФайл         = 0x00000200,//
	ТочкаВосстановления       = 0x00000400,//
    Сжатый				= 0x00000800,//FILE_ATTRIBUTE_COMPRESSED
    Офлайн				= 0x00001000,//FILE_ATTRIBUTE_OFFLINE
...
{
	СоздатьНовый				= 1,//CREATE_NEW
    СоздатьВсегда				= 2,//CREATE_ALWAYS
    ОткрытьСущ					= 3,//OPEN_EXISTING
    ОткрытьВсегда				= 4,//OPEN_ALWAYS
    ОбрезатьСущ					= 5,//TRUNCATE_EXISTING
}

enum ПКопирФайл: бцел
{
// КопируйФайлДоп()
...
enum ПСовмИспФайла:бцел //FILE_SHARE_ХХ
{
    Нет			     = 0x00000000,
	Чтение			 = 0x00000001,// FILE_SHARE_READ
    Запись			 = 0x00000002,//FILE_SHARE_WRITE
    Удаление  		 = 0x00000004,//FILE_SHARE_DELETE
	ВалидныеФлаги	 = 0x00000007//
}

enum ПРежимФайла: бцел
 {
  Ввод = 1,
  Вывод = 2,
  ВыводНов = 6,
...
/***
Имя: Перечень Уровней Приоритета Нити
Описание:
	Уровни приоритета нити.
***/
enum ППриоритетНити: цел
{
    НизРеалВрем =  15,  //THREAD_BASE_PRIORITY_LOWRT
    Макс =    2,   // THREAD_BASE_PRIORITY_MAX
    Мин =    -2,  // THREAD_BASE_PRIORITY_MIN
    Холостая =   -15, //THREAD_BASE_PRIORITY_IDLE
    Низкий =          Мин,
    НижеНормы =    (Низкий+1),
    Норма =          0,
    Высокий =         Макс,
...
enum ППраваПроцесса: бцел
{
///Требуется для удаления объекта. 
ПравоУдалять = cast(бцел)0x00010000L,
/**
* Требуется для чтения информации в дескрипторе безопасности объекта,
* кроме информации в SACL. Для чтения/записи SACL, требуется право доступа
* ACCESS_SYSTEM_SECURITY. См.права доступа к SACL. 
*/
ПравоЧитатьКонтроль  = cast(бцел)0x00020000L,
/**
* Право использовать объект для синхронизации. Это способствует ожиданию нити 
* до того момента, когда объект будет в сигнализируемом состоянии. 
*/
ПравоСинхронизовать = cast(бцел)0x00100000L, 
/**
* Требуется для изменения DACL в дескрипторе безопасности объекта. 
*/
ПравоЗаписиДАК = cast(бцел)0x00040000L,
/**
* Требуется для изменения владельца в дескрипторе безопасности объекта.
*/
ПравоЗаписиВладельца = cast(бцел)0x00080000L,  
///Все возможные права доступа для объекта процесса. 
ВсеПраваДоступа = cast(бцел)0x1F0FFF,
///Требуется для создания процесса. 
ПравоСоздаватьПроцесс = cast(бцел)0x0080,
///Требуется для создания нити. 
ПравоСоздаватьНить = cast(бцел)0x0002,
///Требуется для дубликации хэндла посредством DuplicateHandle.
ПравоДублироватьХэндл = cast(бцел)0x0040,
/**
* Требуется для получения определённой информации о процессе, его токене,
* коде выхода и классе приоритета (функции OpenProcessToken, GetExitCodeProcess,
* GetPriorityClass и IsProcessInJob). 
*/
ПравоЗапроса = cast(бцел)0x0400,
/*
* Требуется для установки ограничений памяти с помощью SetProcessWorkingSetSize. 
*/
ПравоКвотировать = cast(бцел)0x0100,
ПравоУстанавливатьИнфо =cast(бцел)0x0200,
/// Требуется для прекращения процесса посредством TerminateProcess.
ПравоПрерывания = cast(бцел)0x0001, 
/**
* Требуется для проведения операций в виртуальном адресном пространстве процесса
* (см. VirtualProtectEx и WriteProcessMemory).
*/
ПравоОбрабатыватьВП  =cast(бцел)0x0008, 
/**
* Требуется для чтения виртуальной памяти процесса посредством ReadProcessMemory. 
*/
ПравоЧитатьВП = cast(бцел)0x0010,
/// Требуется для записи памяти посредством WriteProcessMemory. 
ПравоЗаписыватьВП  = cast(бцел)0x0020,
}

enum ПСозданиеПроцесса : бцел {
	Отладка               = 0x00000001,//DEBUG_PROCESS
	ОтладкаТолькоЭтого     = 0x00000002,//DEBUG_ONLY_THIS_PROCESS
	СоздатьПодвешенным            = 0x00000004,//CREATE_SUSPENDED
	Откреплённый            = 0x00000008,//DETACHED_PROCESS
	НоваяКонсоль          = 0x00000010,//CREATE_NEW_CONSOLE
	НормКлассПриоритета       = 0x00000020,//NORMAL_PRIORITY_CLASS
	БездействКлассПриоритета         = 0x00000040,//IDLE_PRIORITY_CLASS
	ВысшийКлассПриоритета         = 0x00000080,//HIGH_PRIORITY_CLASS
	РеальноВремКлассПриоритета     = 0x00000100,//REALTIME_PRIORITY_CLASS
	НоваяГруппаПроцессов    = 0x00000200,//CREATE_NEW_PROCESS_GROUP
	СредаЮникод  = 0x00000400,//CREATE_UNICODE_ENVIRONMENT
	Отдельный_WOW_VDM     = 0x00000800,//CREATE_SEPARATE_WOW_VDM
	Совместный_WOW_VDM       = 0x00001000,//CREATE_SHARED_WOW_VDM
	ФорсДос             = 0x00002000,//CREATE_FORCEDOS
	НижеНормКлассПриоритета = 0x00004000,//BELOW_NORMAL_PRIORITY_CLASS
	ВышеНормКлассПриоритета = 0x00008000,//ABOVE_NORMAL_PRIORITY_CLASS
	БезЗадачи   = 0x01000000,//CREATE_BREAKAWAY_FROM_JOB
	СПользоватПрофилем     = 0x02000000,//CREATE_WITH_USERPROFILE
	ДефолтнРежимОшибок   = 0x04000000,//CREATE_DEFAULT_ERROR_MODE
	БезОкна            = 0x08000000,//CREATE_NO_WINDOW
	ПрофильПользователь                = 0x10000000,//PROFILE_USER
	ПрофильЯдро              = 0x20000000,//PROFILE_KERNEL
	ПрофильСервер              = 0x40000000//PROFILE_SERVER
}

// Флаги для ДайКУДоп(),GetDCEx()

enum ПКУДопФ: бцел
{
   Окно =           0x00000001,//DCX_WINDOW
    Кэш =            0x00000002,//DCX_CACHE
    НеПереустАтры =     0x00000004,//DCX_NORESETATTRS
    КлипОтпрыски =     0x00000008,//DCX_CLIPCHILDREN
    КлипСироты =     0x00000010,//DCX_CLIPSIBLINGS
    РодительКлип =       0x00000020,//DCX_PARENTCLIP
    ИсключиРгн =       0x00000040,//DCX_EXCLUDERGN
    ИнтерсектРгн =     0x00000080,//DCX_INTERSECTRGN
    ИсключиОбов =    0x00000100,//DCX_EXCLUDEUPDATE
    ИнтерсектОбнов =  0x00000200,//DCX_INTERSECTUPDATE
    БлокируйОбновОкна = 0x00000400,//DCX_LOCKWINDOWUPDATE
    Валидируй =         0x00200000,//DCX_VALIDATE
}
...
alias ПРежим_Адресации ПРежАдр,ADDRESS_MODE;
...
enum ПВинСокОш: цел
{
	Блокировано =     10035,//WSAEWOULDBLOCK
	Прервано =           10004,//WSAEINTR
	ХостНеНайден =  11001,//WSAHOST_NOT_FOUND
}
...
enum: бцел
{
    БЕСК =              бцел.max,//INFINITE
    ЖДИ_ОБЪЕКТ_0 =         0,//WAIT_OBJECT_0
    ЖДИ_ПОКИНУТЫЙ_0 =      0x80,//WAIT_ABANDONED_0
    ЖДИ_ТАЙМАУТ =          0x102,//WAIT_TIMEOUT
    ЖДИ_ЗАВЕРШЕНИЕ_ВВ =    0xc0,//WAIT_IO_COMPLETION
    ЖДИ_ПОКИНУТЫЙ =        0x80,//WAIT_ABANDONED
    ЖДИ_ПРОВАЛ =           бцел.max,//WAIT_FAILED
}
...
/* VK_0 thru VK_9 are the То же, что и ASCII '0' thru '9' (0x30 - 0x39) */
/* VK_A thru VK_Z are the То же, что и ASCII 'A' thru 'Z' (0x41 - 0x5A) */
...
enum ПКлассКонтекста //CLSCTX
 {
	СерверВнутриПроцесса   = 0x1,//CLSCTX_INPROC_SERVER
	ИнпроцОбработчмк  =  0x2,//CLSCTX_INPROC_HANDLER
	МестныйСервер    = 0x4,//CLSCTX_LOCAL_SERVER
	СерверВнутриПроцесса16 = 0x8,//CLSCTX_INPROC_SERVER16
	УдалённыйСервер   =  0x10,//CLSCTX_REMOTE_SERVER
	ОбработчикВнутриПроцесса16 = 0x20,//CLSCTX_INPROC_HANDLER16
    ВнутриПроцесса           = СерверВнутриПроцесса | ИнпроцОбработчмк,
    Сервер           = СерверВнутриПроцесса | МестныйСервер | УдалённыйСервер,
    Все              = СерверВнутриПроцесса | ИнпроцОбработчмк | МестныйСервер | УдалённыйСервер
}
...
enum ПДрэгДроп{
  Дроп = 0x00040100,
  Отмена = 0x00040101,
  ИспользоватьДефКурсоры = 0x00040102
}
...
enum /* LIBFLAGS */ : бкрат {
  БИБФЛАГ_ОГРАНИЧЕН = 0x1,//LIBFLAG_FRESTRICTED
  БИБФЛАГ_КОНТРОЛ = 0x2,//LIBFLAG_FCONTROL
  БИБФЛАГ_СКРЫТ = 0x4,//LIBFLAG_FHIDDEN
  БИБФЛАГ_ЕСТЬДИСКОБРАЗ = 0x8//LIBFLAG_FHASDISKIMAGE
}
