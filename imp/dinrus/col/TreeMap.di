...
class ДеревоКарта(К, З, alias ШаблРеализац=КЧДерево, alias функСравнить=ДефСравнить) : Карта!(К, З)
{
    ...
    ДеревоКарта удали(З з, ref бул былУдалён);
    ДеревоКарта удалиПо(К ключ);
    ДеревоКарта удалиПо(К ключ, ref бул былУдалён);
    ДеревоКарта удали(Обходчик!(К) поднабор);
    ДеревоКарта удали(Обходчик!(К) поднабор, ref бцел члоУдалённых);
    ДеревоКарта накладка(Обходчик!(К) поднабор, ref бцел члоУдалённых);
    ...
    ДеревоКарта установи(К ключ, З значение, ref бул былДобавлен);
    ...
    ДеревоКарта удалиВсе(З з, ref бцел члоУдалённых);
    ДеревоКарта dup();
    цел opEquals(Объект o);
    ДеревоКарта установи(З[К] исток);
    ДеревоКарта установи(З[К] исток, ref бцел члоДобавленных);
    ДеревоКарта удали(К[] поднабор);
    ДеревоКарта удали(К[] поднабор, ref бцел члоУдалённых);
    ДеревоКарта накладка(К[] поднабор);
    ДеревоКарта накладка(К[] поднабор, ref бцел члоУдалённых);
}
+/


/**
 * Implementation of the Карта interface using Красный-Чёрный trees.  this допускается for
...
class ДеревоКарта(К, З, alias ШаблРеализац=КЧДерево, alias функСравнить=ДефСравнить) : Карта!(К, З)
{
    ...
    /**
     * Removes all the elements whose ключи are in the поднабор.  Sets члоУдалённых
    ...
    ДеревоКарта накладка(Обходчик!(К) поднабор, ref бцел члоУдалённых)
    {
        ...
        члоУдалённых = _дерево.накладка(w);
        return this;
    }

    /**
     * removes all elements in the map whose ключи are NOT in поднабор.  Sets
     * члоУдалённых to the number of elements removed.
     *
     * returns this.
     */
    ДеревоКарта накладка(Обходчик!(К) поднабор)
    ...
    ДеревоКарта установи(К ключ, З значение, ref бул былДобавлен)
    {
        ...
        элт.ключ = ключ;
        элт.знач = значение;
        былДобавлен = _дерево.добавь(элт);
        return this;
    }
     ...
     * ключ that already exists will be overridden.
     *
     * sets члоДобавленных to the number of ключ значение pairs that were добавленный.
     *
     * returns this.
     */
    ДеревоКарта установи(З[К] исток, ref бцел члоДобавленных)
     ...
     * return this.
     *
     * члоУдалённых is установи to the number of elements removed.
     */
    ДеревоКарта удали(К[] поднабор, ref бцел члоУдалённых)
    {
        бцел оригДлина = длина;
     ...
     * Remove all the ключи that are not in the given массив.
     *
     * sets члоУдалённых to the number of elements removed.
     *
     * returns this.
     */
    ДеревоКарта накладка(К[] поднабор, ref бцел члоУдалённых)
