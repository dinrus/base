...
class ДеревоНабор(З, alias ШаблРеализац = КЧДеревоБезОбнова, alias функцСравнения = ДефСравнить) : Набор!(З)
{
    ...
    ДеревоНабор добавь(Обходчик!(З) обх, ref бцел члоДобавленных);
    ДеревоНабор добавь(З[] массив);
    ДеревоНабор добавь(З[] массив, ref бцел члоДобавленных);
    ДеревоНабор dup();
    ДеревоНабор удали(Обходчик!(З) поднабор);
    ДеревоНабор удали(Обходчик!(З) поднабор, ref бцел члоУдалённых);
    ДеревоНабор накладка(Обходчик!(З) поднабор);
    ДеревоНабор накладка(Обходчик!(З) поднабор, ref бцел члоУдалённых);
...
/**
 * Implementation of the Набор interface using Красный-Чёрный trees.  this допускается for
...
class ДеревоНабор(З, alias ШаблРеализац = КЧДеревоБезОбнова, alias функцСравнения = ДефСравнить) : Набор!(З)
{
    ...
    /**
     * Remove all the elements that match in the поднабор.  Sets члоУдалённых to
     ...
     * члоУдалённых to number of elements removed.
     *
     * returns this.
     */
    ДеревоНабор накладка(Обходчик!(З) поднабор, ref бцел члоУдалённых)
    {
        члоУдалённых = _дерево.накладка(поднабор);
        return this;
    }
