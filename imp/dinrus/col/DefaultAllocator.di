...
template ДефолтныйРазместитель(З)
{
    ...
    version(Динрус)
    {
        static if((З).sizeof + ((ук).sizeof * 3) + бцел.sizeof >= (РАЗМЕР_СТРАНИЦЫ - 1)  / 2)
        {
            alias ПростойРазместитель!(З) ДефолтныйРазместитель;
        }
        else
        {
            alias БлокРазместитель!(З, ((РАЗМЕР_СТРАНИЦЫ - 1) - ((ук ).sizeof * 3) - бцел.sizeof) / (З).sizeof) ДефолтныйРазместитель;
        }
    }
    else
    {
