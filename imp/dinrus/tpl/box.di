...
class РазбоксИскл : Исключение
{
    ...
    /**
     * Присвоить параметры и создать сообщение в форме
     * <tt>"Не удаётся разбоксировать из типа ... в ... ."</tt>
     */
...
{
    Бул, /**< бул */
    Бит = Бул,	// для обратной совместимости
    Целое, /**< байт, ббайт, крат, бкрат, цел, бцел, дол, бдол */
    Плав, /**< плав, дво, реал */
    Комплекс, /**< кплав, кдво, креал */
    Мнимое, /**< вплав, вдво, вреал */
    Класс, /**< Наследует у Объект */
    Указатель, /**< Указательный тип (T *) */
    Массив, /**< Массивный тип (T []) */
    Другой, /**< Любой второй тип, такой как делегаты, указатели на фунции, структ, проц... */
}


struct Бокс
{
    ИнфОТипе п_тип; /**< Тип содержимого объекта. */
    
    union
    {
        ук п_долДанные; /**< Массив oсодержимых объектов. */
        проц[8] п_кратДанные; /**< Данные, используемые для небольшого объекта. */
    }
    ...
    /**
     * Свойство для типа, содержащегося в этом боксе.
     * Изначально оно пусто и не может быть присвоено напрямую.
     * Возвращает: тип содержимого объекта.
     */
     ИнфОТипе тип()
    {
        return п_тип;
    ...
    /**
     * Свойство для данных, указывающих на значение бокса.
     * Изначально оно пусто и не может быть присвоено напрямую.
    ...
    protected бул opEqualsInternal(Бокс второй, бул инвертирован)
    {
        if (тип != второй.тип)
        {
            ...
            if (ta <= КлассТипа.Целое && tb <= КлассТипа.Целое)
            {
                ...
                if (na == "бдол"||na == "ulong" || nb == "бдол" || nb == "ulong")
    ...
    protected плав opCmpInternal(Бокс второй, бул инвертирован)
    {
        if (тип != второй.тип)
        {
            ...
            if (ta <= КлассТипа.Целое && tb == КлассТипа.Целое)
            {
                ...
                {
                    бдол va = изБокса!(бдол)(*this), vb = изБокса!(бдол)(второй);
                    return va > vb ? 1 : va < vb ? -1 : 0;
                }
                
                дол va = изБокса!(дол)(*this), vb = изБокса!(дол)(второй);
                return va > vb ? 1 : va < vb ? -1 : 0;
            }
            else if (tb == КлассТипа.Плав)
            {
                реал va = изБокса!(реал)(*this), vb = изБокса!(реал)(второй);
                return va > vb ? 1 : va < vb ? -1 : va == vb ? 0 : плав.nan;
            }
            else if (tb == КлассТипа.Комплекс)
            {
                креал va = изБокса!(креал)(*this), vb = изБокса!(креал)(второй);
                return va == vb ? 0 : плав.nan;
            }
            else if (tb == КлассТипа.Мнимое)
            {
                вреал va = изБокса!(вреал)(*this), vb = изБокса!(вреал)(второй);
                return va > vb ? 1 : va < vb ? -1 : va == vb ? 0 : плав.nan;
            }
