...
struct Кортеж(T ...) {
	...
	/**
		Статически получает кортежный тип с добавочным элементом в конце
	*/
	template добавьТ(X) {
...
template типыДинАргов(цел i, ПарамыФункц, СвязанныеАрги, цел minParamsLeft) {
	...
	template prependType(T, SkipType) {
		...
		static if (is(T == void)) {	// проц means that we aren't adding anything
			...
			alias tmp.мета.приставьТ!(T) рез;
		}
	}
...
template дайТипыДинАргов(ПарамыФункц, СвязанныеАрги, цел минАрговФции) {
	template цикл(цел i) {
		static if (i < члоДинАргов!(СвязанныеАрги).рез) {
			...
			alias цикл!(i+1).рез.мета.приставьТ!(argType) рез;
		} else {
			alias Кортеж!() рез;
		}
	}
...
template ИзвлечённыеСвязанныеАрги(СвязанныеАрги ...) {
	...
	// we'll store all non-dynamic arguments...
		...
		alias ИзвлечённыеСвязанныеАрги!(СвязанныеАрги[1..$]).мета.приставьТ!(СвязанныеАрги[0]) ИзвлечённыеСвязанныеАрги;
	}
...
template ParamsPassMethodTuple_impl(alias Func, цел i = 0) {
	...
	static if (Парамы.length == i) {
		...
		static if (is(typeof(Func(парамы.значение[0..i], constParam, парамы.значение[i+1..$])))) {
			alias ParamsPassMethodTuple_impl!(Func, i+1).рез.мета.приставьТ!(PassByCopy) рез;
		} else {
			alias ParamsPassMethodTuple_impl!(Func, i+1).рез.мета.приставьТ!(PassByRef) рез;
		}
	}
}
...
template FuncReferenceParamsAsPointers_impl(alias Func) {
	...
	template цикл(цел i) {
		static if (i == Парамы.length) {
			...
			alias цикл!(i+1).рез.мета.приставьТ!(тип) рез;
		}		
	}
...
template КортежУказателей(T) {
	static if (T.length > 0) {
		alias КортежУказателей!(Кортеж!(T.тип[1..$])).мета.приставьТ!(T.тип[0]*) КортежУказателей;
	} else {
