/*******************************************************************************
        Этот модуль вмещает множество полезных функций. Все они шаблонизированы,
		с намерением быть применимыми к массивам типа сим, шим и дим. Однако,
		они оперирируют неплохо и с другими типами массивов.
		
        Некоторые из них возвращают индексное значение, представляющее место,
		с которым идентифицированы какие-либо критерии. Когда заявленный
		критерий не находит соответствия, функции возвращают значение,
		представляющее длину массива, который был им предоставлен. То есть,
		для тех сценариев, в которых функции Си, как правило, могли бы вернуть
        -1, вместо этого эти функции возвращают длину. Это неплохо оперирует
        со срезами языка Ди:
        ---
        ...
        Функция содержит() удобна для тривиальных случаев поиска:
        ---
        if (содержит ("fubar", '!'))
            ...
        ---

        Заметьте, что некоторые функции ожидают тип т_мера как аргумент,и
        алгоритм шаблонной сверки Ди потерпит краш, если будет предоставлен
        вместо него тип цел. Это типичная причина ошибки "шаблон не найден".
        Также заметьте, что перегрузка имён в данный момент не поддерживается
        IFTI, и поэтому здесь не применима.
        ...
        разграничь (ист, набор)                          // разбей on delims
        разбей (исток, образец)                     // разбей on образец
        разбейнастр (исток);                        // разбей on строки
        ...
        подставь (исток, свер, замена)     // замени/удали совпадает
        ...
        кавычки (стр, набор)                           // foreach кавычки
        разграничители (стр, набор)                       // foreach разграничители
        образцы (стр, образец)                     // foreach образцы
        ---

        Please note that any 'образец' referred в_ внутри this module
...
/******************************************************************************
        ...
        Trim the given Массив by strИПping the предоставленный свер из_
...
/******************************************************************************
        ...
        Trim the given Массив by strИПping the предоставленный свер из_
        the лево hand sопрe. Возвращает срез of the original контент
...
/******************************************************************************
        ...
        Trim the given Массив by strИПping the предоставленный свер из_
        the право hand sопрe. Возвращает срез of the original контент
...
/******************************************************************************
        ...
        Chop the given исток by strИПping the предоставленный свер из_
        the лево hand sопрe. Возвращает срез of the original контент
...
/******************************************************************************
        ...
        Подставка все экземпляры of свер из_ исток. Набор замена
...
/******************************************************************************
        ...
        Счёт все экземпляры of свер внутри исток 
...
/******************************************************************************
        ...
        Возвращает whether or not the предоставленный Массив содержит an экземпляр
        of the given свер
...
/******************************************************************************
        ...
        разбей the предоставленный Массив on the первый образец экземпляр, и 
        return the результатant голова и хвост. The образец is excluded 
        из_ the two segments. 

        Where a сегмент is не найден, хвост will be пусто и the return
...
/******************************************************************************
        ...
        разбей the предоставленный Массив on the последний образец экземпляр, и 
        return the результатant голова и хвост. The образец is excluded 
        из_ the two segments. 

        Where a сегмент is не найден, голова will be пусто и the return
        значение will be the original Массив.
...
/******************************************************************************
        ...
        разбей the предоставленный Массив wherever a delimiter-набор экземпляр is
...
T[][] разграничь(T) (T[] ист, T[] набор)
{
        T[][] результат;

        foreach (сегмент; разграничители (ист, набор))
                 результат ~= сегмент;
        return результат;
}
...
/******************************************************************************
        ...
        разбей the предоставленный Массив wherever a образец экземпляр is
        найдено, и return the результатant segments. The образец is
...
T[][] разбей(T) (T[] ист, T[] образец)
{
        ...
        foreach (сегмент; образцы (ист, образец))
                 результат ~= сегмент;
        return результат;
}
...
/******************************************************************************
        ...
        буфер can be предоставленный в_ avoопр куча activity - it should be large 
        enough в_ contain the entire вывод, иначе the куча will be использован 
        instead.
...
/******************************************************************************
        ...
        Возвращает the индекс of the первый свер in стр, failing once
...
/******************************************************************************
        ...
        Возвращает the индекс of a не_совпадают between s1 & s2, failing when
...
/******************************************************************************
        ...
        Splits the предоставленный Массив wherever a delimiter-набор экземпляр is
        найдено, и return the результатant segments. The разграничители are
        excluded из_ each of the segments. Note that разграничители are
        matched as набор of alternates rather than как образец.
        ...
        ---
        foreach (сегмент; разграничители ("one,two;three", ",;"))
                 ...
        ---
        
******************************************************************************/

Разделоплод!(T) разграничители(T) (T[] ист, T[] набор)
{
        Разделоплод!(T) элементы;
        элементы.набор = набор;
        элементы.ист = ист;
        return элементы;
}
...
/******************************************************************************
        ...
        разбей the предоставленный Массив wherever a образец экземпляр is найдено, 
        и return the результатant segments. образец are excluded из_
        each of the segments, и an optional подст аргумент enables 
        замена.
        
        ---
        foreach (сегмент; образцы ("one, two, three", ", "))
                 ...
        ---
        
******************************************************************************/

ПлодОбразец!(T) образцы(T) (T[] ист, T[] образец, T[] подст=пусто)
{
        ПлодОбразец!(T) элементы;
        элементы.образец = образец;
        элементы.подст = подст;
        элементы.ист = ист;
        return элементы;
}
...
/******************************************************************************
        ...
        Обходчик для изоляции необязательно взятых в кавычки элементов текста.

        As per элементы(), but with the extension of being quote-aware;
        the набор of разграничители is ignored insопрe a пара of кавычки. Note
...
Кавычкоплод!(T) кавычки(T) (T[] ист, T[] набор)
{
...
/*******************************************************************************
        ...
        Квывод (выкладка (врем, "%1 is после %0", "zero", "one")).нс;
        ---
...
T[] выкладка(T) (T[] вывод, T[][] выкладка ...)
{
        ...
        static T[] toosmall = "{буфер вывода слишком мал}";
...
/******************************************************************************
        ...
        джейхэш() -- хэш a переменная-length ключ преобр_в a 32-bit значение
          ...
          ключ     : the ключ (the unaligned переменная-length Массив of байты)
        ...
        mod a prime (mod is sooo медленно!).  If you need less than 32 биты,
        use a bitmask.  Например, if you need only 10 биты, do

                    h = (h & hashmask(10));

        In which case, the хэш таблица should have hashsize(10) элементы.
        If you are hashing n strings (ub1 **)ключ, do it like this:

                    for (i=0, h=0; i<n; ++i) h = хэш( ключ[i], длин[i], h);
...
т_мера джейхэш (ббайт* ключ, т_мера длин, т_мера c = 0)
{
        т_мера a = 0x9e3779b9,
             b = 0x9e3779b9,
             i = длин;
        ...
        switch (i)
               {
               case 11: c+=(cast(бцел)ключ[10]<<24);
               case 10: c+=(cast(бцел)ключ[9]<<16);
               case 9 : c+=(cast(бцел)ключ[8]<<8);
               case 8 : b+=(cast(бцел)ключ[7]<<24);
               case 7 : b+=(cast(бцел)ключ[6]<<16);
               case 6 : b+=(cast(бцел)ключ[5]<<8);
               case 5 : b+=(cast(бцел)ключ[4]);
               case 4 : a+=(cast(бцел)ключ[3]<<24);
               case 3 : a+=(cast(бцел)ключ[2]<<16);
               case 2 : a+=(cast(бцел)ключ[1]<<8);
               case 1 : a+=(cast(бцел)ключ[0]);
               default:
               }
...
private struct Разделоплод(T)
{
        ...
        цел opApply (цел delegate (ref T[] токен) дг)
        {
                ...
                if (набор.length is 1)
                    while ((поз = местоположение (ист, набор[0], метка)) < ист.length)
                          {
                          токен = ист [метка .. поз];
                          if ((возвр = дг (токен)) != 0)
                               return возвр;
...
debug (UnitTest)
{
        ...
        assert (убериИскейп ("\\знач\\vx") == "\v\vx");
