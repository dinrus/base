 ...
 *******************************************************************************/
module dwtx.jface.dialogs.TrayDialog;
 ...
 * </p>
 *
 * @see dwtx.jface.dialogs.DialogTray
...
public abstract class TrayDialog : Dialog {
    ...
    protected override Control createButtonBar(Composite parent) {
        ...
        composite.setLayoutData(new GridData(DWT.FILL, DWT.CENTER, false, false));
    ...
    private ToolBar createHelpImageButton(Composite parent, Image image) {
        ToolBar toolBar = new ToolBar(parent, DWT.FLAT | DWT.NO_FOCUS);
        ...
        ToolItem item = new ToolItem(toolBar, DWT.NONE);
        item.setImage(image);
    ...
    private Link createHelpLink(Composite parent) {
        Link link = new Link(parent, DWT.WRAP | DWT.NO_FOCUS);
        (cast(GridLayout) parent.getLayout()).numColumns++;
    ...
    private void helpPressed() {
        if (getShell() !is null) {
            ...
            while (c !is null) {
                if (c.isListening(DWT.Help)) {
                    c.notifyListeners(DWT.Help, new Event());
                    break;
                }
                c = c.getParent();
            }
    ...
    public void openTray(DialogTray tray) {
        ...
        Shell shell = getShell();
        leftSeparator = new Label(shell, DWT.SEPARATOR | DWT.VERTICAL);
        leftSeparator.setLayoutData(new GridData(GridData.FILL_VERTICAL));
        sash = new Sash(shell, DWT.VERTICAL);
        sash.setLayoutData(new GridData(GridData.FILL_VERTICAL));
        rightSeparator = new Label(shell, DWT.SEPARATOR | DWT.VERTICAL);
        ...
        data.widthHint = trayControl.computeSize(DWT.DEFAULT, clientArea.height).x;
