...
private extern(C) {
    void        _Dmain();
    void        D2rt4core6thread5Fiber3runMFZv();
}
private {
            ...
            for (i = 1; p && !IsBadReadPtr(p, Layout.sizeof) && i < maxHeapSpace && !IsBadReadPtr(cast(void*)p.ret, 4);) {
                ...
                version (StacktraceTryToBeSmart) {
                    ...
                    if  (
                            ...
                            || true == (inFiber = (
                                callAddr >= cast(uint)&D2rt4core6thread5Fiber3runMFZv
                                && callAddr < cast(uint)&D2rt4core6thread5Fiber3runMFZv + fiberRunFuncLength
                            ))
                        )
                    {
                        foundMain = true;
...
static this() {
    ...
    for (fiberRunFuncLength = 0; fiberRunFuncLength < 0x100; ++fiberRunFuncLength) {
        ubyte* ptr = cast(ubyte*)&D2rt4core6thread5Fiber3runMFZv + fiberRunFuncLength;
        enum {
            RetOpcode = 0xc3
        }
