...
class СтэкНить
{
    ...
    invariant
    {
        ...
        if(лев !is пусто)
        {
            assert(лев.родитель is this);
        }

        if(прав !is пусто)
        {
            assert(прав.родитель is this);
        }
    }
    ...
    private СтэкНить лев = пусто;
    private СтэкНить прав = пусто;
...
public:
    ...
    in
    {
        ...
        assert(сн.лев is пусто);
        assert(сн.прав is пусто);
    }
    body
    {
        размер++;
        ...
        if(поз&1)
        {
            assert(врм.лев !is пусто);
            assert(врм.прав is пусто);
            врм.прав = сн;
        }
        else
        {
    ...
    body
    {
        ...
        //Удалить врм
        if(врм.родитель.лев is врм)
        {
            врм.родитель.лев = пусто;
        }
        else
        {
            assert(врм.родитель.прав is врм);
            врм.родитель.прав = пусто;
        }
        врм.родитель = пусто;
        размер--;
        ...
        {
            if(врм.родитель.лев is сн)
            {
                врм.родитель.лев = врм;
            }
            else
            {
                assert(врм.родитель.прав is сн);
                врм.родитель.прав = врм;
            }
        }

        //Фиксировать врм's лев
        врм.лев = сн.лев;
        if(врм.лев !is пусто)
        {
            врм.лев.родитель = врм;
        }

        //Фиксировать врм's прав
        врм.прав = сн.прав;
        if(врм.прав !is пусто)
        {
            врм.прав.родитель = врм;
        }
    ...
    out(r)
    {
        ...
        assert(r.прав is пусто);
        assert(r.лев is пусто);
    }
    body
    {
        ...
        //Add врм to верх
        врм.лев = голова.лев;
        врм.прав = голова.прав;
        if(врм.лев !is пусто) врм.лев.родитель = врм;
        if(врм.прав !is пусто) врм.прав.родитель = врм;
    ...
    debug (PQueue) проц печать()
    {
        ...
        while(стэк.length > 0)
        {
            ...
            if(врм !is пусто)
            {
                ...
                if(врм.лев !is пусто)
                {
                    assert(врм.лев.м_приоритет <= врм.м_приоритет);
                    стэк ~= врм.лев;
                }

                if(врм.прав !is пусто)
                {
                    assert(врм.прав.м_приоритет <= врм.м_приоритет);
                    стэк ~= врм.прав;
                }
    ...
    проц вспень(СтэкНить сн)
    {
        ...
        while(сн.родитель !is пусто)
        {
            ...
            {
                if(сн.родитель.лев is a)
                {
                    сн.родитель.лев = сн;
                }
                else
                {
                    assert(сн.родитель.прав is a);
                    сн.родитель.прав = сн;
                }
            ...
            if(a.лев is сн)
            {
                ...
                if(сн.прав !is пусто) сн.прав.родитель = сн;
            }
            else
            {
                a.прав = сн.прав;
                ...
                if(сн.лев !is пусто) сн.лев.родитель = сн;
            }

            if(a.прав !is пусто) a.прав.родитель = a;
            if(a.лев !is пусто) a.лев.родитель = a;
    ...
    проц запень(СтэкНить сн)
    {
        while(сн.лев !is пусто)
        {
            ...
            if(сн.прав is пусто ||
                    сн.лев.м_приоритет >= сн.прав.м_приоритет)
            {
                if(сн.лев.м_приоритет > сн.м_приоритет)
                {
                    a = сн.лев;
                    assert(a);
            ...
            else if(сн.прав.м_приоритет > сн.м_приоритет)
            {
                a = сн.прав;
                assert(a);
            ...
            {
                if(a.родитель.лев is сн)
                {
                    a.родитель.лев = a;
                }
                else
                {
                    assert(a.родитель.прав is сн);
                    a.родитель.прав = a;
                }
            }
            else
