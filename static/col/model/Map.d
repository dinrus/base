...
interface Карта(К, З) : СКлючом!(К, З), Коллекция!(З), Мульти!(З)
{
     ...
     * Возвращает.
     *
     * чло_добавленных is установи to the number of элементы that were добавленный.
     */
     ...
     * Возвращает.
     *
     * чло_добавленных is установи to the number of элементы добавленный.
     */
    ...
    /**
     * Remove all the ключи that are not in the given обходчик.
     *
     * returns this.
     */
    ...
    /**
     * Get a установи of the ключи that the map содержит.  This is not a копируй of the
     * ключи, but an actual "window" into the ключи of the map.  If you добавь
     * значения to the map, they will show up in the ключи обходчик.
     *
     * This is not in СКлючом, because some СКлючом containers do not have user
     * defined ключи, и so this would be not quite that useful there.
     */
    Обходчик!(К) ключи();
     ...
     */
    Карта!(К, З) удали(З знач);

    /**
     * Совариант удали (из Коллекция)
     */
    Карта!(К, З) удали(З знач, ref бул удалён_ли);

    /**
     * Совариант удалиВсе (из Мульти)
     */
    Карта!(К, З) удалиВсе(З знач);

    /**
     * Совариант удалиВсе (из Мульти)
     */
    Карта!(К, З) удалиВсе(З знач, ref бцел чло_Удалённых);

    /**
     * Совариант удалиПо (из СКлючом)
     */
     ...
     * Совариант удалиПо (из СКлючом)
     */
    Карта!(К, З) удалиПо(К ключ, ref бул удалён_ли);

    /**
     * Совариант установи (из СКлючом)
     */
    ...
    /**
     * сравни two maps.  Returns true if both maps have the same number of
     * элементы, и both maps have элементы whose ключи и значения are equal.
     *
