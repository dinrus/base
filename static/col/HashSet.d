...
class ХэшНабор(З, alias ШаблРеализац=ХэшБезОбновлений, alias хэшФункц=ДефХэш) : Набор!(З)
{
     ...
     * foreach(ref чистить_ли, знач; &hashSet.очистить)
     * {
     *   чистить_ли = ((знач & 1) == 1);
     * }
     */
     ...
     * Runs in O(n) time.
     */
    ХэшНабор удали(З знач, ref бул удалён_ли)
    {
        курсор обх = найди(знач);
        if(обх == конец)
        {
     ...
     * Runs on average in O(1) time.
     */
    ХэшНабор добавь(З знач, ref бул был_добавлен)
    {
