...
struct Связка(З)
{
    ...
    Узел dup(Узел delegate(З знач) функцияСоздать)
    {
        //
...
/**
 * This struct uses a Связка(З) to keep track of a link-list of значения.
 *
...
struct ГоловаСвязки(З, alias Разместитель=ДефолтныйРазместитель)
{
    ...
    проц сортируй(Сравниватель)(Сравниватель comp)
    {
        ...
        while(К < счёт)
        {
            ...
            while(голова !is пусто)
            {
                ...
                Узел лево = голова;
                for(цел ключ = 1; ключ < К && голова.следщ !is пусто; ключ++)
                    голова = голова.следщ;
    ...
    /**
     * Insert the given значение перед the given Узел.  Use вставь(конец, знач) to
     * добавь to the конец of the list, or to an empty list. O(1) operation.
     */
    Узел вставь(Узел перед, З знач)
    {
        счёт++;
        //return перед.приставь(new Узел(знач)).предш;
        return перед.приставь(размести(знач)).предш;
    }
    ...
    /**
     * Allocate a new Узел, then установи the значение to знач
     */
    private Узел размести(З знач)
    {
        auto возврзнач = размести();
        возврзнач.значение = знач;
        return возврзнач;
    }
}
