...
class FileException : Exception
{
	...
	wchar* soob =cast(wchar*)"Неудачная операция с файлом\n";

    this(string name)
    {
	this(name, "file I/O");	
		ОкноСооб(null, cast(wchar*) soob, "D рантайм:Файловое Исключение", СО_ОК|СО_ПИКТОШИБКА);
    }

    this(string name, string message)
    {
	super(name ~ ": " ~ message);	
		ОкноСооб(null, cast(wchar*) ("Неудачная операция с файлом\n"~toUTF16(message)), "D рантайм:Файловое Исключение", СО_ОК|СО_ПИКТОШИБКА);
    }
...
void write(char[] name, void[] buffer)
{
    ...
    if (os.windows.WriteFile(h,buffer.ptr,buffer.length,&numwritten,null) != 1)
	goto err2;
...
struct DirEntry
{
	...
	clength = std.c.strlen(fd.cFileName.ptr);

	// Convert cFileName[] to unicode
	wlength = os.windows.MultiByteToWideChar(0,0,fd.cFileName.ptr,clength,null,0);
	if (wlength > wbuf.length)
	    wbuf.length = wlength;
	n = os.windows.MultiByteToWideChar(0,0,fd.cFileName.ptr,clength,cast(wchar*)wbuf,wlength);
	assert(n == wlength);
...
void listdir(string pathname, bool delegate(DirEntry* de) callback)
{
	...
	if (h != INVALID_HANDLE_VALUE)	// should we throw exception if invalid?
	{
		...
		{
		    // Skip "." and ".."
		    if (std.c.strcmp(fileinfo.cFileName.ptr, ".") == 0 ||
			std.c.strcmp(fileinfo.cFileName.ptr, "..") == 0)
			continue;
 ...
 * Deprecated: use os.win.charset.toMBSz instead.
 */

char* toMBSz(string s)
{
    return os.win.charset.toMBSz(s);
}
...
{
    if (std.c.remove(toStringz(name)) == -1)
	throw new FileException(name, getErrno());
}
...
alias name имя;
    ...
    {	size_t len = std.c.strlen(fd.d_name.ptr);
...
void listdir(string pathname, bool delegate(DirEntry* de) callback)
{
	...
	try
	{
	    ...
	    {
		// Skip "." and ".."
		if (!std.c.strcmp(fdata.d_name.ptr, ".") ||
		    !std.c.strcmp(fdata.d_name.ptr, ".."))
			continue;
...
void copy(string from, string to)
{
  version (all)
  {
    ...
    void* buf = std.c.malloc(BUFSIZ);
    if (!buf)
    {	BUFSIZ = 4096;
	buf = std.c.malloc(BUFSIZ);
    }
    if (!buf)
    {
        //printf("\topen error, errno = %d\n",getErrno());
