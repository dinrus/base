...
alias toAddrString вТкстАдр;
	...
	const uint АДР_ЛЮБОЙ = INADDR_ANY;	/// Any IPv4 address number.
	const uint АДР_НУ = INADDR_NONE;	/// An invalid IPv4 address number.
...
enum ЭкстрЗакрытиеСокета: цел
{
	ПРИЁМ =  SD_RECEIVE,	/// socket receives are disallowed
	ОТПРАВКА =     SD_SEND,	/// socket sends are disallowed
	ОБА =     SD_BOTH,	/// both RECEIVE and SEND
}
