...
+/
    ...
    static if (vector_type.is_float) {
        ...
        Scalar norm() /*const*/ { 
            return cast(Scalar)std.math.sqrt(sqrnorm());
        }
        ...
        Scalar onenorm() {
            Scalar ret=0;
            foreach(v; values_) ret += std.math.abs(v);
            return ret;
        }

        /// Return the infinity-norm of the vector (max element std.math.absolute value)
        Scalar infnorm() {
