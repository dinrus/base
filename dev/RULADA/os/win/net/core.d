...
class NetException : Exception {
  ...
  this() {
    super(getErrorMessage(os.win.base.native.GetLastError()));
  }
  ...
  private static string getErrorMessage(uint errorCode) {
    ...
    wchar[256] buffer;
    uint result = FormatMessage(os.win.base.native.FORMAT_MESSAGE_FROM_HMODULE, moduleHandle, errorCode, 0, buffer.ptr, buffer.length + 1, null);
    if (result != 0)
      return .toUTF8(buffer[0 .. result]);

    // Otherwise, get the error message from Windows.
    result = FormatMessage(os.win.base.native.FORMAT_MESSAGE_FROM_SYSTEM, null, errorCode, 0, buffer.ptr, buffer.length + 1, null);
    if (result != 0)
      return .toUTF8(buffer[0 .. result]);
