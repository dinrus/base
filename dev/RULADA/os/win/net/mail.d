...
private void setProperty(IDispatch target, string name, ...) {
  try {
    os.win.com.core.setProperty(target, name, _arguments, _argptr);
  }
  catch (Exception e) {
    throw new SmtpException(e.msg);
  }
...
class SmtpClient {
  ...
  final void send(MailMessage message) {
    ...
    if (auto fields = getProperty!(IDispatch)(m, "Fields")) {
      ...
      if (auto fieldsconst = invokeMethod!(IconstVARIANT)(fields, "_NewEnum")) {
        scope(exit) tryRelease(fieldsconst);

        uint fetched;
        VARIANT field;

        while (SUCCEEDED(fieldsconst.Next(1, &field, fetched)) && fetched == 1) {
...
class MailAddress {
  ...
  private void parse(string address) {
    ...
    if (i > 0)
      throw new FormatException("Строка не соответствует форме, необходимой для адреса электронной почты.");
...
class MailMessage {
  ...
  this(string from, string to) {
    if (from == null)
      throw new ArgumentException("Параметр 'from' не может быть пустой строкой.", "from");
    if (to == null)
      throw new ArgumentException("Параметр 'to' не может быть пустой строкой.", "to");
