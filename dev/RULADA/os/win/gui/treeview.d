...
class TreeNode: DObject
{
	...
	final void text(Dstring newText) // setter
	{
		...
		if(created)
		{
			...
			{
				item.pszText = cast(typeof(item.pszText))os.win.gui.x.utf.toUnicodez(ttext);
				m = Message(tview.handle, TVM_SETITEMW, 0, cast(LPARAM)&item);
			}
			else
			{
				item.pszText = cast(typeof(item.pszText))os.win.gui.x.utf.unsafeAnsiz(ttext);
				m = Message(tview.handle, TVM_SETITEMA, 0, cast(LPARAM)&item);
			}
			tview.prevWndProc(m);
		}
...
class TreeNodeCollection
{
	...
	package void populateInsertChildNode(inout Message m, inout TV_ITEMA dest, TreeNode node)
	{
		with(dest)
		{
			...
			if(os.win.gui.x.utf.useUnicode)
			{
				pszText = cast(typeof(pszText))os.win.gui.x.utf.toUnicodez(node.text);
				m.hWnd = tview.handle;
				m.msg = TVM_INSERTITEMW;
			}
			else
			{
				pszText = cast(typeof(pszText))os.win.gui.x.utf.unsafeAnsiz(node.text);
				m.hWnd = tview.handle;
	...
	void _removing(size_t idx, TreeNode val)
	{
		if(size_t.max == idx) // Clearing all...
		{
			...
			_nodes = _nodes[0 .. 0]; // Not nice to os.win.gui.collections, but OK.
			if(created)
			{
				Message m;
				m.hWnd = tview.handle;
...
class TreeView: ControlSuperClass // docmain
{
	...
	protected override void prevWndProc(inout Message msg)
	{
		...
		msg.result = os.win.gui.x.utf.callWindowProc(treeviewPrevWndProc, msg.hWnd, msg.msg, msg.wParam, msg.lParam);
	}
	...
	LRESULT prevwproc(UINT msg, WPARAM wparam, LPARAM lparam)
	{
		...
		return os.win.gui.x.utf.callWindowProc(treeviewPrevWndProc, hwnd, msg, wparam, lparam);
	}
}
