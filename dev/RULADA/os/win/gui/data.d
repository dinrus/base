...
class DataFormats // docmain
{
	...
	Format getFormat(Dstring name)
	{
		...
		// Didn't find it.
		return _didntFindId(os.win.gui.x.utf.registerClipboardFormat(name));
	}
	...
	{
		Dstring result;
		result = os.win.gui.x.utf.getClipboardFormatName(id);
		if(!result.length)
	...
	void[] getCbFileDrop(Dstring[] fileNames)
	{
		...
		{
			sz += (os.win.gui.x.utf.toUnicodeLength(fn) + 1) << 1;
		}
		sz += 2;
...
package Data _doConvertFormat(Data dat, Dstring toFmt)
{
	...
	if(!stringICmp(toFmt, "UTF-8"))
	{
		...
		else if(typeid(ubyte[]) == dat.info)
		{
			ubyte[] ubs;
			ubs = dat.getText();
			result = Data(os.win.gui.x.utf.fromAnsi(cast(Dstringz)ubs.ptr, ubs.length));
		}
	}
	else if(!stringICmp(toFmt, DataFormats.unicodeText))
	{
		...
		else if(typeid(ubyte[]) == dat.info)
		{
			ubyte[] ubs;
			ubs = dat.getText();
			result = Data(os.win.gui.x.utf.ansiToUnicode(cast(Dstringz)ubs.ptr, ubs.length));
		}
	}
	else if(!stringICmp(toFmt, DataFormats.text))
	{
		if(typeid(Dstring) == dat.info)
		{
			result = Data(cast(ubyte[])os.win.gui.x.utf.toAnsi(dat.getString()));
		}
...
class ComToDdataObject: os.win.gui.data.IDataObject // package
{
	...
	Dstring[] getFormats()
	{
		IconstFORMATETC fconst;
		FORMATETC fmte;
		Dstring[] result;
		ULONG nfetched = 1; // ?
		
		if(S_OK != dataObj.constFormatEtc(1, &fconst))
			throw new DflException("Unable to get formats");
		
		fconst.AddRef(); // ?
		for(;;)
		{
			if(S_OK != fconst.Next(1, &fmte, &nfetched))
				break;
			if(!nfetched)
				break;
...
package class constDataObjectFORMATETC: DflComObject, IconstFORMATETC
{
	...
	{
		if(*riid == _IID_IconstFORMATETC)
		{
			*ppv = cast(void*)cast(IconstFORMATETC)this;
			AddRef();
			return S_OK;
		}
	...
	HRESULT Clone(IconstFORMATETC* ppconst)
	{
		HRESULT result;
		
		try
		{
			*ppconst = new constDataObjectFORMATETC(dataObj, fmts, idx);
			result = S_OK;
		}
		catch(Object e)
		{
...
class DtoComDataObject: DflComObject, os.win.gui.x.wincom.IDataObject // package
{
	...
	override HRESULT QueryInterface(IID* riid, void** ppv)
	{
		...
		{
			*ppv = cast(void*)cast(os.win.gui.x.wincom.IDataObject)this;
			AddRef();
			return S_OK;
		}
		else if(*riid == _IID_IUnknown)
	...
	HRESULT constFormatEtc(DWORD dwDirection, IconstFORMATETC* ppconstFormatetc)
	{
		// SHCreateStdconstFmtEtc() requires Windows 2000 +
		...
		try
		{
			...
			{
				*ppconstFormatetc = new constDataObjectFORMATETC(dataObj);
				result = S_OK;
			}
			else
			{
	...
	HRESULT constDAdvise(IconstSTATDATA* ppconstAdvise)
	{
		return E_UNEXPECTED; // TODO: finish.
	}
