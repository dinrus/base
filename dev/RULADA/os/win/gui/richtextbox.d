...
class RichTextBox: TextBoxBase // docmain
{
	...
	override Dstring selectedText() // getter
	{
		if(created)
		{
			...
			return os.win.gui.x.utf.emGetSelText(hwnd, selectionLength + 1);
		}
		return null;
	}
	...
	body
	{
		...
		os.win.gui.x.utf.callWindowProc(richtextboxPrevWndProc, hwnd, EM_GETCHARFORMAT, selection, cast(LPARAM)cf);
	}
	...
	body
	{
		...
		if(!os.win.gui.x.utf.callWindowProc(richtextboxPrevWndProc, hwnd, EM_SETCHARFORMAT, scf, cast(LPARAM)cf))
			throw new DflException("Unable to set text formatting");
		+/
		os.win.gui.x.utf.callWindowProc(richtextboxPrevWndProc, hwnd, EM_SETCHARFORMAT, scf, cast(LPARAM)cf);
	}
	...
	body
	{
		...
		max = os.win.gui.x.utf.sendMessage(handle, EM_GETTEXTRANGE, 0, cast(LPARAM)&tr);
		Dstring result;
		if(os.win.gui.x.utf.useUnicode)
	...
	override void prevWndProc(inout Message m)
	{
		...
		//m.result = os.win.gui.x.utf.callWindowProc(richtextboxPrevWndProc, m.hWnd, m.msg, m.wParam, m.lParam);
	}
