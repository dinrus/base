...
{
	return cast(DialogResult)os.win.gui.x.utf.messageBox(GetActiveWindow(), txt, \0, MB_OK);
}

/// ditto
DialogResult msgBox(IWindow owner, Dstring txt) // docmain
{
	return cast(DialogResult)os.win.gui.x.utf.messageBox(owner ? owner.handle : GetActiveWindow(),
		txt, \0, MB_OK);
}
...
{
	return cast(DialogResult)os.win.gui.x.utf.messageBox(GetActiveWindow(), txt, caption, MB_OK);
}
...
{
	return cast(DialogResult)os.win.gui.x.utf.messageBox(GetActiveWindow(), txt, caption, buttons);
}
...
{
	return cast(DialogResult)os.win.gui.x.utf.messageBox(GetActiveWindow(), txt,
