...
class FolderBrowserDialog: CommonDialog // docmain
{
	...
	final void showNewFolderButton(bool byes) // setter
	{
		...
		// Might need to const child windows looking for window title
	...
	protected override bool runDialog(HWND owner)
	{
		...
		//void* pdescz = std.c.alloca(wchar.sizeof * MAX_PATH);
		//if(!pdescz)
		...
		if(os.win.gui.x.utf.useUnicode)
		{
			...
			biw.lpszTitle = os.win.gui.x.utf.toUnicodez(_desc);
			
			biw.pszDisplayName = cast(wchar*)pdescz;
			if(_desc.length)
			{
				Dwstring tmp;
				tmp = os.win.gui.x.utf.toUnicode(_desc);
			...
			//wchar* wbuf = cast(wchar*)std.c.alloca(wchar.sizeof * MAX_PATH);
			...
			_selpath = os.win.gui.x.utf.fromUnicodez(wbuf.ptr); // Assumes fromUnicodez() copies.
		...
		{
			bia.lpszTitle = os.win.gui.x.utf.toAnsiz(_desc);
			
			bia.pszDisplayName = cast(char*)pdescz;
			if(_desc.length)
			{
				Dstring tmp; // ansi.
				tmp = os.win.gui.x.utf.toAnsi(_desc);
				if(tmp.length >= MAX_PATH)
					_errPathTooLong();
			...
			//char* abuf = cast(char*)std.c.alloca(char.sizeof * MAX_PATH);
			...
			_selpath = os.win.gui.x.utf.fromAnsiz(abuf.ptr); // Assumes fromAnsiz() copies.
		...
		if(fd)
		{
			...
			switch(msg)
			{
				case BFFM_INITIALIZED:
					...
					{
						if(os.win.gui.x.utf.useUnicode)
							SendMessageA(hwnd, BFFM_SETSELECTIONW, TRUE, cast(LPARAM)os.win.gui.x.utf.toUnicodez(s));
						else
							SendMessageA(hwnd, BFFM_SETSELECTIONA, TRUE, cast(LPARAM)os.win.gui.x.utf.toAnsiz(s));
					}
					break;
