...
class ComboBox: ListControl // docmain
{
	...
	final int findString(Dstring str, int startIndex)
	{
		...
		if(isHandleCreated)
		{
			if(os.win.gui.x.utf.useUnicode)
				result = prevwproc(CB_FINDSTRING, startIndex, cast(LPARAM)os.win.gui.x.utf.toUnicodez(str));
			else
				result = prevwproc(CB_FINDSTRING, startIndex, cast(LPARAM)os.win.gui.x.utf.unsafeAnsiz(str));
	...
	final int findStringExact(Dstring str, int startIndex)
	{
		...
		{
			if(os.win.gui.x.utf.useUnicode)
				result = prevwproc(CB_FINDSTRINGEXACT, startIndex, cast(LPARAM)os.win.gui.x.utf.toUnicodez(str));
			else
				result = prevwproc(CB_FINDSTRINGEXACT, startIndex, cast(LPARAM)os.win.gui.x.utf.unsafeAnsiz(str));
			if(result == CB_ERR) // Redundant.
				result = NO_MATCHES;
		}
	...
	uint textLength() // getter
	{
		if(!(ctrlStyle & ControlStyles.CACHE_TEXT) && isHandleCreated)
			...
			return cast(uint)os.win.gui.x.utf.sendMessage(handle, WM_GETTEXTLENGTH, 0, 0);
		return wtext.length;
	}
	...
	static class ObjectCollection
	{
		...
		void addRange(Object[] range)
		{
			...
			{
				throw new Exception("Range cannot be added. Exception in os.win.gui.combobox.d, 574 line");//_wraparray.addRange(range);
			}
		}
		...
		void _added(size_t idx, Object val)
		{
			...
			{
				if(os.win.gui.x.utf.useUnicode)
					lbox.prevwproc(CB_INSERTSTRING, idx, cast(LPARAM)os.win.gui.x.utf.toUnicodez(getObjectString(val)));
				else
					lbox.prevwproc(CB_INSERTSTRING, idx, cast(LPARAM)os.win.gui.x.utf.toAnsiz(getObjectString(val))); // Can this be unsafeAnsiz()?
			}
		}
	...
	protected override void onHandleCreated(EventArgs ea)
	{
		...
		if(os.win.gui.x.utf.useUnicode)
		{
			...
			{
				m.wParam = i;
				m.lParam = cast(LPARAM)os.win.gui.x.utf.toUnicodez(getObjectString(obj)); // <--
		...
		else
		{
			...
			{
				m.wParam = i;
				m.lParam = cast(LPARAM)os.win.gui.x.utf.toAnsiz(getObjectString(obj)); // Can this be unsafeAnsiz()? // <--
	...
	override void createHandle()
	{
		...
		Message m;
		if(os.win.gui.x.utf.useUnicode)
			m = Message(hwnd, WM_SETTEXT, 0, cast(LPARAM)os.win.gui.x.utf.toUnicodez(ft));
		else
			m = Message(hwnd, WM_SETTEXT, 0, cast(LPARAM)os.win.gui.x.utf.toAnsiz(ft)); // Can this be unsafeAnsiz()?
		prevWndProc(m);
	}
	...
	override void prevWndProc(inout Message msg)
	{
		...
		msg.result = os.win.gui.x.utf.callWindowProc(comboboxPrevWndProc, msg.hWnd, msg.msg, msg.wParam, msg.lParam);
	}
	...
	LRESULT prevwproc(UINT msg, WPARAM wparam, LPARAM lparam)
	{
		...
		return os.win.gui.x.utf.callWindowProc(comboboxPrevWndProc, hwnd, msg, wparam, lparam);
	}
}
