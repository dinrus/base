...
abstract class FileDialog: CommonDialog // docmain
{
	...
	final void defaultExt(Dstring ext) // setter
	{
		...
		else
		{
			...
			{
				ofnw.lpstrDefExt = os.win.gui.x.utf.toUnicodez(ext);
			}
			else
			{
				ofna.lpstrDefExt = os.win.gui.x.utf.toAnsiz(ext);
			}
			_defext = ext;
		}
	}
			...
			for(i = starti = 0; i != filterString.length; i++)
			{
				switch(filterString[i])
				{
					case '|':
						...
						if(os.win.gui.x.utf.useUnicode)
						{
							str.sw ~= os.win.gui.x.utf.toUnicode(filterString[starti .. i]);
							str.sw ~= "\0";
						}
						else
						{
							str.sa ~= os.win.gui.x.utf.unsafeAnsi(filterString[starti .. i]);
							str.sa ~= "\0";
						}
	...
	final void initialDirectory(Dstring dir) // setter
	{
		...
		else
		{
			if(os.win.gui.x.utf.useUnicode)
			{
				ofnw.lpstrInitialDir = os.win.gui.x.utf.toUnicodez(dir);
			}
			else
			{
				ofna.lpstrInitialDir = os.win.gui.x.utf.toAnsiz(dir);
			}
			_initDir = dir;
		}
	}
	...
	final void title(Dstring newTitle) // setter
	{
		...
		else
		{
			if(os.win.gui.x.utf.useUnicode)
			{
				ofnw.lpstrTitle = os.win.gui.x.utf.toUnicodez(newTitle);
			}
			else
			{
				ofna.lpstrTitle = os.win.gui.x.utf.toAnsiz(newTitle);
			}
			_title = newTitle;
		}
	}
	...
	void beginOfn(HWND owner)
	{
		if(os.win.gui.x.utf.useUnicode)
		{
			...
			{
				Dwstring ts;
				ts = os.win.gui.x.utf.toUnicode(_fileNames[0]);
				buf[0 .. ts.length] = ts;
				buf[ts.length] = 0;
			}
		...
		else
		{
			...
			{
				Dstring ts;
				ts = os.win.gui.x.utf.unsafeAnsi(_fileNames[0]);
				buf[0 .. ts.length] = ts;
				buf[ts.length] = 0;
			}
	...
	body
	{
		if(ofn.Flags & OFN_ALLOWMULTISELECT)
		{
			...
			if(os.win.gui.x.utf.useUnicode)
			{
				...
				for(;;)
				{
					if(!*p)
					{
						_fileNames ~= os.win.gui.x.utf.fromUnicode(startp, p - startp); // dup later.
			...
			else
			{
				...
				for(;;)
				{
					if(!*p)
					{
						_fileNames ~= os.win.gui.x.utf.fromAnsi(startp, p - startp); // dup later.
		...
		else
		{
			...
			if(os.win.gui.x.utf.useUnicode)
			{
				_fileNames[0] = os.win.gui.x.utf.fromUnicodez(ofnw.lpstrFile);
			}
			else
			{
				_fileNames[0] = os.win.gui.x.utf.fromAnsiz(ofna.lpstrFile);
			}
