...
class Bitmap: Image // docmain
{
	...
	{
		this.hbm = cast(HBITMAP)os.win.gui.x.utf.loadImage(null, fileName, IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);
		if(!this.hbm)
	...
	private void _getInfo(BITMAP* bm)
	{
		if(GetObjectA(hbm, BITMAP.sizeof, bm) != BITMAP.sizeof)
			throw new DflException("Не удаётся получить image information");
	}
...
class Picture: Image // docmain
{
	...
	{
		os.win.gui.x.wincom.IPicture ipic;
	...
	{
		alias os.win.gui.x.winapi.HANDLE HANDLE; // Otherwise, odd conflict with wine.
		...
		hf = os.win.gui.x.utf.createFile(fileName, GENERIC_READ, FILE_SHARE_READ, null,
...
class Graphics // docmain
{
	...
	final void drawText(Dstring text, Font font, Color color, Rect r, TextFormat fmt)
	{
		...
		RECT rect;
		r.getRect(&rect);
		os.win.gui.x.utf.drawTextEx(hdc, text, &rect, DT_EXPANDTABS | DT_TABSTOP |
	...
	final Size measureText(Dstring text, Font font)
	{
		...
		os.win.gui.x.utf.getTextExtentPoint32(hdc, text, &sz);
	...
	final Size measureText(Dstring text, Font font, int maxWidth, TextFormat fmt)
	{
		...
		if(!os.win.gui.x.utf.drawTextEx(hdc, text, &rect, DT_EXPANDTABS | DT_TABSTOP |
			...
			case TextTrimming.ELLIPSIS_PATH:
				{
					...
					if(!os.win.gui.x.utf.drawTextEx(hdc, newtext, &rect, DT_EXPANDTABS | DT_TABSTOP |
	...
	final void drawRectangle(Pen pen, int x, int y, int width, int height)
	{
		...
		os.win.gui.x.winapi.Rectangle(hdc, x, y, x + width, y + height);
	...
	final void drawRectangles(Pen pen, Rect[] rs)
	{
		...
		{
			os.win.gui.x.winapi.Rectangle(hdc, r.x, r.y, r.x + r.width, r.y + r.height);
		}
	...
	{
		return cast(bool)os.win.gui.x.winapi.BitBlt(dest, destX, destY, width, height, this.handle, srcX, srcY, rop);
	}
...
class MemoryGraphics: Graphics // docmain
{
	...
	this(int width, int height, Graphics graphicsCompatible)
	{
		if(cast(MemoryGraphics)graphicsCompatible)
		{
			...
			assert(0, "Графика несовместима с памятью");
		}
	...
	this(int width, int height, HDC hdcCompatible) // package
	{
		...
		if(!hbm)
			throw new DflException("Не удаётся разместить графическую память");
		scope(failure)
		{
			DeleteObject(hbm);
			//hbm = HBITMAP.init;
		...
		if(!hdcc)
			throw new DflException("Не удаётся разместить Графику");
		scope(failure)
			DeleteDC(hdcc);
	...
	final HBITMAP toHBitmap(HDC hdc) // package
	{
		...
		memdc = CreateCompatibleDC(hdc);
		if(!memdc)
			throw new DflException("Ошибка устройства");
		try
		{
			...
			{
				bad_bm:
				throw new DflException("Не удаётся разместить образ");
			}
			oldbm = SelectObject(memdc, result);
	...
	final Bitmap toBitmap(HDC hdc) // package
	{
		...
		hbm = toHBitmap(hdc);
		if(!hbm)
			throw new DflException("Не удаётся создать bitmap");
		return new Bitmap(hbm, true); // Owned.
	}
...
class Icon: Image // docmain
{
	...
	private void _getInfo(ICONINFO* ii, BITMAP* bm = null)
	{
		...
		// Fell through, failed.
		throw new DflException("Не удаётся получить инфорацию об изображении");
	}
...
class Font // docmain
{
	...
	{
		lf.lfa = *plfa;
		lf.faceName = os.win.gui.x.utf.fromAnsiz(plfa.lfFaceName.ptr);
	}
	...
	{
		lf.lfw = *plfw;
		lf.faceName = os.win.gui.x.utf.fromUnicodez(plfw.lfFaceName.ptr);
	}
	...
	{
		HFONT result;
		result = os.win.gui.x.utf.createFontIndirect(lf);
		if(!result)
			throw new DflException("Не удаётся создать шрифт");
		return result;
	}
	...
	package void _info(LOGFONTA* lf) // deprecated
	{
		if(GetObjectA(hf, LOGFONTA.sizeof, lf) != LOGFONTA.sizeof)
			throw new DflException("Не удаётся получить инфомацию об изображении");
	}
	...
	package void _info(LOGFONTW* lf) // deprecated
	{
		...
		if(!proc || proc(hf, LOGFONTW.sizeof, lf) != LOGFONTW.sizeof)
			throw new DflException("Не удаётся получить информацию о шрифте");
	}
	
	
	package void _info(inout LogFont lf)
	{
		if(!os.win.gui.x.utf.getLogFont(hf, lf))
			throw new DflException("Не удаётся получить информацию о шрифте");
	}
