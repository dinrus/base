...
else
{
	...
	abstract class Menu: DObject // docmain
	{
		...
		deprecated static void setDFL092()
		{
			version(SET_DFL_092)
			{
				pragma(msg, "os.win.gui: DFL 0.9.2 compatibility set at compile time");
			}
			else
			{
				//_compat092 = true;
		...
		/+ package +/ protected void _setInfo(UINT uItem, BOOL fByPosition, LPMENUITEMINFOA lpmii, Dstring typeData = null) // package
		{
			if(typeData.length)
			{
				if(os.win.gui.x.utf.useUnicode)
				{
					...
					lpmii.dwTypeData = cast(typeof(lpmii.dwTypeData))os.win.gui.x.utf.toUnicodez(typeData);
					_setMenuItemInfoW(hmenu, uItem, fByPosition, cast(MENUITEMINFOW*)lpmii);
				}
				else
				{
					lpmii.dwTypeData = cast(typeof(lpmii.dwTypeData))os.win.gui.x.utf.unsafeAnsiz(typeData);
