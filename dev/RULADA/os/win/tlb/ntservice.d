...
interface _DNtSvc : IDispatch {
  ...
  /+void AboutBox();+/
  /+enum wchar* Account;+/
  /+enum int ControlsAccepted;+/
  /+enum short Debug;+/
  /+enum wchar* LoadOrderGroup;+/
  /+enum wchar* Dependencies;+/
  /+enum wchar* DisplayName;+/
  /+enum short Interactive;+/
  /+enum wchar* Password;+/
  /+enum wchar* ServiceName;+/
  /+enum SvcStartMode StartMode;+/
  /+enum wchar* _DisplayName;+/
}
