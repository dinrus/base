...
string progIdFromClsid(Guid clsid) {
  ...
  return toUTF8(str[0 .. std.string.wcslen(str)]);
}
...
interface IStorage : IUnknown {
  ...
  int Revert();
  int constElements(uint reserved1, void* reserved2, uint reserved3, out IconstSTATSTG ppconst);
...
interface IBindCtx : IUnknown {
  ...
  int constObjectParam(out IconstString ppconst);
...
interface IMoniker : IPersistStream {
  ...
  int Enum(bool fForward, out IconstMoniker ppconstMoniker);
...
interface IRunningObjectTable : IUnknown {
  ...
  int constRunning(out IconstMoniker ppconstMoniker);
}
...
interface IconstUnknown : IUnknown {
  ...
  int Skip(uint celt);
  int Reset();
  int Clone(out IconstUnknown ppconst);
}
...
interface IconstString : IUnknown {
  ...
  int Skip(uint celt);
  int Reset();
  int Clone(out IconstString ppconst);
}
...
interface IconstMoniker : IUnknown {
  ...
  int Skip(uint celt);
  int Reset();
  int Clone(out IconstMoniker ppconst);
}
...
interface IDataObject : IUnknown {
  ...
  int constFormatEtc(uint dwDirection, out IconstFORMATETC ppconstFormatEtc);
  ...
  int constDAdvise(out IconstSTATDATA ppconstAdvise);
}
...
interface IconstGUID : IUnknown {
  ...
  int Skip(uint celt);
  int Reset();
  int Clone(out IconstGUID ppconst);
}
...
interface IconstCATEGORYINFO : IUnknown {
...
interface ICatInformation : IUnknown {
  ...
  int constCategories(uint lcid, out IconstCATEGORYINFO ppconstCategoryInfo);
  int GetCategoryDesc(inout GUID rcatid, uint lcid, out wchar* pszDesc);
  int constClassesOfCategories(uint cImplemented, GUID* rgcatidImpl, uint cRequired, GUID* rgcatidReq, out IconstGUID ppconstClsid);
  int IsClassOfCategories(inout GUID rclsid, uint cImplemented, GUID* rgcatidImpl, uint cRequired, GUID* rgcatidReq);
  int constImplCategoriesOfClass(inout GUID rclsid, out IconstGUID ppconstCatid);
  int constReqCategoriesOfClass(inout GUID rclsid, out IconstGUID ppconstCatid);
}
...
interface IConnectionPointContainer : IUnknown {
  ...
  int constConnectionPoints(out IconstConnectionPoints ppconst);
  int FindConnectionPoint(ref GUID riid, out IConnectionPoint ppCP);
}

interface IconstConnectionPoints : IUnknown {
  ...
  int Reset();
  int Clone(out IconstConnectionPoints ppconst);
}
...
interface IConnectionPoint : IUnknown {
  ...
  int constConnections(out IconstConnections ppconst);
}
...
template com_cast_impl(T, ExceptionPolicy policy) {
  ...
  T com_cast_impl(U)(U obj) {
    static if (is(U : IUnknown)) {
      ...
      static if (policy == ExceptionPolicy.Throw)
        throw new InvalidCastException("Неверное преобразование из '" ~ U.stringof ~ "' в '" ~ T.stringof ~ "'.");
      else
        return null;
    }
...
class MissingMemberException : Exception {
  ...
  private const string E_MISSINGMEMBER = "Член не найден.";
