...
alias Iconsterable ИПеречислимый;
...
alias getHash дайХэш;
   ...
   * Параметры: value = The object for which a hash code is to be retrieved.
 ...
 */
interface ICollection(T) : Iconsterable!(T) {
...
alias opIndex опИндекс;
   ...
   * ditto
   */
  final void addRange(Iconsterable!(T) range) {
    insertRange(size_, range);
  }
   ...
   * ditto
   */
  final void insertRange(int index, Iconsterable!(T) range) {
    foreach (item; range) {
      insert(index++, item);
    }
  }
...
class KeyNotFoundException : Exception {
  ...
  this(string message = "Ключ не был указан.") {
    super(message);
  }
