...
import os.windows : 
  HIWORD, LOWORD, BOOL, RT_STRING, MAKEINTRESOURCEW, MAKEINTRESOURCE;
...
alias int function(Handle hModule, in wchar* lpType, int lParam) constRESTYPEPROCW;
alias constRESTYPEPROCW constRESTYPEPROC;

int constResourceTypesW(Handle hModule, constRESTYPEPROCW lpconstFunc, int lParam);
alias constResourceTypesW constResourceTypes;

alias int function(Handle hModule, in wchar* lpType, wchar* lpName, int lParam) constRESNAMEPROCW;
alias constRESNAMEPROCW constRESNAMEPROC;

int constResourceNamesW(Handle hModule, in wchar* lpType, constRESNAMEPROCW lpconstFunc, int lParam);
alias constResourceNamesW constResourceNames;
...
alias int function(wchar*) LOCALE_constPROCW;
alias LOCALE_constPROCW LOCALE_constPROC;

int constSystemLocalesW(LOCALE_constPROCW lpLocaleconstProc, uint dwFlags);
alias constSystemLocalesW constSystemLocales;
...
enum : uint {
  ...
  const_ALL_CALENDARS     = 0xffffffff
}
...
alias int function(wchar* lpCalendarInfoString) CALINFO_constPROCW;
alias int function(wchar* lpCalendarInfoString, uint Calendar) CALINFO_constPROCEXW;

int constCalendarInfoW(CALINFO_constPROCW lpCalInfoconstProc, uint Locale, uint Calendar, uint CalType);
alias constCalendarInfoW constCalendarInfo;

int constCalendarInfoExW(CALINFO_constPROCEXW lpCalInfoconstProcEx, uint Locale, uint Calendar, uint CalType);
alias constCalendarInfoExW constCalendarInfoEx;

alias int function(wchar* lpDateFormatString, uint CalendarId) DATEFMT_constPROCEXW;

int constDateFormatsExW(DATEFMT_constPROCEXW lpDateFmtconstProcEx, uint Locale, uint dwFlags);
alias constDateFormatsExW constDateFormatsEx;

alias int function(wchar* lpTimeFormatString) TIMEFMT_constPROCW;

int constTimeFormatsW(TIMEFMT_constPROCW lpTimeFmtconstProc, uint Locale, uint dwFlags);
alias constTimeFormatsW constTimeFormats;
...
enum : uint {
  ...
  KEY_constERATE_SUB_KEYS = 0x0008,
  KEY_NOTIFY             = 0x0010,
  KEY_CREATE_LINK        = 0x0020,

  KEY_READ               = (STANDARD_RIGHTS_READ | KEY_QUERY_VALUE | KEY_constERATE_SUB_KEYS | KEY_NOTIFY) & ~SYNCHRONIZE,
  KEY_WRITE              = (STANDARD_RIGHTS_WRITE | KEY_SET_VALUE | KEY_CREATE_SUB_KEY) & ~SYNCHRONIZE,
  KEY_EXECUTE            = KEY_READ & ~SYNCHRONIZE,
  KEY_ALL_ACCESS         = (STANDARD_RIGHTS_ALL | KEY_QUERY_VALUE | KEY_SET_VALUE | KEY_CREATE_SUB_KEY | KEY_constERATE_SUB_KEYS | KEY_NOTIFY | KEY_CREATE_LINK) & ~SYNCHRONIZE,
}
...
int RegconstValueW(Handle hKey, uint dwIndex, wchar* lpValueName, ref  uint lpcchValueName, uint* lpReserved, uint* lpType, ubyte* lpData, uint* lpcbData);
alias RegconstValueW RegconstValue;

int RegconstKeyExW(Handle hKey, uint dwIndex, wchar* lpName, ref  uint lpcchName, uint* lpReserved, wchar* lpClass, uint* lpcchClass, FILETIME* lpftLastWriteTime);
alias RegconstKeyExW RegconstKeyEx;
...
enum : uint {
  ...
  SC_MANAGER_constERATE_SERVICE  = 0x0004,
...
enum : uint {
  ...
  SERVICE_constERATE_DEPENDENTS  = 0x0008,
...
enum : uint {
  ...
  CERT_STORE_const_ARCHIVED_FLAG               = 0x00000200,
...
CERT_CONTEXT* CertconstCertificatesInStore(Handle hCertStore, CERT_CONTEXT* pPrevCertContext);
...
enum : uint {
  ...
  PP_UI_PROMPT           = 21,
  PP_constALGS_EX         = 22,
  PP_constMANDROOTS       = 25,
  PP_constELECTROOTS      = 26,
  PP_KEYSET_TYPE         = 27,
  PP_ADMIN_PIN           = 31,
...
class DllNotFoundException : Exception {
  ...
  this(string message = "Dll не найдена.") {
    super(message);
  }
...
class EntryPointNotFoundException : Exception {
  ...
  this(string message = "Точка входа в процедуру не найдена.") {
    super(message);
  }
...
private void* addressOfFunction(string dllName, string entryPoint, CharSet charSet) {
  ...
  if (moduleHandle == Handle.init)
    throw new DllNotFoundException("Невозможно загрузить DLL '" ~ dllName ~ "'.");
  ...
  if (func == null) {
    ...
    if (func == null)
      throw new EntryPointNotFoundException("Не удайтся найти точку входа '" ~ entryPoint ~ "' в DLL '" ~ dllName ~ "'.");
  }
