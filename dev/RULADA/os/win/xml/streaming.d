...
interface IXmlReader : IUnknown {
  ...
  int IsDefault();
  int IsEmptyElement();
  int GetLinconstber(out uint pnLinconstber);
...
abstract class XmlReader {
  ...
  void readStartElement(string localName, string ns) {
    ...
    if (localName != this.localName || ns != namespaceURI)
      throw new XmlException("Element '"~ localName ~ "' with namespace name '" ~ ns ~ "' not found.", linconstber, linePosition);
    read();
  }
  ...
  void readStartElement(string name) {
    if (moveToContent() != XmlNodeType.Element)
      throw new XmlException(XmlNodeTypeString[nodeType] ~ " is an invalid XmlNodeType.", linconstber, linePosition);
    if (name != this.name)
      throw new XmlException("Element '"~ name ~ "' not found.", linconstber, linePosition);
    read();
  }
  ...
  string readElementString() {
    ...
    if (!isEmptyElement) {
      ...
      result = readString();
      if (nodeType != XmlNodeType.EndElement)
        throw new XmlException("Unexpected node type '" ~ XmlNodeTypeString[nodeType] ~ "'. 'readElementString' method can only be called on elements with simple content.", linconstber, linePosition);
      read();
    }
    else
      read();
...
private final class XmlLiteReader : XmlReader {
  ...
  override int linconstber() {
    uint value;
    readerImpl_.GetLinconstber(value);
