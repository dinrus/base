...
static:
	...
	{
		makeModule(t, "счёт", function uword(MDThread* t, uword numParams)
		{
			pushFloat(t, math.E);              newGlobal(t, "е");
			pushFloat(t, math.PI);             newGlobal(t, "пи");
			pushFloat(t, mdfloat.nan);         newGlobal(t, "нчло");
			pushFloat(t, mdfloat.infinity);    newGlobal(t, "беск");

			pushInt(t, mdint.sizeof);          newGlobal(t, "размерЦел");
			pushInt(t, mdint.min);             newGlobal(t, "целМин");
			pushInt(t, mdint.max);             newGlobal(t, "целМакс");

			pushInt(t, mdfloat.sizeof);        newGlobal(t, "размерПлав");
			pushFloat(t, mdfloat.min);         newGlobal(t, "плавМин");
			pushFloat(t, mdfloat.max);         newGlobal(t, "плавМакс");
			
			register(t, "абс", &abs);
			register(t, "син", &sin);
			register(t, "кос", &cos);
			register(t, "тан", &tan);
			register(t, "асин", &asin);
			register(t, "акос", &acos);
			register(t, "атан", &atan);
			register(t, "атан2", &atan2);
			register(t, "квкор", &sqrt);
			register(t, "cbrt", &cbrt);
			register(t, "степ", &pow);
			register(t, "эксп", &exp);
			register(t, "лн", &ln);
			register(t, "лог2", &log2);
			register(t, "лог10", &log10);
			register(t, "гипот", &hypot);
			register(t, "лгамма", &lgamma);
			register(t, "гамма", &gamma);
			register(t, "ceil", &ceil);
			register(t, "floor", &floor);
			register(t, "округли", &round);
			register(t, "trunc", &trunc);
			register(t, "нечло_ли", &isNan);
			register(t, "беск_ли", &isInf);
			register(t, "знак", &sign);
