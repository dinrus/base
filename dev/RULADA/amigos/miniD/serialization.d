...
static:
	...
	{
		makeModule(t, "сериализация", function uword(MDThread* t, uword numParams)
		{
			importModuleNoNS(t, "поток");

			newFunction(t, &serializeGraph,   "сериализуйГраф");   newGlobal(t, "сериализуйГраф");
			newFunction(t, &deserializeGraph, "десериализуйГраф"); newGlobal(t, "десериализуйГраф");
			return 0;
		});
	}
	...
	uword serializeGraph(MDThread* t, uword numParams)
	{
		...
		else
		{
			...
			if(as(t, 3, -1))
			{
				...
				paramTypeError(t, 3, "поток.ВыхПоток|поток.Проток");
		}
	...
	uword deserializeGraph(MDThread* t, uword numParams)
	{
		...
		else
		{
			...
			if(as(t, 2, -1))
			{
				...
				paramTypeError(t, 2, "поток.ВыхПоток|поток.Проток");
		}
...
private:
	...
	void writeGraph(word value, word trans)
	{
		...
		({
			...
			newFunction(t, &serializeFunc, "сериализуй", 1);
...
// 	void writeFunction(word idx)
	...
	void serializeInstance(MDInstance* v)
	{
		...
		if(hasField(t, -1, "опСериализовать"))
		{
			field(t, -1, "опСериализовать");
			...
			if(isFunction(t, -1))
			{
				...
				methodCall(t, -4, "опСериализовать", 0);
				return;
			}
			else if(isBool(t, -1))
			{
...
// 	}
	...
	void deserializeInstanceImpl()
	{
		...
		if(isSpecial)
		{
			...
			if(!hasMethod(t, -1, "опДесериализовать"))
			{
				pushToString(t, -1, true);
			...
			methodCall(t, -4, "опДесериализовать", 0);
		}
		else
		{
