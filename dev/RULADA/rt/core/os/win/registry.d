...
/// An consteration representing byte-ordering (Endian) strategies
public enum Endian
{
...
/* /////////////////////////////////////////////////////////////////////////////
 * public consterations
 */

/// consteration of the recognised registry access modes
///
...
public enum REGSAM
{
    ...
    ,   KEY_constERATE_SUB_KEYS  =   0x0008 //!< Permission to consterate subkeys
    ...
    ,   KEY_READ                =   (   STANDARD_RIGHTS_READ
                                    |   KEY_QUERY_VALUE
                                    |   KEY_constERATE_SUB_KEYS
                                    |   KEY_NOTIFY)
                                &   ~(SYNCHRONIZE) //!< Combines the STANDARD_RIGHTS_READ, KEY_QUERY_VALUE, KEY_constERATE_SUB_KEYS, and KEY_NOTIFY access rights
    ...
    ,   KEY_ALL_ACCESS          =   (   STANDARD_RIGHTS_ALL
                                    ...
                                    |   KEY_NOTIFY
                                    |   KEY_CREATE_LINK)
                                &   ~(SYNCHRONIZE) //!< Combines the KEY_QUERY_VALUE, KEY_constERATE_SUB_KEYS, KEY_NOTIFY, KEY_CREATE_SUB_KEY, KEY_CREATE_LINK, and KEY_SET_VALUE access rights, plus all the standard access rights except SYNCHRONIZE
}

/// consteration of the recognised registry value types
///
/// \ingroup group_D_win32_reg
...
extern  (Windows)
{
    ...
    LONG    RegQueryValueExA(   in HKEY hkey, in LPCSTR lpValueName, in Reserved 
                            ...
                            ,   inout DWORD cbData);
    LONG    RegconstKeyExA(  in HKEY hkey, in DWORD dwIndex, in LPSTR lpName
                        ,   inout DWORD cchName, in Reserved , in LPSTR lpClass
                        ,   in LPDWORD cchClass, in FILETIME *ftLastWriteTime);
    LONG    RegconstValueA(  in HKEY hkey, in DWORD dwIndex, in LPSTR lpValueName
...
private LONG Reg_constKeyName_(  in HKEY hkey, in DWORD index, inout char [] name
                            ,   out DWORD cchName)
in
{
    assert(!(null is hkey));
...
body
{
    ...
    for(;;)
    {
        ...
        res = RegconstKeyExA(hkey, index, name.ptr, cchName, RESERVED, null, null, null);
...
private LONG Reg_constValueName_(in HKEY hkey, in DWORD dwIndex, in LPSTR lpName
                            ,   inout DWORD cchName)
in
{
    assert(!(null is hkey));
...
body
{
    return RegconstValueA(hkey, dwIndex, lpName, cchName, RESERVED, null, null, null);
}
...
*/
    ...
    /// An consterable sequence of all the sub-keys of this key
    KeySequence keys()
    {
        return new KeySequence(this);
    }

    /// An consterable sequence of the names of all the sub-keys of this key
    ...
    /// An consterable sequence of all the values of this key
    ValueSequence values()
    {
        return new ValueSequence(this);
    }

    /// An consterable sequence of the names of all the values of this key
...
public:
    ...
    /// \param access The desired access; one of the REGSAM consteration
...
/// An consterable sequence representing the names of the sub-keys of a registry Key
///
...
public:
    ...
    string getKeyName(uint index)
    {
        ...
        res = Reg_constKeyName_(hkey, index, sName, cchName);
...
public:
    int opApply(int delegate(inout string name) dg)
    {
        ...
        for(DWORD index = 0; 0 == result; ++index)
        {
            ...
            {
                throw new RegistryException("Key name consteration incomplete", res);
            }
        }
...
/// An consterable sequence representing the sub-keys of a registry Key
///
/// It would be used as follows:
///
/// <code>&nbsp;&nbsp;Key&nbsp;key&nbsp;=&nbsp;. . .</code>
...
public:
    int opApply(int delegate(inout Key key) dg)
    {
	    ...
	    res     =   Reg_constKeyName_(hkey, index, sName, cchName);
            ...
            {
                throw new RegistryException("Key consteration incomplete", res);
            }
        }
...
/// An consterable sequence representing the names of the values of a registry Key
///
/// It would be used as follows:
///
/// <code>&nbsp;&nbsp;Key&nbsp;key&nbsp;=&nbsp;. . .</code>
...
public:
    ...
    string getValueName(uint index)
    {
        ...
        res = Reg_constValueName_(hkey, index, sName.ptr, cchName);
...
public:
    int opApply(int delegate(inout string name) dg)
    {
	    ...
	    res = Reg_constValueName_(hkey, index, sName.ptr, cchName);
            ...
            {
                throw new RegistryException("Value name consteration incomplete", res);
            }
        }
...
/// An consterable sequence representing the values of a registry Key
///
/// It would be used as follows:
///
/// <code>&nbsp;&nbsp;Key&nbsp;key&nbsp;=&nbsp;. . .</code>
...
public:
    int opApply(int delegate(inout Value value) dg)
    {
	    ...
	    res = Reg_constValueName_(hkey, index, sName.ptr, cchName);
            ...
            {
                throw new RegistryException("Value consteration incomplete", res);
            }
        }
