...
{   // Properly prototyped versions
    ...
    alias BOOL function(HWND, LPARAM) WNDconstPROC;
    ...
    alias BOOL function(HWND, LPCSTR, HANDLE) PROPconstPROCA;
    alias BOOL function(HWND, LPCWSTR, HANDLE) PROPconstPROCW;
    alias BOOL function(HWND, LPSTR, HANDLE, DWORD) PROPconstPROCEXA;
    alias BOOL function(HWND, LPWSTR, HANDLE, DWORD) PROPconstPROCEXW;
...
else
{
    ...
    alias FARPROC WNDconstPROC;
    alias FARPROC HOOKPROC;
    alias FARPROC SENDASYNCPROC;
    alias FARPROC EDITWORDBREAKPROCA;
    alias FARPROC EDITWORDBREAKPROCW;
    alias FARPROC PROPconstPROCA;
    alias FARPROC PROPconstPROCW;
    alias FARPROC PROPconstPROCEXA;
    alias FARPROC PROPconstPROCEXW;
...
enum
{
    ...
    KEY_READ =       cast(int)((STANDARD_RIGHTS_READ | KEY_QUERY_VALUE | KEY_constERATE_SUB_KEYS | KEY_NOTIFY)   & ~SYNCHRONIZE),
    ...
    KEY_ALL_ACCESS = cast(int)((STANDARD_RIGHTS_ALL | KEY_QUERY_VALUE | KEY_SET_VALUE | KEY_CREATE_SUB_KEY | KEY_constERATE_SUB_KEYS | KEY_NOTIFY | KEY_CREATE_LINK) & ~SYNCHRONIZE),
	
	КЛЮЧ_ЗАПРОСИ_ЗНАЧЕНИЕ = KEY_QUERY_VALUE,
	КЛЮЧ_УСТАНОВИ_ЗНАЧЕНИЕ =KEY_SET_VALUE,
	КЛЮЧ_СОЗДАЙ_ПОДКЛЮЧ = KEY_CREATE_SUB_KEY,
	КЛЮЧ_ПЕРЕЧИСЛИ_ПОДКЛЮЧИ = KEY_constERATE_SUB_KEYS,
...
extern (Windows) LONG  RegconstKeyExA(HKEY hKey, DWORD dwIndex, LPSTR lpName, LPDWORD lpcbName, LPDWORD lpReserved, LPSTR lpClass, LPDWORD lpcbClass, FILETIME* lpftLastWriteTime);
extern (Windows) LONG RegconstValueA(HKEY hKey, DWORD dwIndex, LPSTR lpValueName, LPDWORD lpcbValueName, LPDWORD lpReserved,
...
цел ПеречислиКлючиРегДопА(ук ключ, бцел индекс, усим имя, убцел пкбИмя, убцел резерв, усим класс, убцел пкбКласс, ФВРЕМЯ *времяПоследнейЗаписи)
	{
	return cast(цел)  RegconstKeyExA(cast(HKEY) ключ, cast(DWORD) индекс, cast(LPSTR) имя, cast(LPDWORD) пкбИмя, cast(LPDWORD) резерв, cast(LPSTR) класс, cast(LPDWORD) пкбКласс, cast(FILETIME*) времяПоследнейЗаписи);
	}
...
{
return cast(цел) RegconstValueA(cast(HKEY) ключ,cast(DWORD) индекс, cast(LPSTR) имяЗнач, cast(LPDWORD) пкбИмяЗнач, cast(LPDWORD) резерв, cast(LPDWORD) тип, cast(LPBYTE) данные, cast(LPDWORD) пкбДанные);
}
...
extern (C)
{
    ...
    extern IID IID_IconstUnknown;
    extern IID IID_IBindCtx;
    extern IID IID_IconstMoniker;
    ...
    extern IID IID_IconstSTATPROPSTG;
    extern IID IID_IconstSTATPROPSETSTG;
    ...
    extern IID IID_IconstGeneric;
    extern IID IID_IconstHolder;
    extern IID IID_IconstCallback;
