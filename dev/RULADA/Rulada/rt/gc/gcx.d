...
uint threadNum;
export extern (C) void dThreadsNumber(uint n){ threadNum = n;}
/* ======================= Leak Detector =========================== */
...
class GC
{
    ...
    final void enable()
    {
        if (!thread_needLock()||dThreadsNumber() == 1||threadNum ==1)
        {
    ...
    private void *mallocNoSync(size_t size, uint bits = 0)
    {
        ...
        if (bin < B_PAGE)
        {
            ...
            while (!gcx.bucket[bin] && !gcx.allocPage(bin))
            {
                ...
                {
                case 0:
                    size_t freedpages = gcx.fullcollectshell();
                    collected = true;
        ...
        {   void *p2;
            ...
            else
            {
                ...
                if (psize >= PAGESIZE && size >= PAGESIZE)
                {
                    ...
                    auto pagconst = (p - pool.baseAddr) / PAGESIZE;
                    ...
                    {   // Shrink in place
                        synchronized (gcLock)
                        {
                            ...
                            pool.freePages(pagconst + newsz, psz - newsz);
                        }
                        return p;
                    }
                    else if (pagconst + newsz <= pool.npages)
                    {
                        // Attempt to expand in place
                        synchronized (gcLock)
                        {
                            for (size_t i = pagconst + psz; 1;)
                            {
                                if (i == pagconst + newsz)
                                {
                                    debug (MEMSTOMP) cstring.memset(p + psize, 0xF0, size - psize);
                                    cstring.memset(&pool.pagetable[pagconst + psz], B_PAGEPLUS, newsz - psz);
                                    return p;
                                }
                                if (i == pool.ncommitted)
                                {
                                    auto u = pool.extendPages(pagconst + newsz - pool.ncommitted);
                                    if (u == OPFAIL)
                                        break;
                                    i = pagconst + newsz;
                                    continue;
    ...
    body
    {
        ...
        else if (pagconst + psz + sz == pool.ncommitted)
        {
        ...
        memset(pool.pagetable + pagconst + psz, B_PAGEPLUS, sz);
    ...
    private void freeNoSync(void *p)
    {
        ...
        sentinel_Invariant(p);
        p = sentinel_sub(p);
        pagconst = cast(size_t)(p - pool.baseAddr) / PAGESIZE;
        biti = cast(size_t)(p - pool.baseAddr) / 16;
        gcx.clrBits(pool, biti, BlkAttr.ALL_BITS);

        bin = cast(Bins)pool.pagetable[pagconst];
        ...
        {   size_t npages;
            ...
            pool.freePages(pagconst, npages);
        }
        else
        {   // Add to free list
...
////////////////////////////////////////////
    ...
    {
        if (thread_needLock()||dThreadsNumber() != 1)
    ...
    size_t findSize(void *p)
    {
        ...
        if (pool)
        {
            ...
            {   size_t npages = pool.ncommitted;
                ...
                pt = &pool.pagetable[0];
                for (i = pagconst + 1; i < npages; i++)
                {
                    if (pt[i] != B_PAGEPLUS)
                        break;
                }
                size = (i - pagconst) * PAGESIZE;
            }
            cached_size_key = p;
...
struct Pool
{
    ...
    /**
     * Free npages pages starting with pagconst.
     */
    void freePages(size_t pagconst, size_t npages)
    {
        cstring.memset(&pagetable[pagconst], B_FREE, npages);
    }
