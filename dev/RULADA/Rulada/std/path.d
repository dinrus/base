 ...
 *
 * Throws: std.exception.OutOfMemoryException if there is not enough
 * memory to perform
...
private string combineCPathWithDPath(char* c_path, string path, int char_pos)
{
    ...
    size_t end = std.c.strlen(c_path);
...
private string expandFromDatabase(string path)
{
    ...
    while (1)
    {
	extra_memory = std.c.malloc(extra_memory_size);
	...
	if (std.c.getErrno() != ERANGE)
	    goto Lerror;

	// extra_memory isn't large enough
	std.c.free(extra_memory);
	extra_memory_size *= 2;
    }
