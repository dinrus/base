...
import os.windows, std.utf;
deprecated class Base64Exception: Exception
{
	this(char[] msg)
	{
		super(msg);
...
class ИсключениеОснования64: Исключение
{
	this(char[] msg)
	{
		super(msg);
		wchar* soob =cast(wchar*)("Неудачное преобразование\n"~toUTF16(msg));
		ОкноСооб(null, cast(wchar*) soob, "D рантайм:ИсключениеОснования64", СО_ОК|СО_ПИКТОШИБКА);
	}
}
...
class Base64CharException: Exception
{
	this(char[] msg)
	{
		super(msg);
...
class ИсключениеСимвОснования64: Исключение
{
	this(char[] msg)
	{
		super(msg);
		wchar* soob =cast(wchar*)("Использован недопустимый символ\n"~toUTF16(msg));
		ОкноСооб(null, soob, "D рантайм:ИсключениеСимвОснования64", СО_ОК|СО_ПИКТОШИБКА);
	}
}
...
/*
alias encodeLength кодируйДлину;
alias encode кодируй;
alias decodeLength раскодируйДлину;
alias decode раскодируй;
alias ИсключениеСимвОснования64 ИсклСимОснования64;
alias ИсключениеОснования64 ИсклОснования64;
*/

бцел кодируйДлину(бцел сдлин)
{
return cast(бцел) encodeLength(cast(uint) сдлин);
}

ткст кодируй(ткст стр, ткст буф)
{
return cast(ткст) encode(cast(char[]) стр, cast(char[]) буф);
}

ткст кодируй (ткст стр)
{
return cast(ткст) encode(cast(char[])стр);
}

бцел раскодируйДлину(бцел кдлин)
{
return cast(бцел) decodeLength(cast(uint) кдлин);
}

ткст раскодируй(ткст кстр, ткст буф)
{
return cast(ткст) decode(cast(char[]) кстр, cast(char[]) буф);
}

ткст раскодируй(ткст кстр)
{
return cast(ткст) decode(cast(char[]) кстр);
}
////////////////////////////////////////////
 ...
 *	slice into buf[] representing encoded result
 * Errors:
 *	Throws ИсключениеОснования64 on invalid base64 encoding in estr[].
 *	Throws ИсключениеСимвОснования64 on invalid base64 character in estr[].
 */
...
body
{
	void badc(char ch)
	{
		throw new ИсключениеСимвОснования64("Invalid base64 character '" ~ (&ch)[0 .. 1] ~ "'");
	}
	...
	if(estr.length % 4)
		throw new ИсключениеОснования64("Invalid encoded base64 string");
