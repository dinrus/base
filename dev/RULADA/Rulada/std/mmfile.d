			...
			size_t window = 0)
    {
		...
		else version (Posix)
		{
			...
			{	
				...
				fd = os.posix.open(namez, oflag, fmode);
				if (fd == -1)
				{
				...
				if (prot & PROT_WRITE && size > statbuf.st_size)
				{
					...
					os.posix.lseek(fd, cast(int)(size - 1), SEEK_SET);
					char c = 0;
...
unittest {
	...
	version(Win32) {
		/+ these aren't defined in os.windows so let's use the default
         SYSTEM_INFO sysinfo;
