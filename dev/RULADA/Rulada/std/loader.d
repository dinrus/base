...
public:
    ...
    {
        super(message);
		wchar* soob =cast(wchar*)("Неудачное подключение к модулю\n"~toUTF16(msg));
		ОкноСооб(null, cast(wchar*) soob, "D рантайм: ИсключениеИМодуля", СО_ОК|СО_ПИКТОШИБКА);
    }
	...
	wchar* soob =cast(wchar*)("Неудачное подключение к модулю\n"~toUTF16(std.string.toString(strerror(errcode)).dup));
		ОкноСооб(null, cast(wchar*) soob, "D рантайм: ИсключениеИМодуля", СО_ОК|СО_ПИКТОШИБКА);
      }
    }
}
