...
{
    return auxc.zlib.adler32(adler, cast(ubyte *)buf, buf.length);
}
...
{
    return auxc.zlib.crc32(crc, cast(ubyte *)buf, buf.length);
}
...
body
{
    ...
    err = auxc.zlib.compress2(destbuf.ptr, &destlen, cast(ubyte *)srcbuf, srcbuf.length, level);
    if (err)
    {	delete destbuf;
...
void[] uncompress(void[] srcbuf, uint destlen = 0u, int winbits = 15)
{
	...
	err = auxc.zlib.inflateInit2(&zs, winbits);
	if (err)
	{   delete destbuf;
	    throw new ZlibException(err);
	}
	err = auxc.zlib.inflate(&zs, Z_NO_FLUSH);
	switch (err)
	{
	    case Z_OK:
		auxc.zlib.inflateEnd(&zs);
		destlen = destbuf.length * 2;
		continue;

	    case Z_STREAM_END:
		destbuf.length = zs.total_out;
		err = auxc.zlib.inflateEnd(&zs);
		if (err != Z_OK)
		    goto Lerr;
		return destbuf;
