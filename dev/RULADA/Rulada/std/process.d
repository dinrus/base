...
{
    return std.c.system(toStringz(command));
}
...
//
//	return std.c.spawnvp(mode, toStringz(pathname), argv_);
//    }
//}
...
int execv(string pathname, string[] argv)
{
    ...
    return std.c.execv(toStringz(pathname), argv_);
}
...
int execve(string pathname, string[] argv, string[] envp)
{
    ...
    return std.c.execve(toStringz(pathname), argv_, envp_);
}
...
int execvp(string pathname, string[] argv)
{
    ...
    return std.c.execvp(toStringz(pathname), argv_);
}
...
version (Posix)
{
	...
	string[]    envPaths    =   std.string.split(std.string.toString(std.c.getenv("PATH")), ":");
        int         iRet        =   0;
...
else version(Windows)
{
    ...
    return std.c.execvpe(toStringz(pathname), argv_, envp_);
}
else
{
    static assert(0);
