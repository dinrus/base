...
class StringException : Exception
{
    ...
    {
	super(msg);
	wchar* soob =cast(wchar*)("Неудачная операция с текстом\n"~toUTF16(msg));
		ОкноСооб(null, cast(wchar*) soob, "D рантайм:ТекстовоеИсключение", СО_ОК|СО_ПИКТОШИБКА);
    }
}
...
{   auto len = s.length;
	...
	while ((s[len - 1] & 0xC0) == 0x80)
	{
	    len--;
	    if (len == 0)
		throw new std.utf.ИсключениеУТФ("invalid UTF sequence", 0);
	}
