...
public class GtkDClass
{
	...
	public void openGtkDClass(char[] inAPI, ConvParms* convParms)
	{
		...
		gtkDText ~= privPub ~" import gtkD.glib.ConstructionException;\n\n";
		...
		if (needSignalImports)
		{
			...
			if (i < inLines.length)
			{
				gtkDText ~= privPub ~" import gtkD.gobject.Signals;\n";
				gtkDText ~= "public  import gtkD.gtkc.gdktypes;\n";
			}	
		}	
		...
		tangoImportConvs["std.io"] = ["tango.io.Stdout"];
		...
		phobos2ImportConvs["std.io"] = ["std.io"];
