...
/*package*/ class CalendarData {
  ...
  private bool getCalendarData(string localeName, uint calendarId) {
    return constCalendarInfo(localeName, calendarId, CAL_SERASTRING, eraNames);
  }
  
  private static bool constCalendarInfo(string localeName, uint calendar, uint calType, out string[] result) {
    static string[] temp;

    extern(Windows)
    static int constCalendarsProc(wchar* lpCalendarInfoString) {
    ...
    temp = null;
    if (!constCalendarInfo(&constCalendarsProc, culture, calendar, calType))
      return false;
    result = temp.reverse;
    return true;
  }
