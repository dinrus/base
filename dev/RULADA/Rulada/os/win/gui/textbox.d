...
abstract class TextBoxBase: ControlSuperClass // docmain
{
	...
	void selectedText(Dstring sel) // setter
	{
		...
		if(created)
		{
			//os.win.gui.x.utf.sendMessage(handle, EM_REPLACESEL, FALSE, sel);
			os.win.gui.x.utf.sendMessageUnsafe(handle, EM_REPLACESEL, FALSE, sel);
		}
	}
	...
	Dstring selectedText() // getter
	{
		...
		if(created)
			return os.win.gui.x.utf.getSelectedText(handle);
		return null;
	}
	...
	override void prevWndProc(inout Message msg)
	{
		...
		msg.result = os.win.gui.x.utf.callWindowProc(textBoxPrevWndProc, msg.hWnd, msg.msg, msg.wParam, msg.lParam);
	}
...
class TextBox: TextBoxBase // docmain
{
	...
	final void passwordChar(dchar pwc) // setter
	{
		if(pwc)
		{
			...
			if(created)
				...
				os.win.gui.x.utf.emSetPasswordChar(handle, pwc);
			else
		...
		else
		{
			...
			if(created)
				...
				os.win.gui.x.utf.emSetPasswordChar(handle, 0);
			else
				_style(_style() & ~ES_PASSWORD);
		}
	...
	final dchar passwordChar() // getter
	{
		if(created)
			...
			passchar = os.win.gui.x.utf.emGetPasswordChar(handle);
		return passchar;
	}
