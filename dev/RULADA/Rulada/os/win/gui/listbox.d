...
class ListBox: ListControl // docmain
{
		...
		int opApply(int delegate(inout int) dg)
		{
			...
			if(lbox.isMultSel())
			{
				...
				if(items.length != lbox.prevwproc(LB_GETSELITEMS, items.length, cast(LPARAM)cast(int*)items))
					throw new DflException("Unable to consterate selected list items");
	...
	final int findString(Dstring str, int startIndex)
	{
		...
		if(created)
		{
			if(os.win.gui.x.utf.useUnicode)
				result = prevwproc(LB_FINDSTRING, startIndex, cast(LPARAM)os.win.gui.x.utf.toUnicodez(str));
			else
				result = prevwproc(LB_FINDSTRING, startIndex, cast(LPARAM)os.win.gui.x.utf.unsafeAnsiz(str));
	...
	final int findStringExact(Dstring str, int startIndex)
	{
		...
		{
			if(os.win.gui.x.utf.useUnicode)
				result = prevwproc(LB_FINDSTRINGEXACT, startIndex, cast(LPARAM)os.win.gui.x.utf.toUnicodez(str));
			else
				result = prevwproc(LB_FINDSTRINGEXACT, startIndex, cast(LPARAM)os.win.gui.x.utf.unsafeAnsiz(str));
			if(result == LB_ERR) // Redundant.
				result = NO_MATCHES;
		}
	...
	static class ObjectCollection
	{
		...
		void _added(size_t idx, Object val)
		{
			...
			{
				if(os.win.gui.x.utf.useUnicode)
					lbox.prevwproc(LB_INSERTSTRING, idx, cast(LPARAM)os.win.gui.x.utf.toUnicodez(getObjectString(val)));
				else
					lbox.prevwproc(LB_INSERTSTRING, idx, cast(LPARAM)os.win.gui.x.utf.toAnsiz(getObjectString(val))); // Can this be unsafeAnsiz()?
			}
		}
	...
	LRESULT prevwproc(UINT msg, WPARAM wparam, LPARAM lparam)
	{
		...
		return os.win.gui.x.utf.callWindowProc(listboxPrevWndProc, hwnd, msg, wparam, lparam);
	}
}
