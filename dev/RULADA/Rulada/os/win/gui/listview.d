...
class ListView: ControlSuperClass // docmain
{
	...
	protected override void prevWndProc(inout Message msg)
	{
		...
		msg.result = os.win.gui.x.utf.callWindowProc(listviewPrevWndProc, msg.hWnd, msg.msg, msg.wParam, msg.lParam);
	}
	...
	package final LRESULT _ins(int index, ColumnHeader header)
	{
		...
		if(os.win.gui.x.utf.useUnicode)
		{
			lvc.lvcw.pszText = cast(typeof(lvc.lvcw.pszText))os.win.gui.x.utf.toUnicodez(header.text);
			return prevwproc(LVM_INSERTCOLUMNW, cast(WPARAM)index, cast(LPARAM)&lvc.lvcw);
		}
		else
		{
			lvc.lvca.pszText = cast(typeof(lvc.lvca.pszText))os.win.gui.x.utf.toAnsiz(header.text);
	...
	LRESULT updateColumnText(int colIndex, Dstring newText)
	{
		...
		if(os.win.gui.x.utf.useUnicode)
		{
			lvc.lvcw.pszText = cast(typeof(lvc.lvcw.pszText))os.win.gui.x.utf.toUnicodez(newText);
			return prevwproc(LVM_SETCOLUMNW, cast(WPARAM)colIndex, cast(LPARAM)&lvc.lvcw);
		}
		else
		{
			lvc.lvca.pszText = cast(typeof(lvc.lvca.pszText))os.win.gui.x.utf.toAnsiz(newText);
	...
	LRESULT prevwproc(UINT msg, WPARAM wparam, LPARAM lparam)
	{
		...
		return os.win.gui.x.utf.callWindowProc(listviewPrevWndProc, hwnd, msg, wparam, lparam);
	}
}
