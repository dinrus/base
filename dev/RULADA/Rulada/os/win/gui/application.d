...
final class Application // docmain
{
							...
							if(manifestlen)
							{
								...
								if(hf != INVALID_HANDLE_VALUE)
								{
									...
									if(WriteFile(hf, MANIFEST.ptr, MANIFEST.length, &written, null))
									{
										...
										{
											os.win.gui.x.winapi.ULONG_PTR ul;
	...
	{
		return os.win.gui.x.utf.getModuleFileName(HMODULE.init);
	}
	...
	{
		return pathGetDirName(os.win.gui.x.utf.getModuleFileName(HMODULE.init));
	}
	...
	package bool _doEvents(bool* keep)
	{
		...
		try
		{
			...
			while(os.win.gui.x.utf.peekMessage(&msg._winMsg, HWND.init, 0, 0, PM_REMOVE))
			{
				gotMessage(msg);
	...
	bool doEvents(uint msDelay)
	{
		if(msDelay <= 3)
			return doEvents();
		struct TMR { public import os.win.gui.timer; }
	...
	private static class ErrForm: Form
	{
		...
		private int inThread2()
		{
			try
			{
				...
				//while(os.win.gui.x.utf.getMessage(&msg, HWND.init, 0, 0))
				{
					if(!IsDialogMessageA(handle, &msg))
					//if(!os.win.gui.x.utf.isDialogMessage(handle, &msg))
					{
						TranslateMessage(&msg);
						DispatchMessageA(&msg);
						//os.win.gui.x.utf.dispatchMessage(&msg);
					}
	...
	else
	{
		...
		package int addMenuItem(MenuItem menu)
		{
			...
			tempmenus = cast(Menu*)std.c.realloc(menus, Menu.sizeof * (nmenus + 1));
			if(!tempmenus)
			{
		...
		package void addContextMenu(ContextMenu menu)
		{
			...
			idx = nmenus;
			nmenus++;
			tempmenus = cast(Menu*)std.c.realloc(menus, Menu.sizeof * nmenus);
			if(!tempmenus)
			{
				nmenus--;
				//throw new OutOfMemory;
		...
		package void removeMenu(Menu menu)
		{
			...
			else
			{
				...
				nmenus--;
				menus = cast(Menu*)std.c.realloc(menus, Menu.sizeof * nmenus);
	...
	{
		os.win.gui.x.winapi.TlsSetValue(tlsControl, cast(Control*)ctrl);
	}
	...
	{
		os.win.gui.x.dlib.gcPin(p);
	}
	
	
	package void punpin(void* p)
	{
		os.win.gui.x.dlib.gcUnpin(p);
	}
	...
	void filters(IMessageFilter[] filters) // setter
	{
		...
		TlsFilterValue* val = cast(TlsFilterValue*)os.win.gui.x.winapi.TlsGetValue(tlsFilter);
		if(!val)
			val = new TlsFilterValue;
		val.filters = filters;
		os.win.gui.x.winapi.TlsSetValue(tlsFilter, cast(LPVOID)val);
	}
	
	
	IMessageFilter[] filters() // getter
	{
		TlsFilterValue* val = cast(TlsFilterValue*)os.win.gui.x.TlsGetValue(tlsFilter);
		if(!val)
			return null;
		return val.filters;
	}
	...
	version(CUSTOM_MSG_HOOK)
	{
		...
		{
			os.win.gui.x.winapi.TlsSetValue(tlsHook, cast(LPVOID)hhook);
		}
		
		
		HHOOK msghook() // getter
		{
			return cast(HHOOK)os.win.gui.x.winapi.TlsGetValue(tlsHook);
		}
	}
	...
	{
		return cast(Control)cast(Control*)os.win.gui.x.winapi.TlsGetValue(tlsControl);
	}
	...
	{
		return cast(TF)cast(DWORD)os.win.gui.x.winapi.TlsGetValue(tlsThreadFlags);
	}
	
	
	void threadFlags(TF flags) // setter
	{
		if(!os.win.gui.x.winapi.TlsSetValue(tlsThreadFlags, cast(LPVOID)cast(DWORD)flags))
			assert(0);
	}
	...
	void gotMessage(inout Message msg)
	{
		...
		TranslateMessage(&msg._winMsg);
		//DispatchMessageA(&msg._winMsg);
		os.win.gui.x.utf.dispatchMessage(&msg._winMsg);
	}
}
...
extern(Windows) void _gcTimeout(HWND hwnd, UINT uMsg, UINT idEvent, DWORD dwTime)
{
	...
	os.win.gui.x.dlib.gcFullCollect();
...
void _initCommonControls(DWORD dwControls)
{
	...
	{
		pragma(msg, "os.win.gui: extended common controls supported at compile time");
...
body
{
	...
	if(!os.win.gui.x.utf.registerClass(wc))
...
// Returns the old wndProc.
WNDPROC superClass(HINSTANCE hinst, Dstring className, Dstring newClassName, out os.win.gui.x.utf.WndClass getInfo) // package
{
	WNDPROC wndProc;
	
	if(!os.win.gui.x.utf.getClassInfo(hinst, className, getInfo))
		throw new DflException("Unable to obtain information for window class '" ~ className ~ "'");
	...
	if(!os.win.gui.x.utf.registerClass(getInfo))
		//throw new DflException("Unable to register window class '" ~ newClassName ~ "'");
		return null;
	return wndProc;
}
