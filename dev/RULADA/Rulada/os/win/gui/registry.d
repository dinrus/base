...
class RegistryKey // docmain
{
			...
			if(ERROR_SUCCESS == querycode)
			{
				...
				else
				{
					try
					{
						...
						next_subkey:
						len = skn.length;
						LONG constcode = RegconstKeyExA(openHkey, 0, skn.ptr, &len, null, null, null, null);
						switch(constcode)
						{
							case ERROR_SUCCESS:
	...
	final Dstring[] getSubKeyNames()
	{
		...
		{
			len = buf.length;
			LONG rr = RegconstKeyExA(hkey, idx, buf.ptr, &len, null, null, null, null);
			switch(rr)
			{
				case ERROR_SUCCESS:
	...
	final Dstring[] getValueNames()
	{
		...
		{
			len = buf.length;
			LONG rr = RegconstValueA(hkey, idx, buf.ptr, &len, null, null, null, null);
			switch(rr)
			{
				case ERROR_SUCCESS:
					//result ~= buf[0 .. len].dup;
