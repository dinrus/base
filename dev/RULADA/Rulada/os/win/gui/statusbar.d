...
/+
const StatusBarPanelAutoSize: ubyte
{
	NONE,
	CONTENTS,
	SPRING,
...
class StatusBar: ControlSuperClass // docmain
{
	...
	class StatusBarPanelCollection
	{
		...
		void _fixtexts()
		{
			...
			if(os.win.gui.x.utf.useUnicode)
			{
				foreach(idx, pnl; _panels)
				{
					sb.prevwproc(SB_SETTEXTW, cast(WPARAM)idx | pnl._utype, cast(LPARAM)os.win.gui.x.utf.toUnicodez(pnl._txt));
				}
			}
			else
			{
				foreach(idx, pnl; _panels)
				{
					sb.prevwproc(SB_SETTEXTA, cast(WPARAM)idx | pnl._utype, cast(LPARAM)os.win.gui.x.utf.toAnsiz(pnl._txt));
				}
			}
		}
	...
	protected override void prevWndProc(inout Message msg)
	{
		...
		msg.result = os.win.gui.x.utf.callWindowProc(statusbarPrevWndProc, msg.hWnd, msg.msg, msg.wParam, msg.lParam);
	}
	...
	LRESULT prevwproc(UINT msg, WPARAM wparam, LPARAM lparam)
	{
		...
		return os.win.gui.x.utf.callWindowProc(statusbarPrevWndProc, hwnd, msg, wparam, lparam);
	}
	...
	void _sendidxtext(int idx, WPARAM utype, Dstring txt)
	{
		...
		if(os.win.gui.x.utf.useUnicode)
			prevwproc(SB_SETTEXTW, cast(WPARAM)idx | utype, cast(LPARAM)os.win.gui.x.utf.toUnicodez(txt));
		else
			prevwproc(SB_SETTEXTA, cast(WPARAM)idx | utype, cast(LPARAM)os.win.gui.x.utf.toAnsiz(txt));
	}
}
