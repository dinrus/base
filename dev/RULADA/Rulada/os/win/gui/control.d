...
version(DFL_NO_DRAG_DROP) {} else
{
	...
	class DragEventArgs: EventArgs
	{
		...
		///
		final os.win.gui.data.IDataObject data() // getter
		{
			return _dobj;
		}
...
package alias BOOL delegate(HWND) constWindowsCallback;


// Callback for constWindows() and constChildWindows().
private extern(Windows) BOOL constingWindows(HWND hwnd, LPARAM lparam)
{
	constWindowsCallback dg = *(cast(constWindowsCallback*)lparam);
	return dg(hwnd);
}
...
{
	HWND hwParent;
	constWindowsCallback dg;
}


// Callback for constChildWindows(). -lparam- = pointer to Efi;
private extern(Windows) BOOL constingFirstWindows(HWND hwnd, LPARAM lparam)
{
...
package BOOL constWindows(constWindowsCallback dg)
{
	static assert((&dg).sizeof <= LPARAM.sizeof);
	return constWindows(&constingWindows, cast(LPARAM)&dg);
}


package BOOL constChildWindows(HWND hwParent, constWindowsCallback dg)
{
	static assert((&dg).sizeof <= LPARAM.sizeof);
	return constChildWindows(hwParent, &constingWindows, cast(LPARAM)&dg);
}


// Only the parent's children, not its children.
package BOOL constFirstChildWindows(HWND hwParent, constWindowsCallback dg)
{
	Efi efi;
	efi.hwParent = hwParent;
	efi.dg = dg;
	return constChildWindows(hwParent, &constingFirstWindows, cast(LPARAM)&efi);
}
...
class Control: DObject, IWindow // docmain
{
	...
	static class ControlCollection
	{
		...
		int indexOf(Control ctrl)
		{
			if(_owner.isHandleCreated)
			{
				...
				constFirstChildWindows(_owner.handle, &consting);
				return foundi;
			}
			else
			{
	...
	package final void _fixRtol(RightToLeft val)
	{
		...
		invalidate(false); // Since children are consterated.
	}
	...
	body
	{
		...
		if(isHandleCreated)
		{
			GetZIndex gzi;
			gzi.find = this;
			constChildWindows(parent.hwnd, &getZIndexCallback, cast(LPARAM)&gzi);
			return gzi.index;
		}
		else
		{
	...
	{
		return os.win.gui.x.utf.getWindowText(hwnd);
	}
	...
	void text(Dstring txt) // setter
	{
		if(isHandleCreated)
		{
			...
			os.win.gui.x.utf.setWindowText(hwnd, txt);
		}
		else
		{
			wtext = txt;
	...
	version(DFL_NO_DRAG_DROP) {} else
	{
		...
		final DragDropEffects doDragDrop(os.win.gui.data.IDataObject dataObj, DragDropEffects allowedEffects)
		{
			...
			DWORD effect;
			DropSource dropsrc;
			os.win.gui.x.wincom.IDataObject dropdata;
	...
	**/
	...
	final void delayInvoke(void function(Control, size_t[]) fn, size_t[] params ...)
	{
		...
		DflInvokeParam* p;
		p = cast(DflInvokeParam*)std.c.malloc(
								...
								try
								{
									if(osver.dwPlatformId <= VER_PLATFORM_WIN32_WINDOWS)
									{
										...
										else
										{
											// ANSI.
											Dstring ansi;
											ansi = os.win.gui.x.utf.toAnsi(this.name);
									...
									else
									{
										// Unicode.
										Dwstring uni;
										uni = os.win.gui.x.utf.toUnicode(this.name);
	...
	protected void defWndProc(inout Message msg)
	{
		...
		msg.result = os.win.gui.x.utf.defWindowProc(msg.hWnd, msg.msg, msg.wParam, msg.lParam);
	}
	...
	deprecated package final void createClassHandle(Dstring className)
	{
		...
		hwnd = os.win.gui.x.utf.createWindowEx(wexstyle, className, wtext, wstyle, wrect.x, wrect.y,
	...
	protected void createHandle()
	{
		...
		with(cp)
		{
			...
			Application.creatingControl(this);
			hwnd = os.win.gui.x.utf.createWindowEx(exStyle, className, caption, (style & ~WS_VISIBLE), x, y,
	...
	public deprecated void setDFL095()
	{
		version(SET_DFL_095)
		{
			pragma(msg, "os.win.gui: DFL 0.9.5 compatibility set at compile time");
		}
		else
		{
	...
	LRESULT defwproc(UINT msg, WPARAM wparam, LPARAM lparam)
	{
		...
		return os.win.gui.x.utf.defWindowProc(hwnd, msg, wparam, lparam);
	}
