...
/*void install()
{
	...
	else
	{
		...
		{
			fbd.description = "DMD was не найден at that location. Please try again.";
	...
	switch(arg)
	{
		...
		case "doc":
			...
			if(!std.file.exists(arg))
				throw new Exception("'" ~ arg ~ "' не найден");
		...
		case "release":
			if(debugSpecified)
				throw new Exception("-release задан с -debug");
		...
		case "debug":
			if(!isDebug)
				throw new Exception("-debug задан с -release");
			debugSpecified = true;
			return false;
		...
		case "tangobos", "Tangobos":
			if(optPhobos)
				throw new Exception("-phobos задан с -tangobos");
			optTangobos = true;
			return true;
		
		case "phobos", "Phobos":
			if(optTango)
				throw new Exception("-tango задан с -phobos");
			if(optTangobos)
				throw new Exception("-tangobos задан с -phobos");
			optPhobos = true;
			return true;
		
		case "tango", "Tango":
			if(optPhobos)
				throw new Exception("-phobos задан с -tango");
			optTango = true;
			return true;
...
void showUsage()
{
	writefln("DFL написал Christopher E. Miller");
	writef("Использование:\n"
		"   dfl [<switches...>] <files...>\n\n");
	writef("Switches:\n"
		"   -dmd             Показать использование DMD.\n"
		"   -dfl-ver         Показать версию установленной DFL.\n"
		"   -dfl-nover       Не выполнять проверку версии.\n"
		"   -dfl-build       Строить файлы DFL lib.\n"
	...
	if(os.win.gui.x.utf.useUnicode)
	{
		...
		else
		{
			...
			if(!proc)
			{
				...
				if(!proc)
					throw new Exception("GetShortPathNameW не найден");
			}
		}
				...
				if(!std.file.exists(importdir) || !std.file.exists(std.path.join(importdir, "dfl")))
				{
					...
					if(!std.file.exists(importdir) || !std.file.exists(std.path.join(importdir, "dfl")))
					{
						...
						if(!std.file.exists(importdir) || !std.file.exists(std.path.join(importdir, "dfl")))
						{
							...
							{
								importdir = null;
								throw new Exception("DFL import directory не найден");
							}
						}
					}
				}
		...
		void findlibdir()
		{
			...
			if(!dfllib.length)
			{
				...
				if(!std.file.exists(dfllib))
				{
					...
					{
						dfllib = null;
						writefln("DFL lib files не найден.");
						if(askBuildDflNow())
							goto try_lib_again;
						throw new Exception(libfile ~ " не найден");
					}
				}
			}
		}
