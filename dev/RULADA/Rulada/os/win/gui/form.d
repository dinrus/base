...
class Form: ContainerControl, IDialogResult // docmain
{
	...
	protected override void createHandle()
	{
		...
		if(killing)
		{
			...
			create_err:
			Dstring kmsg = "Ошибка при создании формы";
			if(name.length)
				kmsg ~= " (" ~ name ~ ")";
			debug
			{
		...
		if(wowner)
		//	wowner.createHandle(); // DMD 0.111: class os.win.gui.control.Control member createHandle is not accessible
		...
		with(cp)
		{
			...
			hwnd = os.win.gui.x.utf.createWindowEx(exStyle, className, caption, wstyle & ~WS_VISIBLE,
			...
			if(!hwnd)
			{
				debug
				{
					version(Tango)
					{
						er = "Ошибка функции CreateWindowEx";
					}
					else
					{
						er = std.string.format("Ошибка функции CreateWindowEx {className=%s;exStyle=0x%X;style=0x%X;parent=0x%X;menu=0x%X;inst=0x%X;}",
			...
			case WM_SETFOCUS:
				...
				else
				{
					if(isMdiChild)
					{
						...
						msg.result = os.win.gui.x.utf.defMDIChildProc(msg.hWnd, msg.msg, msg.wParam, msg.lParam);
						return;
					}
				}
			...
			default: def_def:
				...
				else
				{
					if(mdiClient && mdiClient.isHandleCreated && msg.msg != WM_SIZE)
						...
						msg.result = os.win.gui.x.utf.defFrameProc(msg.hWnd, mdiClient.handle, msg.msg, msg.wParam, msg.lParam);
					else if(isMdiChild)
						//msg.result = DefMDIChildProcA(msg.hWnd, msg.msg, msg.wParam, msg.lParam);
						msg.result = os.win.gui.x.utf.defMDIChildProc(msg.hWnd, msg.msg, msg.wParam, msg.lParam);
					else
					...
					case WM_CHAR:
						switch(cast(Keys)m.wParam)
						{
							case Keys.ENTER:
								...
								{
									os.win.gui.x.utf.isDialogMessage(form.handle, &m._winMsg);
									return true; // Prevent.
								}
								return false;
							...
							case Keys.ESCAPE:
								...
								{
									//os.win.gui.x.utf.isDialogMessage(form.handle, &m._winMsg); // Closes the parent; bad for nested controls.
							...
							case Keys.RIGHT, Keys.LEFT:
								//if(os.win.gui.x.utf.isDialogMessage(form.handle, &m._winMsg)) // Stopped working after removing controlparent.
								//	return true; // Prevent.
								{
									LRESULT dlgc;
...
version(NO_MDI) {} else
{
	...
	class MdiClient: ControlSuperClass
	{
		...
		override void createHandle()
		{
			...
			Application.creatingControl(this);
			hwnd = os.win.gui.x.utf.createWindowEx(wexstyle, MDICLIENT_CLASSNAME, wtext, wstyle, wrect.x, wrect.y,
		...
		protected override void prevWndProc(inout Message msg)
		{
			...
			msg.result = os.win.gui.x.utf.callWindowProc(mdiclientPrevWndProc, msg.hWnd, msg.msg, msg.wParam, msg.lParam);
		}
