...
final class Graphics : IDisposable {
   ...
   *   srcUnit = A member of the GraphicsUnit consteration that specifies the units of measure used by srcRect.
   */
...
final class PathIterator {
  ...
  /**
   */
  int consterate(ref PointF[] points, ref ubyte[] types) {
    if (points.length != types.length)
      throw statusException(Status.InvalidParameter);

    int resultCount;
    Status status = GdipPathIterconsterate(nativeIter_, resultCount, points.ptr, types.ptr, points.length);
    if (status != Status.OK)
      throw statusException(status);
    return resultCount;
  }
