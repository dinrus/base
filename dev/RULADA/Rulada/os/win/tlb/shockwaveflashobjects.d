...
interface IShockwaveFlash : IDispatch {
  ...
  int get_Framconst(out int pVal);
  int put_Framconst(int pVal);
  ...
  int Rewind();
  int StopPlay();
  int GotoFrame(int Framconst);
  int CurrentFrame(out int Framconst);
  int IsPlaying(out short Playing);
  int PercentLoaded(out int percent);
  int FrameLoaded(int Framconst, out short loaded);
  ...
  int TGotoFrame(wchar* target, int Framconst);
  int TGotoLabel(wchar* target, wchar* label);
  int TCurrentFrame(wchar* target, out int Framconst);
  ...
  int TCallFrame(wchar* target, int Framconst);
