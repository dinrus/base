...
static:
	...
	{
		makeModule(t, "отладка", function uword(MDThread* t, uword numParams)
		{
			newFunction(t, &setHook,        "устХук");        newGlobal(t, "устХук");
			newFunction(t, &getHook,        "дайХук");        newGlobal(t, "дайХук");
			newFunction(t, &callDepth,      "callDepth");      newGlobal(t, "callDepth");
			newFunction(t, &sourceName,     "имяИстока");     newGlobal(t, "имяИстока");
			newFunction(t, &sourceLine,     "строкаИстока");     newGlobal(t, "строкаИстока");
			newFunction(t, &getFunc,        "дайФунк");        newGlobal(t, "дайФунк");
			newFunction(t, &numLocals,      "нумМестные");      newGlobal(t, "нумМестные");
			newFunction(t, &localName,      "имяМестного");      newGlobal(t, "имяМестного");
			newFunction(t, &getLocal,       "дайМестный");       newGlobal(t, "дайМестный");
			newFunction(t, &setLocal,       "устМестный");       newGlobal(t, "устМестный");
			...
			newFunction(t, &currentLine,    "текСтрока");    newGlobal(t, "текСтрока");
			newFunction(t, &lineInfo,      "инфОСтроке");       newGlobal(t,"инфОСтроке");
			newFunction(t, &getMetatable,   "дайМетатабль");   newGlobal(t, "дайМетатабль");
			newFunction(t, &setMetatable,   "устМетатабль");   newGlobal(t, "устМетатабль");
			newFunction(t, &getRegistry,    "дайРеестр");    newGlobal(t, "дайРеестр");
			newFunction(t, &getExtraBytes,  "дайЭкстраБайты");  newGlobal(t, "дайЭкстраБайты");
			newFunction(t, &setExtraBytes,  "устЭкстраБайты");  newGlobal(t, "устЭкстраБайты");
			newFunction(t, &numExtraFields, "нумЭкстраПоля"); newGlobal(t, "нумЭкстраПоля");
			newFunction(t, &getExtraField,  "дайЭкстраПоля");  newGlobal(t, "дайЭкстраПоля");
			newFunction(t, &setExtraField,  "устЭкстраПоля");  newGlobal(t, "устЭкстраПоля");

			return 0;
		});

		importModuleNoNS(t, "отладка");
	}
	...
	uword lineInfo(MDThread* t, uword numParams)
	{
		...
		else
		{
			...
			pushNull(t);
			methodCall(t, -2, "ключи", 1);
			pushNull(t);
			methodCall(t, -2, "сортируй", 1);
		}
	...
	uword getMetatable(MDThread* t, uword numParams)
	{
		...
		switch(name)
		{
			case "пусто":      getTypeMT(t, MDValue.Type.Null);      break;
			case "бул":      getTypeMT(t, MDValue.Type.Bool);      break;
			case "цел":       getTypeMT(t, MDValue.Type.Int);       break;
			case "плав":     getTypeMT(t, MDValue.Type.Float);     break;
			case "сим":      getTypeMT(t, MDValue.Type.Char);      break;
			case "ткст":    getTypeMT(t, MDValue.Type.String);    break;
			case "табль":     getTypeMT(t, MDValue.Type.Table);     break;
			case "массив":     getTypeMT(t, MDValue.Type.Array);     break;
			case "функция":  getTypeMT(t, MDValue.Type.Function);  break;
			case "класс":     getTypeMT(t, MDValue.Type.Class);     break;
			case "экземпл":  getTypeMT(t, MDValue.Type.Instance);  break;
			case "зоним": getTypeMT(t, MDValue.Type.Namespace); break;
			case "нить":    getTypeMT(t, MDValue.Type.Thread);    break;
			case "искоб": getTypeMT(t, MDValue.Type.NativeObj); break;
			case "слабссыл":   getTypeMT(t, MDValue.Type.WeakRef);   break;
	...
	uword setMetatable(MDThread* t, uword numParams)
	{
		...
		switch(name)
		{
			case "пусто":      setTypeMT(t, MDValue.Type.Null);      break;
			case "бул":      setTypeMT(t, MDValue.Type.Bool);      break;
			case "цел":       setTypeMT(t, MDValue.Type.Int);       break;
			case "плав":     setTypeMT(t, MDValue.Type.Float);     break;
			case "сим":      setTypeMT(t, MDValue.Type.Char);      break;
			case "ткст":    setTypeMT(t, MDValue.Type.String);    break;
			case "табль":     setTypeMT(t, MDValue.Type.Table);     break;
			case "массив":     setTypeMT(t, MDValue.Type.Array);     break;
			case "функция":  setTypeMT(t, MDValue.Type.Function);  break;
			case "класс":     setTypeMT(t, MDValue.Type.Class);     break;
			case "экземпл":  setTypeMT(t, MDValue.Type.Instance);  break;
			case "зоним": setTypeMT(t, MDValue.Type.Namespace); break;
			case "нить":    setTypeMT(t, MDValue.Type.Thread);    break;
			case "искоб": setTypeMT(t, MDValue.Type.NativeObj); break;
			case "слабссыл":   setTypeMT(t, MDValue.Type.WeakRef);   break;
