...
static:
	...
	{
		makeModule(t, "ткст", function uword(MDThread* t, uword numParams)
		{
			newFunction(t, &joinArray, "сложиМассив"); newGlobal(t, "сложиМассив");

			newNamespace(t, "ткст");
				newFunction(t, &opApply,     "опПрименить");     fielda(t, -2, "опПрименить");
				newFunction(t, &join,        "объедини");        fielda(t, -2, "объедини");
				newFunction(t, &toInt,       "вЦел");       fielda(t, -2, "вЦел");
				newFunction(t, &toFloat,     "вПлав");     fielda(t, -2, "вПлав");
				newFunction(t, &compare,     "сравни");     fielda(t, -2, "сравни");
				newFunction(t, &icompare,    "сравнилюб");    fielda(t, -2, "сравнилюб");
				newFunction(t, &find,        "найди");        fielda(t, -2, "найди");
				newFunction(t, &ifind,       "найдилюб");       fielda(t, -2, "найдилюб");
				newFunction(t, &rfind,       "найдрек");       fielda(t, -2, "найдрек");
				newFunction(t, &irfind,      "найдлюбрек");      fielda(t, -2, "найдлюбрек");
				newFunction(t, &toLower,     "впроп");     fielda(t, -2, "впроп");
				newFunction(t, &toUpper,     "взаг");     fielda(t, -2, "взаг");
				newFunction(t, &repeat,      "повтори");      fielda(t, -2, "повтори");
				newFunction(t, &reverse,     "реверсни");     fielda(t, -2, "реверсни");
				newFunction(t, &split,       "разбей");       fielda(t, -2, "разбей");
				newFunction(t, &splitLines,  "разбейнастр");  fielda(t, -2, "разбейнастр");
				newFunction(t, &strip,       "убери");       fielda(t, -2, "убери");
				newFunction(t, &lstrip,      "уберилев");      fielda(t, -2, "уберилев");
				newFunction(t, &rstrip,      "убериправ");      fielda(t, -2, "убериправ");
				newFunction(t, &replace,     "поменяй");     fielda(t, -2, "поменяй");
				newFunction(t, &startsWith,  "началоНа");  fielda(t, -2, "началоНа");
				newFunction(t, &endsWith,    "конецНа");    fielda(t, -2, "конецНа");
				newFunction(t, &istartsWith, "началоНа_люб"); fielda(t, -2, "началоНа_люб");
				newFunction(t, &iendsWith,   "конецНа_люб");   fielda(t, -2, "конецНа_люб");
			setTypeMT(t, MDValue.Type.String);

			return 0;
		});
		
		importModuleNoNS(t, "ткст");
	}
	...
	uword opApply(MDThread* t, uword numParams)
	{
		...
		if(optStringParam(t, 1, "") == "реверсни")
		{
			...
			newFunction(t, &iteratorReverse, "реверсОбходчика", 1);
			dup(t, 0);
			pushInt(t, len(t, 0));
		}
		else
		{
			pushInt(t, 0);
			newFunction(t, &iterator, "обходчик", 1);
			dup(t, 0);
			pushInt(t, -1);
		}
