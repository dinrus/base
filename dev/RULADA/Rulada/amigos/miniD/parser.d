...
// ================================================================================================================================================
	...
	*/
	public FuncDef parseStatements(char[] name)
	{
		...
		params ~= FuncDef.Param(new(c) Identifier(c, l.loc, c.newString("сей")));
	...
	*/
	public FuncDef.Param[] parseFuncParams(out bool isVararg)
	{
		...
		{
			Param p = void;
			p.name = new(c) Identifier(c, l.loc, c.newString("сей"));
		...
		{
			ret ~= Param(new(c) Identifier(c, l.loc, c.newString("сей")));
	...
	*/
	public ushort parseParamType(out Expression[] classTypes)
	{
			...
			switch(l.type)
			{
				...
				default:
					...
					else
					{
						...
						{
							case "бул":      addConstraint(MDValue.Type.Bool); break;
							case "цел":       addConstraint(MDValue.Type.Int); break;
							case "плав":     addConstraint(MDValue.Type.Float); break;
							case "сим":      addConstraint(MDValue.Type.Char); break;
							case "ткст":    addConstraint(MDValue.Type.String); break;
							case "табль":     addConstraint(MDValue.Type.Table); break;
							case "массив":     addConstraint(MDValue.Type.Array); break;
							case "нить":    addConstraint(MDValue.Type.Thread); break;
							case "искоб": addConstraint(MDValue.Type.NativeObj); break;
							case "слабссыл":   addConstraint(MDValue.Type.WeakRef); break;
		...
		else if(l.type == Token.Ident && l.tok.stringValue == "любой")
		{
			l.next();
			ret = TypeMask.Any;
		}
	...
	*/
	public ClassDef parseClassDef(bool nameOptional)
	{
		...
		if(l.type == Token.Colon)
		{
			...
			baseClass = new(c) IdentExp(c, new(c) Identifier(c, l.loc, c.newString("Объект")));
		...
		while(l.type != Token.RBrace)
		{
			switch(l.type)
			{
				...
				case Token.This:
					...
					addMethod(parseFuncBody(loc, new(c) Identifier(c, loc, c.newString("конструктор"))));
					break;
				...
				case Token.At:
					...
					if(l.type == Token.Function || l.type == Token.This)
					{
						...
						{
							auto loc = l.expect(Token.This).loc;
							fd = parseFuncBody(loc, new(c) Identifier(c, loc, c.newString("конструктор")));
						}
	...
	*/
	public MethodCallExp parseSuperCallExp()
	{
		...
		if(l.type == Token.Dot)
		{
			...
			method = new(c) StringExp(c, location, c.newString("конструктор"));
