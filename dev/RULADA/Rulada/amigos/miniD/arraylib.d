...
static:
	...
	{
		makeModule(t, "массив", function uword(MDThread* t, uword numParams)
		{
			newFunction(t, &array_new, "нов");     newGlobal(t, "нов");
			newFunction(t, &range,     "охват");   newGlobal(t, "охват");

			newNamespace(t, "массив");
				newFunction(t, &sort,      "сортируй");       fielda(t, -2, "сортируй");
				newFunction(t, &reverse,   "реверсни");    fielda(t, -2, "реверсни");
				newFunction(t, &array_dup, "дубль");        fielda(t, -2, "дубль");

					newFunction(t, &iterator,        "обходчик");
					newFunction(t, &iteratorReverse, "реверсОбходчика");
				newFunction(t, &opApply,   "опПрименить", 2); fielda(t, -2, "опПрименить");

				newFunction(t, &expand,    "раскорой");     fielda(t, -2, "раскорой");
				newFunction(t, &toString,  "вТкст");   fielda(t, -2, "вТкст");
				newFunction(t, &apply,     "примени");      fielda(t, -2, "примени");
				newFunction(t, &map,       "карта");        fielda(t, -2, "карта");
				newFunction(t, &reduce,    "reduce");     fielda(t, -2, "reduce");
				newFunction(t, &each,      "каждый");       fielda(t, -2, "каждый");
				newFunction(t, &filter,    "фильтруй");     fielda(t, -2, "фильтруй");
				newFunction(t, &find,      "найди");       fielda(t, -2, "найди");
				newFunction(t, &findIf,    "найдиЕсли");     fielda(t, -2, "найдиЕсли");
				newFunction(t, &bsearch,   "бпоиск");    fielda(t, -2, "бпоиск");
				newFunction(t, &array_pop, "вынь");        fielda(t, -2, "вынь");
				newFunction(t, &set,       "установи");        fielda(t, -2, "установи");
				newFunction(t, &min,       "мин");        fielda(t, -2, "мин");
				newFunction(t, &max,       "макс");        fielda(t, -2, "макс");
				newFunction(t, &extreme,   "extreme");    fielda(t, -2, "extreme");
				newFunction(t, &any,       "любой");        fielda(t, -2, "любой");
				newFunction(t, &all,       "все");        fielda(t, -2, "все");
				newFunction(t, &fill,      "заполни");       fielda(t, -2, "заполни");
				newFunction(t, &append,    "добвк");     fielda(t, -2, "добвк");

					newTable(t);
				newFunction(t, &flatten,   "flatten", 1); fielda(t, -2, "flatten");

				newFunction(t, &makeHeap,  "сделайКучу");   fielda(t, -2, "сделайКучу");
				newFunction(t, &pushHeap,  "суньКучу");   fielda(t, -2, "суньКучу");
				newFunction(t, &popHeap,   "выньКучу");    fielda(t, -2, "выньКучу");
				newFunction(t, &sortHeap,  "сортируйКучу");   fielda(t, -2, "сортируйКучу");
				newFunction(t, &count,     "сочти");      fielda(t, -2, "сочти");
				newFunction(t, &countIf,   "сочтиЕсли");    fielda(t, -2, "сочтиЕсли");
			setTypeMT(t, MDValue.Type.Array);

			return 0;
		});

		importModuleNoNS(t, "массив");
	}
	...
	uword sort(MDThread* t, uword numParams)
	{
		...
		if(numParams > 0)
		{
			if(isString(t, 1) && getString(t, 1) == "реверсни")
			{
