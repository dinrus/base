...
class FuncDef : AstNode
{
	...
	*/
	public this(ICompiler c, CompileLoc location, Identifier name, Param[] params, bool isVararg, Statement code)
	{
		...
		assert(params.length > 0 && params[0].name.name == "сей");
...
{
	/**
	This indicates "дефолт" protection, which means global at module-level scope and local
	everywhere else.
	*/
	Default,
...
class ForeachStmt : Statement
{
	...
	*/
	...
	long.  Semantically, the first element is the "обходчик", the second the "состояние", and
	the third the "index".  However MiniD will automatically call opApply on the "обходчик"
...
class MethodCallExp : PostfixExp
{
	...
	*/
	...
	/**
	If this member is true, 'op' will be null (and will be interpreted as a "сей").
	*/
