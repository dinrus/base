...
static:
	...
	{
		makeModule(t, "время", function uword(MDThread* t, uword numParams)
		{
			Timer.init(t);
			newFunction(t, &microTime,  "микроВремя");  newGlobal(t, "микроВремя");
			newFunction(t, &dateString, "текстДаты"); newGlobal(t, "текстДаты");
			newFunction(t, &dateTime,   "датаВремя");   newGlobal(t, "датаВремя");
			newFunction(t, &culture,    "культура");    newGlobal(t, "культура");
			newFunction(t, &timestamp,  "штампВремени");  newGlobal(t, "штампВремени");
			newFunction(t, &timex,      "timex");      newGlobal(t, "timex");
			newFunction(t, &sleep,      "сон");      newGlobal(t, "сон");
			newFunction(t, &compare,    "сравни");    newGlobal(t, "сравни");

			return 0;
		});

		importModuleNoNS(t, "время");
	}
	...
	{
		auto year = field(t, tab, "год");
		auto month = field(t, tab, "месяц");
		auto day = field(t, tab, "день");
		auto hour = field(t, tab, "час");
		auto min = field(t, tab, "мин");
		auto sec = field(t, tab, "сек");
	...
	{
		pushInt(t, time.date.year);    fielda(t, dest, "год");
		pushInt(t, time.date.month);   fielda(t, dest, "месяц");
		pushInt(t, time.date.day);     fielda(t, dest, "день");
		pushInt(t, time.time.hours);   fielda(t, dest, "час");
		pushInt(t, time.time.minutes); fielda(t, dest, "мин");
		pushInt(t, time.time.seconds); fielda(t, dest, "сек");
	}
	...
	static:
		...
		{
			CreateClass(t, "Таймер", (CreateClass* c)
			{
				c.method("пуск", &start);
				c.method("стоп", &stop);
				c.method("секунды", &seconds);
				c.method("миллисек", &millisecs);
				c.method("микросек", &microsecs);
			});

			newFunction(t, &allocator, "Таймер.аллокатор");
			setAllocator(t, -2);
		...
		{
			return checkInstParam!(Members)(t, 0, "Таймер");
		}
