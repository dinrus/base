  ...
  public:
	...
	void drawBounds(Point parentPos = Point(0,0))
	{
		if (hide)
			...
			drawRectangle(position + parentPos, size, color, false);
	}
	...
	Widget getWidget(char[] widgetName)
	{
		...
		{
			throw new Exception("Error: The widget " ~ widgetName ~ " does not exist!");
		}
	...
	void addWidget(char[] wname, Widget w)
	{
		if (wname in widgets)
		{
			throw new Exception("Error: The widget " ~ wname ~ " already exist!");
		}
