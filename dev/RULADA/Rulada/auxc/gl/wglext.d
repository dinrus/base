module auxc.gl.wglext;

pragma(lib, "auxC.lib");

version (Windows) {
private import auxc.gl.gl;
version (Rulada){ import os.windows, std.loader;}
else{private import std.c.windows.windows, std.loader;}
...
private void* getProc(char[] procname) {
	...
	if (symbol is null) {
		printf ("Failed to load WGL extension proc address %s.\n", procname);
	}
	return (symbol);
}
