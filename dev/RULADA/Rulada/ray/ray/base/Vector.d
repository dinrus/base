...
struct Vec2 {
	...
	in {
		debug ( FLOAT_PARANOID ) {
			assert( Math.isValid( v.x ) && Math.isValid( v.y ) );
			assert( Math.abs( v.x - 0.0f ) > 1e-14 && Math.abs( v.y - 0.0f ) > 1e-14 );
		}
	}
	body {
		x /= v.x;
	...
	in {
		assert( maxMagnitude > 0.0f );
	}
	body {
		float	mag = magnitudeSquared;

		if ( mag > maxMagnitude * maxMagnitude ) {
			mag = maxMagnitude / Math.sqrt( mag );
			x *= mag;
			y *= mag;
		}
	}
	...
	char[] toUtf8() {
		return Math.toUtf8( "( x:{0:E} y:{1:E} )", x, y );
	}
