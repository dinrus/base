module vector_tester;

import helpers.gdkkeysyms;

version (Tango)
{
	debug(logging) import tango.util.log.Trace;//Thread safe console output.
...
//import rae.ui.Scrollbar;//for PixelPerPixelRectangle
import rae.ui.Label;
...
class PixelPerPixelRectangle : Rectangle
{
	...
	void render(Draw draw)
	{
		...
		version(zoomGL)
		{
			if( orientation == OrientationType.HORIZONTAL )
			{
				renderPixelPerPixel( draw, (w / g_rae.pixel)+102.0f, texture.height );
			}
			else if( orientation == OrientationType.VERTICAL )
			{
				renderPixelPerPixel( draw, texture.height, (h / g_rae.pixel)+102.0f );
			}
		}
...
class ProjectWindow : Window
{
	this()
	{
		...
		*/
		...
		winwin2.side1.container.signalMouseMotion.attach(&mouseHandler);
		winwin2.side1.container.signalScrollUp.attach(&mouseHandler);
		winwin2.side1.container.signalScrollDown.attach(&mouseHandler);
		add(winwin2);
		
		container.signalScrollUp.attach(&mouseHandler);
		container.signalScrollDown.attach(&mouseHandler);
		
		Rectangle rect1 = new PixelPerPixelRectangle();
		//rect1.shape = new ShapeRoundRectangle(rect1.ix1, rect1.iy1, rect1.ix2, rect1.iy2);
		rect1.shape = new ShapeRoundRectangle( -0.05f, -0.025f, 0.05f, 0.025f);
		//rect1.shape.texture = g_rae.getTextureFromTheme("Rae.Slider.control");
		rect1.shape.themeTexture = "Rae.Slider.control";
		rect1.name = "Red";
		rect1.arrangeType = ArrangeType.FREE;
		rect1.defaultSize( 0.1f, 0.05f );
		...
		add(rect1);
			...
			Rectangle rect2 = new PixelPerPixelRectangle();
			rect2.shape = new ShapeRoundRectangle(rect1.ix1, rect1.iy1, rect1.ix2, rect1.iy2);
			//rect2.shape.texture = g_rae.getTextureFromTheme("Rae.Slider.control");
			rect2.shape.themeTexture = "Rae.Slider.control";
			rect2.name = "Green";
			rect2.arrangeType = ArrangeType.FREE;
			rect2.defaultSize( 0.05f, 0.025f );
			...
			rect1.add(rect2);
				...
				Rectangle rect4 = new PixelPerPixelRectangle();
				rect4.shape = new ShapeRoundRectangle(rect1.ix1, rect1.iy1, rect1.ix2, rect1.iy2);
				//rect4.shape.texture = g_rae.getTextureFromTheme("Rae.Slider.control");
				rect4.shape.themeTexture = "Rae.Slider.control";
				rect4.name = "Blue";
				rect4.arrangeType = ArrangeType.FREE;
				rect4.defaultSize( 0.05f, 0.025f );
	...
	void mouseHandler( InputState input, Rectangle wid )
	{
		...
		if( currentWidget !is null )
		{
			...
			infoLabel2.name = "zoom:" ~ Float.toString( currentWidget.zoom );
			infoLabel3.name = "xPos:" ~ Float.toString( currentWidget.xPos );
			infoLabel4.name = "yPos:" ~ Float.toString( currentWidget.yPos );
		}
	}
