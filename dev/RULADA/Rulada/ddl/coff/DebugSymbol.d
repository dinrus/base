...
//		CVSegInfo[]	segInfo;
	...
	// symbols list
		...
		bit isInAddress( uint addr )
		{
			return isInAddress( codeSection, addr - codeAddress );
		}

		bit isInAddress( ushort sect, uint offset )
		{
			if( codeSection != sect )
				return false;
...
{
	public:
		bit isClass;			// is a class def
		bit isPacked;			// structure is packed
		bit isForwardRef;		// is only a forward reference
		bit hasCtor;			// has a constructor
		bit hasOverloadedOps;	// has overloaded operators
		bit hasOpAssign;		// has an assignment operator
		bit hasOpCast;			// has a cast operator
		bit isNested;			// nested structure
		bit hasNested;			// contains nested structures
		bit isScoped;			// is scoped
...
class SymbolManager
{
	...
	// source line management
		bit GetLineFromAddr( uint address, out uint line, out char[] file )
		{
			return false;
		}
