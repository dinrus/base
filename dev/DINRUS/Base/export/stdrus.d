...
public import tpl.args, tpl.stream;
...
/* Требуется единица каждого формата, так как субнормальные числа с пз
 * могут конвертироваться в нормальные реалы.
 */
...
цел нормален_ли(X)(X x)
{
    ...
    static if(real.mant_dig==106) { // doubledouble
        // doubledouble нормален, если нормальна наименее значимая часть.
        return нормален_ли((cast(дво*)&x)[МАНТИССА_МЗЧ]);
    } else {
        // смешное предупреждение DMD
...
alias длина length;
    ...
    enum Режим
    {
	Чтение,		/// читать существующий файл
	ЧтенЗапНов,	/// удалить существующий файл, записать новый файл
	ЧтенЗап,	/// читать/записать существующий файл, создать, если ещё не существует
	ЧтенКопирПриЗап, /// читать/записать существующий файл, копировать при записи
...
abstract class Адрес
{
	...
	ПСемействоАдресов семействоАдресов();	/// Семейство данного адреса.
	ткст вТкст();		/// Читаемая человеком строка, представляющая данный адрес.
}
...
extern (Windows) alias бцел function(ук) stdfp;
...
{	
    ...
    this(цел function(ук) fp, ук арг, т_мера размстека = 0);
   ...
   проц старт();		
    ...
    enum СН
    {
	НАЧАЛЬНОЕ,	/// Поток ещё не запущен.
	ПУЩЕНА,	/// Этот поток выполняется или на паузе.
	ПРЕРВАНА,	/// Этот поток завершён.
        ЗАВЕРШЕНА        /// Этот поток очищен.
    }
    ...
    enum ПРИОРИТЕТ
    {
	УВЕЛИЧЬ,	/// Увеличить приоритет потока
	УМЕНЬШИ,	/// Уменьшить приоритет потока
	НИЗКИЙ,		/// Присвоить потоку низкий приоритет
	ВЫСОКИЙ,	/// Присвоить потоку высокий приоритет
	НОРМАЛЬНЫЙ,
    }
...
extern(D)
{
	...
	проц выведиМД5Дайджест(ббайт[16] дайджест); 
	ткст дайджестМД5вТкст(ббайт[16] дайджест); 
	//ЗИП

	//ЗЛИБ
	бцел адлер32(бцел адлер, проц[] буф); 
	бцел цпи32(бцел кс, проц[] буф);
	...
	//утилиты
	ткст текстСисОшибки(бцел кодош); 
	...
	бкрат кодируйДлину64(бцел сдлин); 
	ткст кодируй64(ткст стр, ткст буф = ткст.init);
	бкрат раскодируйДлину64(бцел кдлин); 
	...
	цел числобукв_ли(дим б);
	цел буква_ли(дим б); 
	цел управ_ли(дим б); 
	цел цифра_ли(дим б); 
	цел проп_ли(дим б); 
	цел пунктзнак_ли(дим б); 
	цел межбукв_ли(дим б); 
	цел заг_ли(дим б); 
	цел цифраикс_ли(дим б); 
