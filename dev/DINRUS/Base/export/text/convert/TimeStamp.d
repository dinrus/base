/*******************************************************************************
        Преобразует между исконныи и текстовым представлениями значения
		HTTP времени. Внутренне время представляется как UTC с эпохой,
        фиксированной на 1-го Янв 1970. Это текстовое представление
        форматируется в соотвествии с RFC 1123 и парсер принимает форматы
        RFC 1123, RFC 850 или анзисивремя.

        Детали можн найти по ссылке http://www.w3.org/Protocols/rfc2616/rfc2616-sec3.html.

        ---
        import ШтампВремени = text.convert.TimeStamp;

        auto t = ШтампВремени.разбор ("Sun, 06 Nov 1994 08:49:37 GMT");
        ---
...
T[] форматируй(T) (T[] вывод, Время t)
{
    ...
    if (t is t.макс)
        throw new ИсклНелегальногоАргумента ("TimeStamp.форматируй :: непригодный аргумент Время");
...
/******************************************************************************
        ...
        ANSI C's анзисивремя() форматируй :: "Sun Nov 6 08:49:37 1994"
