    ...
    debug (UnitTest)
    {
        unittest
        {
            ...
            foreach (бцел i, ткст test_key; test_keys)
            {
                ...
                r.init(да, new СимметричныйКлюч(БайтКонвертер.раскодируйГекс(test_key)));
                
                // Encryption
                r.обнови(БайтКонвертер.раскодируйГекс(test_plaintexts[i]), буфер);
                ...
                // Decryption
                r.обнови(БайтКонвертер.раскодируйГекс(test_ciphertexts[i]), буфер);
                результат = БайтКонвертер.кодируйГекс(буфер);
