...
enum ПРежимДоступаКРеестру
{
    ...
    ,   Чтение                =   (   0x00020000L
                                    ...
                                    |   Уведомление)
                                &   ~(0x00100000L) //!< Комбинирует права доступа для СтандартноеЧтение, ЗапросЗначения, ПеречислениеПодключей и Уведомление
    ,   Запись               =   (   0x00020000L
                                    |   УстановкаЗначения
                                    |   СозданиеПодключа)
                                &   ~(0x00100000L) //!< Комбинирует права доступа для СтандартнаяЗапись, УстановкаЗначения и СозданиеПодключа
    ,   Выполнение             =   Чтение
                                &   ~(0x00100000L) //!< Разрешение на доступ для чтения
...
extern(D) class Ключ
{
 ...
 this(HKEY hkey, ткст имя, булево созд);
    ...
    /// Перечислительная последовательность имён всех подключей этого ключа
    РядИмёнКлючей именаКлючей();
	
    /// Число значений
    бцел члоЗначений();
	
    ///Перечислительная последовательность всех значений этого ключа
    РядЗначений значения();

    /// Перечислительная последовательность имён всех значений этого ключа
...
/// Представляет локальный системный реестр.
///
/// \ingroup group_D_win32_reg
...
extern(D) class РядИмёнКлючей
{
    ...
    /// Имя ключа по указанному индексу
    ///
