...
enum : DWORD
{
    ...
    EXCEPTION_FLT_INEXACT_результат = (0xc000008f),
...
enum : DWORD
{
    ...
    STATUS_FLOAT_INEXACT_результат = (0xC000008F),
...
version(Win32SansUnicode)
{
    alias GCP_результатSA GCP_результатS;
}
else
{
    alias GCP_результатSW GCP_результатS;
}
alias GCP_результатS* LPGCP_результатS;
alias GCP_результатS TAGGCP_результатS;
alias GCP_результатS TGCPрезультатS;
alias GCP_результатS* PGCPрезультатS;
...
DWORD GetCharacterPlacementA(HDC, LPCSTR, int, int, LPGCP_результатS, DWORD);
...
DWORD GetCharacterPlacementW(HDC, LPCWSTR, int, int, LPGCP_результатS, DWORD);
