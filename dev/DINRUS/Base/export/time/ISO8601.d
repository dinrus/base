...
)
{
    ...
    цел sec =void;
    if (
        парсируйЦел(p, мин(cast(т_мера)2, осталось()), sec) != 2 ||
        sec > 60 ||
        (пд.конецДня && sec != 0)
    )
        return толькоМинута;
    ...
    добавьСекунды(пд, sec);

    auto onlySecond = съедено();
...
байт дайДесяток(T)(ref T* p, т_мера длин, ref ПолнаяДата пд, ббайт which)
{
    ...
    if (
        ...
        // than unity, the decimal знак shall be preceded by two
        // zeros"...
...
байт дайВременнуюЗону(T)(ref T* p, т_мера длин, ref ПолнаяДата пд, ббайт разделители, бул delegate(T[]) готово)
{
    ...
    цел час =void;
    if (парсируйЦел(p, мин(cast(т_мера)2, длин-1), час) != 2 || час > 12 || (час == 0 && factor == 1))
        return ПЛОХ;

    добавьЧасы(пд, factor * час);
...
{
    d.знач += ИнтервалВремени.изДней   (n-1);    // ditto
}
