...
)
{
    ...
    case 4:
        ...
        if (разделители == ДА)
        {
            // День не принимаем: так если вы получили только месяц
            p -= 2;
            i = месяц;
        goto case 2;
        }
    ...
    case 3:
        // (год)-DDD
        // i - это порядковый день внутри года
 ...
 * связанных с временем полей. Пока нет уверенности, что никогда не встретится значение
 * полуночи, нужно проверять возвратное значение, чтобы убедиться в
 * ожидаемом успехе разбора.
 *
 * Экстра поля в РасширеннаяДата:
 *
 * Секунды могут равняться 60, если часы и минуты равны 23 и 59, так как високосная сек
 * добавлена к UTC время. Сек Время в этом случае будет 59.
 *
 * Часы могут быть 0-24: последнее отмечает конец  дня, а первое - его начало,
 * хотя все они относятся к одному и тому же мгновению времени. Время
 * в любом случае будет точно 00:00.
 *
 * Примеры:
 * ---
 * Время t;
...
// всёВалидно используется только для получения разборДатыИВремени() для правильного отлова ошибок
...
)
{
    ...
    // Добавим часы даже, если конецДня: день должен быть следщ день, а не
    // предыдущий
    добавьЧасы(пд, час);
...
/** Разбирает комбинированные дату и время в формате, заданном в ISO 8601:2004.
 *
 * Возвращает число  символов, использованных для компонирования валидной даты и времени.
 * ZНоль возвращается, если полные дата и время не могут быть извлечены. В таком
 * случае, значение результирующего Время или РасширеннаяДата неопределённое.
 *
 * Эта функция более строга, чем простой вызов разборДаты, за которым следует
 * разборВремени: не допускаются развёрнутые годы или уменьшенные даты
 * (годы в двух цифрах), а использование разделителя должно быть консистентным.
 *
 * Хотя в этом стандарте допускается пропуск T между датой и
 * временем, сама функция этого требует.
 *
 * Примеры:
 * ---
 * Время t;
 ...
 *
 * // Апрель 12-ое, 1985 23:50:30,042
 * разборДатыИВремени("1985W155T235030,042", t);
 *
 * // Неверное время: возвращает ноль
 * разборДатыИВремени("1902-03-04T10:1a", t);
 *
 * // Отделение T опущено: возвращается ноль
 * разборДатыИВремени("1985-04-1210:15:30+04:00", t);
 *
 * // Неконсистентные разделители: во всех возвращается ноль
...
public т_мера разборДатыИВремени(T)(T[] ист, ref ПолнаяДата пд)
{
    ...
    if (
        ...
        // по взаимному соглашению этот T может быть опущен,
        // но это лишь удобный метод для дата+время
...
Приватные ф-ции, используемые датой
...
бул парсируйГод(T)(ref T* p, т_мера длин, ббайт развёрнуто, ref ПолнаяДата пд)
{
    ...
    бул выполниРазбор()
    {
        ...
        // it's Y{развёрнуто}YY, Y{развёрнуто}YYYY, либо недопустимо
...
// Находит месяц и день, если даны календарная неделя и день недели.
// Использует пд.год для вычислений високосного года.
// Возвращает нет, если неделя и пд.год несовместимы.
бул дайМесяцИДеньИзНедели(ref ПолнаяДата пд, цел неделя, цел день = 1)
{
    if (неделя < 1 || неделя > 53 || день < 1 || день > 7)
        return нет;

    цел год = пд.год;

    // Только у годов, начинающихся с четверга и у високосных, начинающихся со среды,
    // 53 недели.
    if (неделя == 53)
    {
        цел стартовыйДень = деньНедели(год, 1, 1);

        if (!(стартовыйДень == 4 || (високосен_ли(год) && стартовыйДень == 3)))
            return нет;
    }

    // XXX
    // дни с год-01-04, плюс 4 (?)...
    /* В реале это как-то странно: Не ***понимаю, почему это работает***.
     * Наткнулся на это чисто случайно. Кажется, это работает, хотя -
     * иногда нет...
    */
    добавьДни(пд, 7*(неделя - 1) + день - деньНедели(год, 1, 4) + 4);
...
Приватные, используемые для время.
...
// ДВ всегда UTC, поэтому добавляется только смещение к полям даты;
