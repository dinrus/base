...
// Limits for when to switch between multИПlication algorithms.
     ...
     */
    бцел многобайтСдвигЛБезММХ(бцел [] приёмник, бцел [] ист, бцел numbits);
     ...
     *  numbits must be in the range 1..31
     */
    проц многобайтСдвигПБезММХ(бцел [] приёмник, бцел [] ист, бцел numbits);
...
// It is used by Всё MulAdd, multИПlyAccumulate, and triangleAccumulate
    ...
    /**
     * приёмник[#] += ист[#] * множитель OP carry(0..FFFF_FFFF).
     * where op == '+' or '-'
     * Возвращаетs carry out of MSB (0..FFFF_FFFF).
     */
    бцел многобайтУмножПрибавь(сим op)(бцел [] приёмник, бцел[] ист, бцел множитель, бцел carry);
...
//приёмник += ист[0]*ист[1...$] + ист[1]*ист[2..$] + ... + ист[$-3]*ист[$-2..$]+ ист[$-2]*ист[$-1]
// assert(приёмник.length = ист.length*2);
// assert(ист.length >= 3);
    проц многобайтПрямоугАккумАсм(бцел[] приёмник, бцел[] ист);
