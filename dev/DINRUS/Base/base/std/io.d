...
module std.io;
import std.io, win, std.utf, std.format, std.file, std.utf;
...
export extern(D)
{
	...
	проц инфо(ткст сооб)
			{
			ОкноСооб(null, вЮ16(сооб), "Сообщение Динрус", ПСооб.Инфо|ПСооб.Поверх);
			}

ткст читайстр()
	{
		ткст buf;
		readln(cidrus.стдвхо, buf);
		 return buf;
		/+
	бцел  mode, get;
	ткст input[8];
	...
	writefx( cidrus.стдвых, _arguments, _argptr, 0);
	}

	проц пишифнс(...)//////
	{
	writefx( cidrus.стдвых, _arguments, _argptr, 1);
	}
   ...
   // ткст fmt = null;
	...
	проц пишиф_в(cidrus.фук чф, ...)//////
	{
		...
		writefx( чф, _arguments, _argptr, 0);
	}

	проц пишифнс_в(cidrus.фук чф, ...)///////
	{
		writefx( чф, _arguments, _argptr, 1);
	}
...
import std.string;
import runtime;
public import rt.charset: fromMBSz;
