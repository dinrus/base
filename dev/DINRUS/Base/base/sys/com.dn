 ...
 Исключение исклКомРез(цел кодОшибки) {
  switch (кодОшибки) {
    case ПКомРез.Нереализовано:
      throw new Error("КОМ: Нереализованная функция");
    case ПКомРез.НеИнтерфейс:
      throw new Error("КОМ: Неудачное преобразование типа");
    case ПКомРез.Ук:
      throw new Error("КОМ: Ссылка на пустое");
    case ПКомРез.НетДоступа:
     throw new Error("КОМ: Недостаточно право для доступа");
    case ПКомРез.ВнеПамяти:
       throw new ВнеПамИскл();
	  case ПКомРез.НевернАрг:
      throw new Error("КОМ: Использован неверный параметр");
    default:
  }
