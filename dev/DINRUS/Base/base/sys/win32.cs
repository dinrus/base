...
internal static class Win32Native
{
    ...
    internal const int CLAIM_SECURITY_ATTRIBUTE_DISABLED = 0x10;
    internal const int CLAIM_SECURITY_ATTRIBUTE_DISABLED_BY_DEFAULT = 8;
    internal const int CLAIM_SECURITY_ATTRIBUTE_MANDATORY = 0x20;
    ...
    internal const int CLAIM_SECURITY_ATTRIBUTE_TYPE_OCTET_STRING = 0x10;
    ...
    internal const int CLAIM_SECURITY_ATTRIBUTE_VALID_FLAGS = 0x3f;
    ...
    [DllImport("advapi32.dll", EntryPoint="ConvertSidToStringSidW", CharSet=CharSet.Unicode, SetLastError=true, ExactSpelling=true)]
    ...
    {
        StringBuilder lpBuffer = StringBuilderCache.Acquire(0x200);
        if (FormatMessage(0x3200, IntPtr.Zero, errorCode, 0, lpBuffer, lpBuffer.Capacity, IntPtr.Zero) != 0)
        {
            return StringBuilderCache.GetStringAndRelease(lpBuffer);
        }
        StringBuilderCache.Release(lpBuffer);
        object[] values = new object[] { errorCode };
        return Environment.GetResourceString("UnknownError_Num", values);
    }
    ...
    [ReliabilityContract(Consistency.WillNotCorruptState, Cer.Success), DllImport("kernel32.dll", EntryPoint="RtlZeroMemory")]
    ...
    internal struct SID_AND_ATTRIBUTES
    {
        ...
        {
            SizeOf = Marshal.SizeOf(typeof(Win32Native.SID_AND_ATTRIBUTES));
        }
    }
