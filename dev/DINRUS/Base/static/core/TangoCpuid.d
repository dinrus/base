/**
 Определяет характеристики CPU (ЦПБ), предоставляя информацию
 о размерах кэшов и подсказках по аптимизации ассемблера.

  Этот код основывается на информации, полученной из:
...
struct ИнфОКэше
{
    /// Размер кэша, в килобайтах, per CPU.
    /// For L1 unified (данные + код) caches, этот размер равен половине физического размера.
    ...
    бцел размерСтроки;
    /// как many threads совместно the cache, 0=неизвестно
    бцел чСовместнНитей;
...
/// Тип главного цпб
static ИнфОЦПБ главныйЦПБ;
static this()
{
    version(X86_CPU)
    {
        главныйЦПБ=new ИнфОЦПБX86();
        главныйЦПБ.дайДанныеОЦпб();
    }
    else version(PPC_CPU)
    {
        главныйЦПБ=new ИнфОЦПБPpc();
    }
    else version(ARM)
    {
        главныйЦПБ=new ИнфОЦПБArm();
    }
    else version(SPARC)
    {
        главныйЦПБ=new ИнфОЦПБSparc();
    }
}
...
public:
    ...
    /// Поддерживается ли гиперпоточность?
    бул гиперПоточность()
    {
        return потоковНаЦПБ()>ядерНаЦПБ();
    }
    /// Возвращает число потоков на ЦПБ
    бцел потоковНаЦПБ()
    {
        return 1;
    }
    /// Возвращает число ядер на ЦПБ
    бцел ядерНаЦПБ()
    {
        return 1;
    }
    /// удаляет информацию, сохранённую в данном объекте
    проц сотри()
    {
        ...
        члоУровнейКэша=0;
        имяПроизводителя="неизвестно";
        имяПроцессора="неизвестно";
    }
    /// дублирует данный объект
    ИнфОЦПБ dup()
    {
    ...
    protected проц фиксируйКэш()
    {
        ...
        {
            кэшДанных[0].чСовместнНитей=потоковНаЦПБ()/ядерНаЦПБ();
        }
        члоУровнейКэша = 1;
        ...
        for (цел i=1; i< кэшДанных.length; ++i)
        {
            ...
            {
                кэшДанных[i].чСовместнНитей=потоковНаЦПБ();
            }
        }
    }
    version(X86_CPU)
    {
        /// утилитный метод для получения информации о процессорах x86 
        final ИнфОЦПБX86 x86()
        {
            if (auto рез=cast(ИнфОЦПБX86)this)
                return рез;
            throw new Исключение("не является x86 cpu",__FILE__,__LINE__);
        }
    }
    version(PPC)
    {
        /// утилитный метод для получения информации о  процессорах PPC
        final ИнфОЦПБPpc ppc()
        {
            if (auto рез=cast(ИнфОЦПБPpc)this)
                return рез;
            throw new Исключение("не является ppc cpu",__FILE__,__LINE__);
        }
    }
    version(ARM)
    {
        /// утилитный метод для получения информации о процессорах arm 
        final ИнфОЦПБArm arm()
        {
            if (auto рез=cast(ИнфОЦПБArm)this)
                return рез;
            throw new Исключение("не является arm cpu",__FILE__,__LINE__);
        }
    }
    version(SPARC)
    {
        /// утилитный метод для получения информации о процессорах sparc 
        final ИнфОЦПБSparc sparc()
        {
            if (auto рез=cast(ИнфОЦПБSparc)this)
                return рез;
            throw new Исключение("не является sparc cpu",__FILE__,__LINE__);
        }
    }
}
    ...
    version(Really_D_InlineAsm_X86)
    {
        ...
        проц getcacheinfoCPUID2()
        {
            ...
            do
            {
                ...
                if (firstTime)
                {
                    ...
                    numinfos = a[0] & 0xFF;
                    // and иначе it should be ignored
                    a[0] &= 0xFFFF_FF00;
                    firstTime = нет;
                }
                for (цел c=0; c<4; ++c)
        ...
        проц cpuidX86()
        {
            ...
            if (max_extended_cpuid >= 0x8000_0004)
            {
                ...
                // Intel P4 and PM пад at front with пробелы.
...
final class ИнфОЦПБPpc: ИнфОЦПБ
{
    ...
    {
        super.сотри();
        имяПроизводителя="Неизвестен";
        имяПроцессора="НеизвестныйPPC";
...
final class ИнфОЦПБSparc: ИнфОЦПБ
{
    ...
    проц cpuidSparc(Sparc_Cputype cputype)
    {
        ...
        case Sparc_Cputype.Sparc64V:
            ...
            break;
        default:
            throw new Исключение("непригодный тип цпб");
        }
