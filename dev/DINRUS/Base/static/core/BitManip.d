...
{
    /**
     * Сканирует биты в знач, начиная с бита 0, ища
     * первый установленный бит.
     * Возвращает:
     *	Номер первого установленного бита.
     *	Возвратное значение неопределено, если знач равен нулю.
     */
    цел bsf( бцел знач );


    /**
     * Scans the биты in знач из_ the most significant bit
     * в_ the least significant bit, looking
     * for the first установи bit.
     * Возвращает:
     *	The bit число of the first bit установи.
     *	The return значение is undefined if знач is zero.
     * Example:
     * ---
     ...
     * and progressing. It адресes биты like the Выражение:
    ---
