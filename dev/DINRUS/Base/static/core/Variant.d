...
{
    pragma(msg, "Примечание: Фуекционал Вариант vararg не поддерживается для этой "
...
private
{
    ...
    struct ХранилищеВариантов
    {
        ...
        /*
         * Используется для безопасной установки структуры Массив.  We're essentially
...
struct Вариант
{
     ...
     * -----
     *  цел life = 42;
     *  auto знач = Вариант(typeid(typeof(life)), &life);
     * -----
     */
    ...
    /**
     * This operator допускается you в_ присвой arbitrary values directly преобр_в an
     * existing Вариант.
     *
     * Параметры:
     *  значение = The значение you wish в_ помести in the Вариант.
     ...
     * is указанного тип.  Note that this сравнение is exact: it does
     * not take implicit casting rules преобр_в account.
     *
     * Возвращает:
     *  да if the Вариант содержит a значение of тип T, нет иначе.
     *
     * Example:
     * -----
     *  auto знач = Вариант(cast(цел) 42);
     *  assert(   знач.isA!(цел) );
     *  assert( ! знач.isA!(крат) ); // note no implicit conversion
     * -----
     */
    бул isA(T)()
    {
    ...
    /**
     * This member can be used в_ determine if the значение stored in the Вариант
     * is указанного тип.  This сравнение попытки в_ take implicit
     ...
     *  auto знач = Вариант(cast(цел) 42);
     *  assert( знач.isA!(цел) );
     *  assert( знач.isA!(крат) ); // note implicit conversion
     * -----
     */
    бул isImplicitly(T)()
    {
     ...
     *
     * Возвращает:
     *  да if the Вариант does not contain a значение, нет иначе.
     */
    бул пуст_ли()
    {
        return isA!(проц);
     ...
     * в_ use a Вариант as Всё the лево-hand and право-hand sопрes of an
     * Выражение.  One sопрe of the operator must be a concrete тип in order
    ...
    {
        return получи!(T) - rhs;    /// описано ранее
    }
    ...
    /**
     * Возвращает ткст представление of the тип being stored in this
     * Вариант.
     *
     * Возвращает:
     *  The ткст представление of the тип contained within the Вариант.
     */
    ткст вТкст()
    {
        return тип.вТкст;
...
debug( UnitTest )
{
    ...
    {
        Вариант знач;
        assert( знач.isA!(проц), знач.тип.вТкст );
        assert( знач.пуст_ли, знач.тип.вТкст );

        // Test basic целое хранилище and implicit casting support
        знач = 42;
        assert( знач.isA!(цел), знач.тип.вТкст );
        assert( знач.isImplicitly!(дол), знач.тип.вТкст );
        assert( знач.isImplicitly!(бдол), знач.тип.вТкст );
        assert( !знач.isImplicitly!(бцел), знач.тип.вТкст );
        ...
        // Test strings
        знач = "Hello, World!"c;
        assert( знач.isA!(ткст), знач.тип.вТкст );
        assert( !знач.isImplicitly!(шим[]), знач.тип.вТкст );
        ...
        знач = &знач;
        assert( знач.isA!(Вариант*), знач.тип.вТкст );
        assert( !знач.isImplicitly!(цел*), знач.тип.вТкст );
        assert( знач.isImplicitly!(проц*), знач.тип.вТкст );
        ...
        // Test объект хранилище
        {
            scope o = new Объект;
            знач = o;
            assert( знач.isA!(Объект), знач.тип.вТкст );
...
interface B :
        ...
        знач = 3.1413;
        assert( знач.isA!(дво), знач.тип.вТкст );
        assert( знач.isImplicitly!(реал), знач.тип.вТкст );
        assert( !знач.isImplicitly!(плав), знач.тип.вТкст );
