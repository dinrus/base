 ...
 *
 * Authors:  William З. Baxter III (OLM Digital, Inc.)
 * Дата: 23 Nov 2007
...
void переставь(Т)(inout Т a, inout Т b)
{
    Т t = a;
    a = b;
    b = t;
...
/**
 *  Матрица is a генерный M x N matrix тип.
 *  Dimensions (M rows, N columns) are fixed at compile time.
 *
 *  Matrix элементы are stored in column-major order
 *  like FORTRAN or OpenGL.
 *
 *  Compatible with math.linalg.Vector.
 */
struct Матрица(Т, цел M, цел N)
{
    ...
    /** This contstructor takes M*N элементы listed in ряд-major order.
     *  Пример:
    ...
    /** This constructor takes an массив of M*N элементы listed in ряд-major order:
     *  Пример:
     *      auto M = Матрица!(плав,2,3)([ a00, a01, a02
     *                                     a10, a11, a12 ]);
     */
    static Матрица opCall(Т[] знач)
    {
        assert(знач.length == M*N, "Входной размер вектора должен быть "~ctfe_itoa(M*N));
        Матрица Mat;
        with(Mat)
        {
            mixin(_gen_row_major_body!(M,N)("знач"));
        }
        return Mat;
    }
    ...
    /// Compile-time const matrix with all элементы установи to 0.
     ...
     *                                     a10, a11, a12 ]);
     */
    static Матрица ряд_майор(Т[] знач)
    {
        return Матрица(знач);
    }

    /** This constructor takes an массив of M*N элементы listed in column-major order:
     *  Пример:
     *      auto M = Матрица!(плав,2,3)([ a00, a10,
     *                                     a01, a11,
     *                                     a02, a12 ]);
     */
    static Матрица столб_майор(Т[] знач)
    {
        assert(знач.length == M*N, "Размер входного вектора должен быть "~ctfe_itoa(M*N));
        Матрица M;
        M.значения_[] = знач;
        return M;
    }
    ...
    /// The форма (rows и columns) of the matrix
    static бцел[2] форма = [M,N];

    // Возвращает pointer to the элементы
    Скаляр* укз()
    {
        return значения_.ptr;
    }

    // Возвращает pointer to the i,j-th элемент
    Скаляр* укз(цел i, цел j)
    {
        return значения_.ptr + j*члорядов+i;
    }
    ...
    Скаляр opIndexAssign(Скаляр знач, бцел ряд, бцел столб)
    {
        return значения_[члорядов*столб + ряд] = знач;
    }

    /// Add прирост прир to the элемент [ряд,столб]
    void добавь_элт(бцел ряд, бцел столб, Скаляр прир)
    {
        return значения_[члорядов*столб + ряд] += прир;
    }

    /// Multiply элемент [ряд,столб] by the given фактор
    ...
    Матрица opDiv(Скаляр знач)
    {
        return умножь_скаляр(1/знач);
    }

    /// Or A/=s multiplication by a скаляр
    void opDivAssign(Скаляр знач)
    {
        умножь_присвой_скаляр(1/знач);
    }
    ...
    {
        Скаляр возвр=0;
        foreach(знач; значения_) возвр += знач*знач;
        return возвр;
    }
    ...
    template Матрица_ли(ТАрг)
    {
        ...
        // .члостолбцов, .члорядов и indexing with two значения
    ...
    {
        // Check if it Реализует a suitable concept of
        // .length и indexing with one value
    ...
    ///   (Only действителен if A.члостолбцов == B.члорядов)
    /// Or A*s multiplication by a скаляр
    ...
    /// A*=B Linear algebraic multiplication of A * B
    ///   (Only действителен if A.члостолбцов == B.члорядов == B.члостолбцов)
    /// Or A=*s multiplication by a скаляр
    ...
    Матрица умножь_скаляр(Скаляр f)
    {
        ...
        for(цел i=0; i<размер; i++)
        {
            возвр.значения_[i] = значения_[i]*f;
        }
        return возвр;
    }
    ...
    /// Возвращает транспонированный копируй of the matrix.
    /// For non-sqyare matrices use 'транспонированный'.
...
/** Decomposes A into R*S where R is orthogonal (pure вращение) и
    S is symmetric (scaling и shearing).
    ...
    See
    1. Shoemake и Duff "Matrix Animation и Polar Decomposition",
       Proceedings of Graphics Interface, 1992

    2. Higham, "Computing the Polar Decomposition -- with applications",
       SIAM J. Sci. Stat. Comput, Vol 7,  pp 1160-1174, 1986.

    3. Higham и Schreiber, "Fast Polar Decomposition of an Arbitrary Matrix",
    ...
    out Матрица!(Т, N,N) S)
{
    ...
    // Higham86, и Higham & Schreiber 90
    ...
    //  cases where A имеется scaling near ноль along one ось.)

    цел максОбходов = 100;  // shouldn't ever hit this
    Matrix Rprev = R;
    R = 0.5f * (R + R.transposeauxd.инверсия);
    while (!((R - Rprev).квнорм < 1000*Т.epsilon) && максОбходов--)
    {
        Matrix Rinv = R.инверсия;
        // Alternate гамма, may (or may not) be a bit slower.
        //реал гамма = квкор( квкор( Rinv.квнорм / R.квнорм ) );
        ...
        реал гамма = квкор( квкор( (Rinv.норм1 * Rinv.бескнорм) / (R.норм1 * R.бескнорм) ) );
        Rprev = R;

        //Calc: R = 0.5f * (гамма * R + (1.0 / гамма) * Rinv.транспонированный);
        R *= гамма;
        Rinv.транспонируй;
        Rinv /= гамма;
        R += Rinv;
    ...
    // the case where A имеется a negative детерминанта.
...
// Generate the элементы m00,m01,m02... элементы
...
// Generate the opCall constructor taking a list of M*N значения
...
private ткст _gen_row_major_body(цел M, цел N)(ткст Vals)
{
    ...
    {
        возвр ~= "        ";
        for(цел столб=0; столб<N; ++столб,++знач)
        {
            возвр ~= "значения_["~ctfe_itoa(столб*M+ряд)~"] = "~Vals~"[" ~ ctfe_itoa(знач) ~ "];  ";
        }
        возвр ~= "\n";
    }
    return возвр;
...
private ткст _gen_outer_product_body(цел M, цел N)(ткст a, ткст b)
{
    ...
    {
        возвр ~= "        ";
        for (цел ряд=0; ряд<M; ++ряд,++знач)
        {
            возвр ~= "возвр.значения_["~ctfe_itoa(знач)~"] = ";
...
private ткст _gen_identity_matrix(цел M, цел N)(ткст storage_class, ткст имя)
{
    ткст возвр = storage_class ~" Матрица "~имя~" = {[cast(Т)";
    for(цел столб=0; столб<N; ++столб)
    {
        for (цел ряд=0; ряд<M; ++ряд)
        {
...
private ткст _gen_zero_matrix(цел M, цел N)(ткст storage_class, ткст имя)
{
    ткст возвр = storage_class ~" Матрица "~имя~" = {[cast(Т)";
    for(цел столб=0; столб<N; ++столб)
    {
...
debug (MatrixMain)
{
    private
    {
        ...
        void проверь_mat(ref Mat33 A, num_t[9] знач)
        {
            assert(A.m00 == знач[0]);
            assert(A.m01 == знач[1]);
            assert(A.m02 == знач[2]);
        ...
        void проверь_matf(ref Mat33 A, num_t[9] знач)
        {
            assert(абс(A.m00 - знач[0]) < 1e-4);
            assert(абс(A.m01 - знач[1]) < 1e-4);
            assert(абс(A.m02 - знач[2]) < 1e-4);
            assert(абс(A.m10 - знач[3]) < 1e-4);
            assert(абс(A.m11 - знач[4]) < 1e-4);
            assert(абс(A.m12 - знач[5]) < 1e-4);
            assert(абс(A.m20 - знач[6]) < 1e-4);
            assert(абс(A.m21 - знач[7]) < 1e-4);
            assert(абс(A.m22 - знач[8]) < 1e-4);
        }
        void проверь_mat(ref Mat22 A, num_t[4] знач)
        {
            assert(A.m00 == знач[0]);
            assert(A.m01 == знач[1]);
            assert(A.m10 == знач[2]);
        ...
        void проверь_mat(ref Mat23 A, num_t[6] знач)
        {
            assert(A.m00 == знач[0]);
            assert(A.m01 == знач[1]);
            assert(A.m02 == знач[2]);
        ...
        void проверь_mat(ref Mat32 A, num_t[6] знач)
        {
            assert(A.m00 == знач[0]);
            assert(A.m01 == знач[1]);
            assert(A.m10 == знач[2]);
        ...
        void проверь_vec(ref Vec2 x, num_t[2] знач)
        {
            assert(x[0] == знач[0]);
            assert(x[1] == знач[1]);
        }
        void проверь_vec(ref Vec3 x, num_t[3] знач)
        {
            assert(x[0] == знач[0]);
            assert(x[1] == знач[1]);
            assert(x[2] == знач[2]);
        ...
        void main()
        {
            ...
            проверь_mat(mat32, [9,8,
            7,6,
            5,4]);
            Mat23 mat23 = {[9,8, 7,6, 5,4]};//столб major
            проверь_mat(mat23, [9,7,5,
                                       8,6,4]);
            mat32.m10 = 3;
            проверь_mat(mat32, [9,8,
                                       3,6,
            ...
            mat32 = Mat32(1,2,
                          3,4,
                          5,6);
            проверь_mat(mat32, [1,2,
                                       3,4,
                                       5,6]);

            mat23 = Mat23([1,2,3,
                           4,5,6]);
            проверь_mat(mat23, [1,2,3,
                                       4,5,6]);

            mat32 = Mat32.столб_майор([1,2,3,4,5,6]);
            проверь_mat(mat32, [1,4, 2,5, 3,6]);

            mat23 = Mat23.столб_майор([1,2, 3,4, 5,6]);
            проверь_mat(mat23, [1,3,5,
                                       2,4,6]);

            auto mat33 = умножь_мат(mat32,mat23);
            проверь_mat(mat33, [9,19,29,
                                       12,26,40,
                                       15,33,51]);
            ...
            проверь_mat(mat22, [22,49, 28,64]);

            // opMulAssign
            mat22 = Mat22([1.,2,
                           5, 3]);
            mat32 *= mat22;
            проверь_mat(mat22, [1,2, 5,3]);
            проверь_mat(mat32, [21,14, 27,19, 33,24]);

            // opMul (скаляр)
            mat22 = 2 * mat22;
            проверь_mat(mat22, [2,4, 10,6]);

            // opDiv (скаляр)
            проверь_mat(mat22/2, [1,2, 5,3]);
            ...
            mat22 = Mat22([1,2, 5,3]);
            // opAdd
            проверь_mat(mat22+mat22, [2,4, 10,6]);
            проверь_mat(mat22, [1,2, 5,3]);
            проверь_mat(mat23 + mat23, [2,6,10,  4,8,12]);
            проверь_mat(mat23, [1,3,5,  2,4,6]);

            // opSub
            проверь_mat(mat22-Mat22(2,4,3,9), [-1,-2, 2,-6]);

            // Transpose / транспонированный
            mat22 = Mat22([1,2, 5,3]);
            проверь_mat(mat22.транспонированный, [1,5, 2,3]);
            проверь_mat(mat22,            [1,2, 5,3]);
            mat22.транспонируй();
            проверь_mat(mat22,            [1,5, 2,3]);

            mat23 = Mat23(1,2,3, 4,5,6);
            проверь_mat(mat23.транспонированный, [1,4, 2,5, 3,6]);
            проверь_mat(mat23,            [1,2,3, 4,5,6]);
            //mat23.транспонируй();  // compile time ошибка

            mat32 = Mat32(1,2, 3,4, 5,6);
            проверь_mat(mat32.транспонированный, [1,3,5, 2,4,6]);
            проверь_mat(mat32,            [1,2, 3,4, 5,6]);
            //mat32.транспонируй(); // compile-time ошибка

            mat33 = Mat33(1,2,3, 4,5,6, 7,8,9);
            проверь_mat(mat33.транспонированный, [1,4,7, 2,5,8, 3,6,9]);
            проверь_mat(mat33,            [1,2,3, 4,5,6, 7,8,9]);
            mat33.транспонируй();
            проверь_mat(mat33,            [1,4,7, 2,5,8, 3,6,9]);

            mat33.добавь_элт(2,1, 5.0);
            проверь_mat(mat33,            [1,4,7, 2,5,8, 3,11,9]);

            Mat33 mat33inv = mat33.инверсия();
            проверь_matf(mat33inv*3, [-4.3, 4.1,-0.3,
                                             0.6, -1.2, 0.6,
                                             0.7,  0.1,-0.3]);
            проверь_matf(mat33*mat33inv, [1.,0,0,  0,1,0, 0,0,1]);
            проверь_matf(mat33inv*mat33, [1.,0,0,  0,1,0, 0,0,1]);

            проверь_mat(Mat33.подобие, [1.,0,0,  0,1,0, 0,0,1]);
            проверь_mat(Mat33.ноль, [0,0,0,  0,0,0, 0,0,0]);

            Vec3 x = {[1,-3,7]};
            Vec2 Ax;
            mat23 = Mat23(1,2,3, 4,5,6);
            умножь_верни_мат_век(mat23, x, Ax);
            проверь_vec(Ax, [16,31]);

            умножь_верни_век_мат(Ax, mat23, x);
            проверь_vec(x, [140,187,234]);

            x = Век3д([-1, 2, -3]);
            Век3д y = {[3,2,1]};
            верни_внешн_продукт(x, y, mat33);
            проверь_mat(mat33, [-3, -2, -1,
                                       +6,  4,  2,
                                       -9, -6, -3]);
