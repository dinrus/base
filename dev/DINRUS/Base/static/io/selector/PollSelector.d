...
version (Posix)
{
    ...
    public class PollSelector: АбстрактныйСелектор
    {
         ...
         *
         * Параметры:
         * таймаут  = Timespan with the maximum amount времени that the
        ...
        public цел выбери(ИнтервалВремени таймаут)
        {
            ...
            debug (selector)
            Стдвыв.форматнс("--- PollSelector.выбери({0} мс): waiting on {1} handles",
                                          в_, _count);
            ...
            while (да)
            {
                ...
                if (_eventCount < 0)
                {
                    ...
                    debug (selector)
                    Стдвыв.выведи("--- Restarting poll() после being interrupted\n");
                }
                else
                {
        ...
        цел opApply(цел delegate(ref КлючВыбора) дг)
        {
            ...
            foreach (pfd; fds)
            {
                ...
                if(pfd.revents != 0)
                {
                    ...
                    auto ключ = (cast(ИВыбираемый.фукз)pfd.fd) in ключи;
                    if(ключ !is пусто)
                    {
                        КлючВыбора текущ = ключ.ключ;
    ...
    /**
     * Класс that holds the information that the PollSelector needs в_ deal
