...
{
/+
extern(C) static цел версияДинрус();

public struct Версия
{
	static бул ДИНРУС1 = false;
	static бул ДИНРУС2 = false;
	static бул НЕИЗВЕСТНА = false;
	static Версия opCall(){
	установи();
	static if (Версия.ДИНРУС1) { version = Dinrus1;}
		else { version = Dinrus1;}
	}
	проц установи()
		{
		if (версияДинрус == 1) { ДИНРУС1 = true;}
			else if (версияДинрус == 2) { ДИНРУС2 = true;}
			  else НЕИЗВЕСТНА = true;
		}
}		
    ...
    version(Dinrus1)
	{
        static assert(Версия.ДИНРУС1);
	}
	
	version(Dinrus2)
	{
        static assert(Версия.ДИНРУС2);
	}
...
enum ЦветКонсоли : цел
{
 ...
 extern(C)  проц устЦветКонсоли(ЦветКонсоли цвет);
...
version(Dinrus)
{
	...
	//else{
		alias цел LONG, BOOL, WINBOOL, WINT, INT, LONG32, INT32, RETCODE, ACL_INFORMATION_CLASS;
		alias уцел PINT, LPINT, LPLONG, PWINBOOL, LPWINBOOL, PBOOL, LPBOOL, PLONG, PLONG32, PINT32;
		alias LONG HRESULT, SCODE, LPARAM, LRESULT;

		alias бцел ЛКИД, DWORD, UINT, ULONG, FLONG, LCID, ULONG32, DWORD32, UINT32, LCTYPE, PROPID;
		alias убцел PULONG, PUINT, PLCID, LPUINT, PULONG32, PDWORD32, PUINT32;
		alias UINT WPARAM;
		alias DWORD   CALTYPE,CALID, COLORREF,TCOLORREF;
		...
		alias CHAR *LPSTR, PSTR, LPCSTR, PCSTR, LPCCH, LPCH;
		//alias LPSTR LPTCH, PTCH, PTSTR, LPTSTR, LPCTSTR;

		alias шим WCHAR, OLECHAR;
		alias шткст0 PWCHAR, LPWCH, PWCH, LPCWCH, LPWSTR, PWSTR, LPCWSTR, PCWSTR, LPOLESTR, LPCOLESTR;
...
*/
		...
		alias ббайт BYTE, BCHAR, UCHAR,UINT8;
		alias уббайт PUINT8;
		alias UCHAR *PUCHAR;
		...
		alias ук HANDLE, PVOID, LPVOID, LPCVOID, PVOID64, PCVOID, POINTER;
		
		alias HANDLE HINST, HGLOBAL, HLOCAL, HWND, HINSTANCE, HGDIOBJ, HACCEL, HBITMA, HBRUSH, HCOLORSPACE, HDC, HDBC, HGLRC, HDESK, HENHMETAFILE, HENV, HFILE, HFONT, HICON, HMENU, HMETAFILE, HPALETTE, HPEN, HRGN, HRSRC, HMONITOR, HSTR, HTASK, HWINSTA, HKEY, HKL, HBITMAP, HCONV, HCONVLIST, HDDEDATA, HDROP, HDWP, HRASCONN, HHOOK, HIMAGELIST, HMODULE, HCURSOR, HSTMT, HSZ, SC_HANDLE;
