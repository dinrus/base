...
проц монитор (проц delegate() начало, проц delegate(цел, цел) конец)
 ...
 проц вкл()
    {
        if (!нить_нужнаБлокировка()||stdrus.Нить.дайЧлоНитей() ==1)
        {
 ...
 бцел дайАтр( ук p )
     ...
     */
бцел устАтр( ук p, ПАтрБлока маска )
    {
         if (!p)
        {
            return 0;
     ...
     *  Результат вызова дайАтр устанавливается после установки указанного бита.
     */ 
     бцел удалиАтр( ук p, ПАтрБлока маска )
    {
        if (!p)
        {
            return 0;
     ...
     */
     ук празмести(т_мера размер, бцел биты = 0, т_мера *разм_размещ = null)
    {
        if (!размер)
        {
		  if(разм_размещ)
     ...
     */
     ук кразмести(т_мера размер, бцел биты = 0, т_мера *разм_размещ = null)
    {
        if (!размер)
        {
		    if(разм_размещ)
     ...
     */
     ук перемести(ук p, т_мера размер, бцел биты = 0, т_мера *разм_размещ = null)
    {
        synchronized (смЗамок)
        {
...
//debug скажинс("Экономия пространства выполнена");
    ...
    т_мера полная_уборка(ук верхСтэка)
    {
        ...
        нить_заморозьВсе();
		if (stdrus.Нить.дайЧлоНитей() > 0)stdrus.Нить.паузаВсем();
        ...
        нить_разморозьВсе();
        if (stdrus.Нить.дайЧлоНитей() > 0) stdrus.Нить.возобновиВсе();
/+
        debug(PROFILING)
        {
            stop = clock();
...
private	проц инвариант()
	...
	if (адрОсновы)
        {
            assert(адрОсновы + члостр * РАЗМЕР_СТРАНИЦЫ == адрВерха, "Первый ассёрт мнварианта Пула СМ");
            assert(члопереданных <= члостр, "Второй ассёрт мнварианта Пула СМ");
        }

        for (т_мера инд = 0; инд < члостр; инд++)
        {   Бины бин = cast(Бины)таблица_страниц[инд];

            assert(бин < Б_МАКС, "Третий ассёрт мнварианта Пула СМ");
        }
	}
