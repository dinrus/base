...
const бит СГЛАЖИВАНИЕ_ВЫКЛ = 0;
const бит СГЛАЖИВАНИЕ_ВКЛ  = 1;
...
проц ОбработайСобытие();
    ...
    цел function(СобытиеСДЛ*,цел,SDL_eventaction,бцел) сдлСобыПип;
    цел function(СобытиеСДЛ*) сдлСобПолл;
    цел function(СобытиеСДЛ*) сдлСобЖдать;
    цел function(СобытиеСДЛ*) сдлСобПуш;
    ...
    ДжойстикСДЛ* function(цел) сдлДжойстОткрой;
    цел function(цел) сдлДжойстОткрыт;
    цел function(ДжойстикСДЛ*) сдлИндексДжойста;
    цел function(ДжойстикСДЛ*) сдлЧлоОсейДжойста;
    цел function(ДжойстикСДЛ*) сдлЧлоШаровДжойста;
    цел function(ДжойстикСДЛ*) сдлЧлоШляпокДжойста;
    цел function(ДжойстикСДЛ*) сдлЧлоКнопокДжойста;
    проц function() сдлДжойстОбнови;
    цел function(цел) сдлСостСобДжойста;
    крат function(ДжойстикСДЛ*,цел) сдлДайОсьДжойста;
    ббайт function(ДжойстикСДЛ*,цел) сдлДайШляпкуДжойста;
    цел function(ДжойстикСДЛ*,цел,цел*,цел*) сдлДайШарДжойста;
    ббайт function(ДжойстикСДЛ*,цел) сдлДайКнопкуДжойста;
    проц function(ДжойстикСДЛ*)сдлЗакройДжойст;
    ...
    КурсорСДЛ* function(ббайт*,ббайт*,цел,цел,цел,цел) сдлСоздайКурсор;
    проц function(КурсорСДЛ*) сдлУстановиКурсор;
    КурсорСДЛ* function() сдлДайКурсор;
    проц function(КурсорСДЛ*) сдлОсвободиКурсор;
    цел function(цел) сдлПокажиКурсор;

    // МютексСДЛ.в
    МютексСДЛ* function() сдлСоздайМютекс;
    цел function(МютексСДЛ*) сдлМютексП;
    цел function(МютексСДЛ*) сдлМютексВ;
    проц function(МютексСДЛ*) сдлУдалиМютекс;
    СемСДЛ* function(бцел) сдлСоздайСемафор;
    проц function(СемСДЛ*) сдлУдалиСемафор;
    цел function(СемСДЛ*) сдлСемЖди;
    цел function(СемСДЛ*) сдлСемПробуйЖди;
    цел function(СемСДЛ*,бцел) сдлСемЖдиТаймаут;
    цел function(СемСДЛ*) сдлСемПост;
    бцел function(СемСДЛ*) сдлСемЗнач;
    КондСДЛ* function() сдлСоздайУслов;
    проц function(КондСДЛ*) сдлУдалиУслов;
    цел function(КондСДЛ*) сдлУсловСигнал;
    цел function(КондСДЛ*) сдлУсловВещан;
    цел function(КондСДЛ*,МютексСДЛ*) сдлУсловЖди;
    цел function(КондСДЛ*,МютексСДЛ*,бцел) сдлУсловЖдиТаймаут;

    цел сдлБлокируйМютекс(МютексСДЛ *mutex)
    {
        return сдлМютексП(mutex);
    }

    цел сдлРазблокируйМютекс(МютексСДЛ *mutex)
    {
    ...
    // ВерсияСДЛ.в
    ВерсияСДЛ* function() сдлЛинкВерсия;

    // SDL_syswm.в
    цел function(SDL_SysWMinfo*) сдлДайИнфОбОк;

    // SDL_thread.в
    НитьСДЛ* function(цел (*fm)(ук), ук) сдлТяниНить;
    бцел function() сдлИДНити;
    бцел function(НитьСДЛ*) сдлДайИДНити;
    проц function(НитьСДЛ*,цел*) сдлЖдиНить;
    проц function(НитьСДЛ*) сдлРвиНить;
    ...
    ОверлейСДЛ* function(цел,цел,бцел,ПоверхностьСДЛ*) сдлСоздайЮВНакладку;
    цел function(ОверлейСДЛ*) сдлБлокируйЮВНакладку;
    проц function(ОверлейСДЛ*) сдлРазблокируйЮВНакладку;
    цел function(ОверлейСДЛ*,ПрямоугСДЛ*) сдлПокажиЮВНакладку;
    проц function(ОверлейСДЛ*) сдлОсвободиЮВНакладку;
...
enum : бцел
{
    ...
    SDL_INIT_NOPARACHUTE = 0x00100000, // Don't catch fatal сигналы
...
struct SDL_AudioSpec
{
 ...
 // SDL_audio.в константы
enum : бкрат
{
    AUDIO_U8           = 0x0008,// Unsigned 8-бит samples
    AUDIO_S8           = 0x8008,// Signed 8-бит samples
    AUDIO_U16LSB       = 0x0010,// Unsigned 16-бит samples
    AUDIO_S16LSB       = 0x8010,// Signed 16-бит samples
...
extern(C) typedef цел function(СобытиеСДЛ *событие) сдлФильтрСобытий;
...
enum
{
    /* The keyboard syms have been cleverly chosen to мап to ASCII */
    SDLK_UNKNOWN        = 0,
    SDLK_FIRST      = 0,
    SDLK_BACKSPACE      = 8,
    SDLK_TAB        = 9,
...
enum : ббайт
{
    ...
    СДЛ_КНОП_Х1           = 6,
    СДЛ_КНОП_Х2           = 7,
    СДЛ_КНОП_ЛМАСКА        = 1 << (СДЛ_ЛЕВ_КНОП-1),
    СДЛ_КНОП_СМАСКА        = 1 << (СДЛ_СРЕДН_КНОП-1),
    СДЛ_КНОП_ПМАСКА        = 1 << (СДЛ_ПРАВ_КНОП-1),
    СДЛ_КНОП_Х1МАСКА       = 1 << (СДЛ_КНОП_Х1-1),
    СДЛ_КНОП_Х2МАСКА       = 1 << (СДЛ_КНОП_Х2-1),
}

ббайт СДЛ_КНОП(ббайт x)
{
...
struct КондСДЛ {// Generic Cond structure ( was empty too!!!)
    МютексСДЛ* замок;
    цел ждут;
    цел сигналы;
    СемСДЛ* жди_сем;
...
struct ЧЗоперации
{
    extern(C)
    {
        цел function(ЧЗоперации *контекст, цел смещение, цел whence) seek;
        цел function(ЧЗоперации *контекст, ук указ, цел размер, цел максчло) read;
        цел function(ЧЗоперации *контекст, ук указ, цел размер, цел чло) write;
        цел function(ЧЗоперации *контекст) close;
    }
...
цел сдлЧЗПерейди(ЧЗоперации *контекст, цел смещение, цел whence)
{
    return контекст.seek(контекст, смещение, whence);
}
...
проц версияСДЛ(ВерсияСДЛ *X)
{
    X.майор = SDL_MAJOR_VERSION;
    X.минор = SDL_MINOR_VERSION;
    X.патч = SDL_PATCHLEVEL;
}

бцел номверсииСДЛ(ббайт майор, ббайт минор, ббайт патч)
{
    return (майор * 1000 + минор * 100 + патч);
}

const бцел SDL_COMPILEDVERSION =  SDL_MAJOR_VERSION * 1000 +
                                  SDL_MINOR_VERSION * 100 + SDL_PATCHLEVEL;

бул SDL_VERSION_ATLEAST(ббайт майор, ббайт минор, ббайт патч)
{
    return cast(бул)(SDL_COMPILEDVERSION >= номверсииСДЛ(майор,минор,патч));
}
...
version(Windows)
{
    ...
    struct SDL_SysWMmsg
    {
        // this is named 'version' in SDL_syswm.в, but since version is a keyword,
        // 'ver' will have to do
        ВерсияСДЛ ver;
        HWND hwnd;
        UINT msg;
...
alias ук ИДТаймераСдл;
alias ИДТаймераСдл ИДТаймера;
// SDL_video.в
...
{
    return cast(бул)(поверхность.смещение ||
