...
public:
    ...
    */
    ...
    бул проверь()
    {
        ...
        for (i=0; i<размер(); ++i)
        {
            if (((j=лев(i))<размер()) && интерфейс_.больше(запись(i), запись(j)))
            {
                ошибка("Нарушение условий для Кучи");
                ok=false;
            }
            if (((j=прав(i))<размер()) && интерфейс_.больше(запись(i), запись(j)))
            {
                ошибка("Нарушение условий для Кучи");
                ok=false;
            }
...
private:
    ...
    проц upheap(бцел _idx)
    {
        ...
        while ((_idx>0) &&
                интерфейс_.меньше(h, запись(parentIdx=родитель(_idx))))
        {
    ...
    /// Get родитель's index
    бцел родитель(бцел _i)
    {
        return (_i-1)>>1;
    }
    /// Get лев child's index
    бцел лев(бцел _i)
    {
        return (_i<<1)+1;
    }
    /// Get прав child's index
    бцел прав(бцел _i)
    {
        return (_i<<1)+2;
    }
