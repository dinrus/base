...
/**
 * Interface defining an object that accesses значения by ключ.
 */
interface СКлючом(K, V) : Ключник!(K, V), ЧистящийКлючи!(K, V)
{
     ...
     * Returns this.
     *
     * был_Удалён is установи to true if the элемент existed и was removed.
     */
    СКлючом!(K, V) удалиПо(K ключ, ref бул был_Удалён);

    /**
     * доступ a значение based on the ключ
     */
    V opIndex(K ключ);
     ...
     *
     * Use this to вставь a ключ/значение пара into the collection.
     *
