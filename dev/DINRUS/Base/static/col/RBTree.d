...
struct КЧУзел(З)
{
    ...
    Узел dup(Узел delegate(З знач) разм)
    {
        //
 ...
 */
struct КЧДерево(З, alias функСравнить, alias обновлФункц, alias Разместитель=ДефолтныйРазместитель, бул допускатьДубликаты=false, бул обновлять_ли=true)
{
    /**
     * Convenience alias
     */
    ...
    бул добавь(З знач)
    {
        ...
        if(конец.лево is пусто)
            конец.лево = добавленный = размести(знач);
        else
        {
            Узел новРодитель = конец.лево;
            while(true)
            {
                цел значсравн = функСравнить(новРодитель.значение, знач);
                if(значсравн == 0)
                {
                    //
                    ...
                    else
                    {
                        static if(обновлять_ли)
                            обновлФункц(новРодитель.значение, знач);
                        return false;
                    }
                }
                if(значсравн < 0)
                {
                    ...
                    if(nxt is пусто)
                    {
                        ...
                        //
                        новРодитель.право = добавленный = размести(знач);
                        break;
                    }
                    else
                        новРодитель = nxt;
                ...
                else
                {
                    ...
                    if(nxt is пусто)
                    {
                        ...
                        //
                        новРодитель.лево = добавленный = размести(знач);
                        break;
                    }
                    else
                        новРодитель = nxt;
    ...
    Узел найди(З знач)
    {
        ...
        {
            //
            // найди the лево-most знач, this allows the pointer to traverse
            // through all the знач's.
            //
            Узел тек = конец;
            Узел n = конец.лево;
            while(n !is пусто)
            {
                цел резсравн = функСравнить(n.значение, знач);
                if(резсравн < 0)
                {
                    n = n.право;
                }
        ...
        else
        {
            Узел n = конец.лево;
            цел резсравн;
            while(n !is пусто && (резсравн = функСравнить(n.значение, знач)) != 0)
            {
                if(резсравн < 0)
                    n = n.право;
                else
    ...
    version(КЧПроверять)
    {
         ...
         * It does not печать значения, only the tree structure и цвет of узелs.
         */
    ...
    {
        /**
         * счёт all the times знач appears in the collection.
         *
         * Runs in O(m * lg(n)) where m is the number of знач экземпляры in the
        ...
        бцел считайВсе(З знач)
        {
            Узел n = найди(знач);
            бцел возврзнач = 0;
            while(n !is конец && функСравнить(n.значение, знач) == 0)
            {
                возврзнач++;
                n = n.следщ;
            }
        ...
        /**
         * удали all the узелs that сверь знач
         *
         * Runs in O(m * lg(n)) where m is the number of знач экземпляры in the
         * collection, и n is the счёт of the collection.
         */
...
{
    alias КЧДерево!(З, функСравнить, функСравнить, Разместитель, false, false) КЧДеревоБезОбнова;
}
/**
...
{
    alias КЧДерево!(З, функСравнить, функСравнить, Разместитель, true, false) КЧДеревоДуб;
}
