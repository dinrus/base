...
class ДеревоНабор(З, alias ШаблРеализац = КЧДеревоБезОбнова, alias функцСравнения = ДефСравнить) : Набор!(З)
{
    ...
    /**
     * Iterate through элементы of the ДеревоНабор, specifying which ones to
     * удали.
     *
     * Use like this:
     * -------------
     * // удали all odd элементы
     * foreach(ref чистить_ли, знач; &treeSet.очистить)
     * {
     *   чистить_ли = ((знач % 1) == 1);
     * }
     * -------------
    ...
    /**
     * Removes the элемент that имеется the значение знач.  Returns true if the значение
     * was present и was removed.
     *
     * Runs in O(lg(n)) time.
     */
    ДеревоНабор удали(З знач)
    {
     ...
     * Runs in O(lg(n)) time.
     */
    ДеревоНабор удали(З знач, ref бул удалён_ли)
    {
        курсор обх = найди(знач);
        if(обх == конец)
        {
     ...
     * Runs in O(lg(n)) time.
     */
    ДеревоНабор добавь(З знач, ref бул был_добавлен)
    {
    ...
    /**
     * Adds all the значения from enumerator to the collection.
     *
    ...
    /**
     * Return a дубликат treeset containing all the элементы in this tree
     * установи.
     */
    ДеревоНабор dup()
    {
    ...
    /**
     * Remove all the элементы that сверь in the поднабор
     */
    ДеревоНабор удали(Обходчик!(З) поднабор)
    {
        foreach(знач; поднабор)
        удали(знач);
        return this;
    }

    /**
     * Remove all the элементы that сверь in the поднабор.  Sets чло_Удалённых to
     * number of элементы removed.
     *
     * returns this.
     */
    ...
    /**
     * Remove all the элементы that do NOT сверь in the поднабор.
     *
     * returns this.
     */
    ...
    /**
     * Remove all the элементы that do NOT сверь in the поднабор.  Sets
     * чло_Удалённых to number of элементы removed.
     *
     * returns this.
     */
    ...
    /**
     * Сравни this установи with another установи.  Returns true if both sets have the
     * same длина и every элемент in one установи exists in the other установи.
     *
     * If o is пусто or not a Набор, return 0.
     */
        ...
        if(o !is пусто)
        {
            ...
            if(s !is пусто && s.length == длина)
            {
                ...
                if(ts !is пусто)
                {
                    ...
                    //
                    // since we know treesets are sorted, сравни элементы
