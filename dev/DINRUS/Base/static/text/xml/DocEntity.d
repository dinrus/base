...
/******************************************************************************
        ...
        Преобразует образцы сущности XML в нормальные символы.
...
T[] изСущности (T) (T[] ист, T[] приёмн = пусто)
{
        ...
        if ((дельта = Util.индексУ (s, '&', длин)) < длин)
           {
           ...
           do {
              ...
              s += токен, длин -= токен;
              } while ((дельта = Util.индексУ (s, '&', длин)) < длин);
...
/******************************************************************************
        ...
        Этому варианту не требуется interim workspace, и instead
...
проц изСущности (T) (T[] ист, проц delegate(T[]) излей)
{
        ...
        // проверим, что в первом
        if ((дельта = Util.индексУ (s, '&', длин)) < длин)
           {
           // скопируем сегменты, чанк за раз
           do {
              излей (s [0 .. дельта]);
              длин -= дельта;
              s += дельта;

              // транслируем сущность
              auto токен = 0;
...
/******************************************************************************
        ...
        Преобразует зарезервированные символы в сущности. Например: " => &quot; 

        Возвращается либо срез предоставленного буфера вывода, либо 
        оригинальный контент, в зависимости от того, были ли там зарезервированные		
        символы. Буфер вывода должен быть в состоянии вместить  
        преобразованный вывод, либо  он размещается из кучи. 
...
/******************************************************************************
        ...
        Convert зарезервированные символы в_ entities. For example: " => &quot; 
