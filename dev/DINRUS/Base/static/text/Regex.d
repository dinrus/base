/*******************************************************************************
    ...
    См. <a href="http://en.wikipedia.org/wiki/Regular_Выражение">Статью о регулярных выражениях в Wikpedia</a> об общих деталях, касающихся регулярных выражений.
    ...
    Преимущество этого метода в его производительности, недостаток - в неспособности реализовать некоторые средства,
    которые есть у Perl-подобных регулярных выражений (напр. задний-references).
    См. <a href="http://swtch.com/~rsc/regexp/regexp1.html">&quot;Сверка Регулярных Выражений Может Быть Простой И Быстрой&quot;</a>
    с подробностями о различиях.

    The время for совпадают a regular Выражение against an ввод ткст of length N is in O(M*N), where M depends on the
    число of совпадают brackets и the complexity of the Выражение. That is, M is constant wrt. the ввод
    и therefore совпадают is a linear-время process.

    The syntax of a regular Выражениеs is as follows.
    <i>X</i> и <i>Y</i> stand for an arbitrary regular Выражение.
    ...
    $(TR $(TD X{n,m}) $(TD at least n, at most m экземпляры of X.<br>If n is missing, it's установи в_ 0.<br>If m is missing, it is установи в_ infinity.) )
...
private class Список(T)
{
    ...
    Список opCatAssign(T знач)
    {
        if ( хвост is пусто )
            голова = хвост = new Элемент(знач);
        else
        {
            хвост.следщ = new Элемент(знач);
    ...
    Список вставьПосле(T w, T знач)
    {
        foreach ( e; &this.элементы )
        {
            if ( e.значение is w )
                return вставьПосле(e, знач);
        }
        return пусто;
    }

    Список вставьПосле(Элемент e, T знач)
    {
        auto врем = new Элемент(знач);
        врем.предш = e;
        врем.следщ = e.следщ;
        e.следщ.предш = врем;
        e.следщ = врем;
    ...
    Список суньФронт(T знач)
    {
        if ( голова is пусто )
            голова = хвост = new Элемент(знач);
        else
        {
            голова.предш = new Элемент(знач);
    ...
    Список вставьПеред(T w, T знач)
    {
        foreach ( e; &this.элементы )
        {
            if ( e.значение is w )
                return вставьПеред(e, знач);
        }
        return пусто;
    }

    Список вставьПеред(Элемент e, T знач)
    {
        auto врем = new Элемент(знач);
        врем.предш = e.предш;
        врем.следщ = e;
...
/* *****************************************************************************
    Стэк, основанный на динамическом Массиве
*******************************************************************************/
...
/* ************************************************************************************************
    Контейнер Набор, основанный на ассоциативном Массиве
**************************************************************************************************/
private struct Набор(T)
{
    ...
    бул содержит(T знач)
    {
        return (знач in данные) !is пусто;
    }
...
struct ДиапазонСимволов(т_сим)
{
    ...
    /* ********************************************************************************************
        Сравнивает диапазоны, соответственно из началам.
    **********************************************************************************************/
...
/* ************************************************************************************************
    Представляет класс символов, как использован в регулярных выражениях (напр. [0-9a-z], и т.д.)
**************************************************************************************************/
...
Louter:
    ...
    **********************************************************************************************/
    проц отрицай()
    {
        ...
        // первый часть touches лево boundary of значение range
        ...
        // последний часть does not touch право boundary
        if ( старт != т_сим.max )
...
private struct Предикат(т_сим)
{
    ...
    // http://d.puremagic.com/issues/show_bug.cgi?ид=2632 --- don't помести it задний
    ...
    проц компилируй()
    {
        ...
        {
            режим = тип==Тип.используй ? ПРежимСверки.single_char : ПРежимСверки.single_char_l;
        ...
        режим = тип==Тип.используй ? ПРежимСверки.bitmap : ПРежимСверки.bitmap_l;
        return;
...
LnoBitmap:
        /*
                ...
                режим = тип==Тип.используй ? ПРежимСверки.string_search : ПРежимСверки.string_search_l;
                return;
        */
Lgeneric:
        data_str = cast(т_сим[])ввод.части;
        режим = тип==Тип.используй ? ПРежимСверки.генерный : ПРежимСверки.generic_l;
    }
...
/* ************************************************************************************************
    Priority classes использован в_ linearize priorities после non-linear переход_в creation.
**************************************************************************************************/
...
private class TNFATransition(т_сим)
{
    ...
    /******************************************************************************
        ...
        the one with highest priority. If the highest priority переход_в из_ a
        состояние isn't an epsilon переход_в, нет is returned.
...
{
    alias TNFATransition!(т_сим)   trans_t;
    alias TNFAFragment!(т_сим)     т_фраг;
    alias TNFAState!(т_сим)        state_t;
    ...
    **********************************************************************************************/
    проц разбор(бул без_якоря)
    {
        Список!(т_фраг)       фраги       = new Список!(т_фраг);
        Стэк!(Оператор)    opСтэк;
        Стэк!(бцел)        тэгСтэк;
        ...
        ******************************************************************************************/
        бул выполни(Оператор nрасш_op, бул explicit_operator=да)
        {
            ...
            case Act.poc:
                switch ( opСтэк.верх )
                {
                case Оператор.concat:
                    constructConcat(фраги);
                    break;
                case Оператор.altern:
                    constructAltern(фраги);
                    break;
                case Оператор.zero_one_g:
                    constructZeroOne(фраги, PriorityClass.greedy);
                    break;
                case Оператор.zero_one_ng:
                    constructZeroOne(фраги, PriorityClass.reluctant);
                    break;
                case Оператор.zero_one_xr:
                    constructZeroOne(фраги, PriorityClass.extraReluctant);
                    break;
                case Оператор.zero_more_g:
                    constructZeroMore(фраги, PriorityClass.greedy);
                    break;
                case Оператор.zero_more_ng:
                    constructZeroMore(фраги, PriorityClass.reluctant);
                    break;
                case Оператор.zero_more_xr:
                    constructZeroMore(фраги, PriorityClass.extraReluctant);
                    break;
                case Оператор.one_more_g:
                    constructOneMore(фраги, PriorityClass.greedy);
                    break;
                case Оператор.one_more_ng:
                    constructOneMore(фраги, PriorityClass.reluctant);
                    break;
                case Оператор.one_more_xr:
                    constructOneMore(фраги, PriorityClass.extraReluctant);
                    break;
                case Оператор.occur_g:
                    Пара!(бцел) occur = occurСтэк.вынь;
                    constructOccur(фраги, occur.a, occur.b, PriorityClass.greedy);
                    break;
                case Оператор.occur_ng:
                    Пара!(бцел) occur = occurСтэк.вынь;
                    constructOccur(фраги, occur.a, occur.b, PriorityClass.reluctant);
                    break;
                default:
            ...
            case Act.poa:
                ...
                break;
            case Act.pca:
                if ( opСтэк.верх == Оператор.open_par )
                {
                    if ( тэгСтэк.пустой )
                        throw new ИсклРегВыр(Формат.преобразуй("Отсутствует открывающая скобка у сим {} \"{}\"", cursor, Utf.вТкст(образец[cursor..$])));
                    constructBracket(фраги, тэгСтэк.верх);
                    тэгСтэк.вынь;
                }
                else
                {
                    assert(opСтэк.верх == Оператор.open_par_nm);
                    constructBracket(фраги);
                }
                opСтэк.вынь;
                break;
            case Act.don:
        ...
        // добавь implicit extra reluctant .* (with . == any_char) at the beginning for без_якоря совпадает
        // и совпадают bracket for total сверь группа
        if ( без_якоря )
        {
            фраги ~= constructChars(т_кс.any_char, predicate_t.Тип.используй);
            выполни(Оператор.zero_more_xr, нет);
            выполни(Оператор.concat, нет);
            выполни(Оператор.open_par, нет);
        }
        ...
        while ( !конецОбразца )
        {
            ...
            case '(':
                ...
                {
                    читайОбразец;
                    выполни(свопMatchingBracketSyntax?Оператор.open_par:Оператор.open_par_nm);
                }
                else
                    выполни(свопMatchingBracketSyntax?Оператор.open_par_nm:Оператор.open_par);
                break;
            case ')':
                выполни(Оператор.close_par);
                break;
            case '?':
                if ( просмотриОбразец == '?' )
                {
                    читайОбразец;
                    выполни(Оператор.zero_one_ng);
                }
                else
                    выполни(Оператор.zero_one_g);
                break;
            case '*':
                if ( просмотриОбразец == '?' )
                {
                    читайОбразец;
                    выполни(Оператор.zero_more_ng);
                }
                else
                    выполни(Оператор.zero_more_g);
                break;
            case '+':
                if ( просмотриОбразец == '?' )
                {
                    читайОбразец;
                    выполни(Оператор.one_more_ng);
                }
                else
                    выполни(Оператор.one_more_g);
                break;
            case '{':
                Пара!(бцел) occur;
                ...
                {
                    читайОбразец;
                    выполни(Оператор.occur_ng);
                }
                else
                    выполни(Оператор.occur_g);
                break;
            case '[':
                if ( implicit_concat )
                    выполни(Оператор.concat, нет);
                implicit_concat = да;
                фраги ~= constructCharClass(pred_type);
                break;
            case '.':
                if ( implicit_concat )
                    выполни(Оператор.concat, нет);
                implicit_concat = да;
                фраги ~= constructChars(т_кс.dot_oper, pred_type);
                break;
            case '$':
                if ( implicit_concat )
                    выполни(Оператор.concat, нет);
                ...
                фраги ~= constructChars(т_кс.начкон_стр, predicate_t.Тип.lookahead);
                break;
            case '^':
                if ( implicit_concat )
                    выполни(Оператор.concat, нет);
                ...
                фраги ~= constructChars(т_кс.начкон_стр, predicate_t.Тип.lookbehind);
                break;
            case '>':
                c = читайОбразец;
            ...
            case '\\':
                ...
                {
                case 't':
                    фраги ~= constructSingleChar('\t', pred_type);
                    break;
                case 'n':
                    фраги ~= constructSingleChar('\n', pred_type);
                    break;
                case 'r':
                    фраги ~= constructSingleChar('\r', pred_type);
                    break;
                case 'w':   // alphanumeric и _
                    фраги ~= constructChars(т_кс.alphanum_, pred_type);
                    break;
                case 'W':   // non-(alphanum и _)
                    auto cc = т_кс(т_кс.alphanum_);
                    cc.отрицай;
                    фраги ~= constructChars(cc, pred_type);
                    break;
                case 's':   // пробел
                    фраги ~= constructChars(т_кс.пробел, pred_type);
                    break;
                case 'S':   // non-пробел
                    ...
                    break;
                case 'd':   // цифра
                    фраги ~= constructChars(т_кс.цифра, pred_type);
                    break;
                case 'D':   // non-цифра
                ...
                case 'b':   // either конец of word
                    ...
                    выполни(Оператор.open_par_nm);

                    фраги ~= constructChars(cc, predicate_t.Тип.lookbehind);
                    выполни(Оператор.concat, нет);
                    фраги ~= constructChars(т_кс.пробел, predicate_t.Тип.lookahead);
                    выполни(Оператор.altern, нет);
                    фраги ~= constructChars(т_кс.пробел, predicate_t.Тип.lookbehind);
                    выполни(Оператор.concat, нет);
                    фраги ~= constructChars(cc, predicate_t.Тип.lookahead);

                    выполни(Оператор.close_par, нет);
                    break;
...
//                            throw new ИсклРегВыр(Формат.преобразуй("Unknown escape sequence \\{}", c));
        ...
        // добавь implicit reluctant .* (with . == any_char) at the конец for без_якоря совпадает
        if ( без_якоря )
        {
            выполни(Оператор.close_par, нет);
            if ( implicit_concat )
                выполни(Оператор.concat, нет);
            фраги ~= constructChars(т_кс.any_char, predicate_t.Тип.используй);
            выполни(Оператор.zero_more_ng, нет);
        }

        // пустой operator стэк
        while ( !выполни(Оператор.eos) ) {}
...
private:
    ...
    т_фраг constructSingleChar(т_сим c, predicate_t.Тип тип)
    {
        ...
        т_фраг frag = new т_фраг;
        frag.exit_state ~= trans;
        frag.записи    ~= trans;
        return frag;
    }

    т_фраг constructChars(т_ткст симвы, predicate_t.Тип тип)
    {
        т_кс cc;
    ...
    т_фраг constructChars(т_кс charclass, predicate_t.Тип тип)
    {
    ...
    т_фраг constructCharClass(predicate_t.Тип тип)
    {
        ...
        for ( ; !конецОбразца && просмотриОбразец != ']'; )
        {
            ...
            case '-':
                ...
                if ( !have_range_start )
                    throw new ИсклРегВыр("Missing range старт for '-' operator после \""~Utf.вТкст(образец)~"\"");
                else if ( конецОбразца || просмотриОбразец == ']' )
                    throw new ИсклРегВыр("Missing range конец for '-' operator после \""~Utf.вТкст(образец)~"\"");
                else
                {
                    c = читайОбразец;
            ...
            case '\\':
                if ( конецОбразца )
                    throw new ИсклРегВыр("неожиданный конец of ткст после \""~Utf.вТкст(образец)~"\"");
                c = читайОбразец;
                switch ( c )
                {
                case 't':
    ...
    проц constructBracket(Список!(т_фраг) фраги, бцел тэг=0)
    {
        debug(tnfa) Стдвыв.форматнс("constructBracket");

        state_t Запись = добавьСостояние,
        exit = добавьСостояние;
        фраги.хвост.значение.setEntry(Запись);
        фраги.хвост.значение.setExit(exit);
    ...
    проц constructOneMore(Список!(т_фраг) фраги, PriorityClass prioClass)
    {
        debug(tnfa) Стдвыв.форматнс("constructOneMore");

        if ( фраги.пустой )
            throw new ИсклРегВыр("слишком мало аргументов для + у \""~Utf.вТкст(образец[cursor..$])~"\"");
        ...
        фраги.хвост.значение.setExit(s);
        s.transitions ~= повтори;
        s.transitions ~= cont;

        т_фраг frag = new т_фраг;
        frag.записи ~= фраги.хвост.значение.записи;
        frag.entry_state ~= фраги.хвост.значение.entry_state;
    ...
    проц constructZeroMore(Список!(т_фраг) фраги, PriorityClass prioClass)
    {
    ...
    проц constructZeroOne(Список!(т_фраг) фраги, PriorityClass prioClass)
    {
        ...
        state_t s = добавьСостояние;
        фраги.хвост.значение.setEntry(s);
        use.мишень = s;

        т_фраг frag = new т_фраг;
        frag.записи ~= use;
        frag.записи ~= пропусти;
        frag.exits ~= фраги.хвост.значение.exits;
        frag.exit_state ~= фраги.хвост.значение.exit_state;
        frag.exit_state ~= пропусти;
        фраги.вынь;
        фраги ~= frag;
    }

    проц constructOccur(Список!(т_фраг) фраги, бцел minOccur, бцел maxOccur, PriorityClass prioClass)
    {
        debug(tnfa) Стдвыв.форматнс("constructOccur {},{}", minOccur, maxOccur);

        if ( фраги.пустой )
            throw new ИсклРегВыр("слишком мало аргументов для {x,y} у \""~Utf.вТкст(образец[cursor..$])~"\"");

        state_t s;
        т_фраг  total = new т_фраг,
        предш;

        for ( цел i = 0; i < minOccur; ++i )
        {
            т_фраг f = клонируй(фраги.хвост.значение);
            if ( предш !is пусто )
            {
                s = добавьСостояние;
                предш.setExit(s);
        ...
        for ( цел i = minOccur; i < maxOccur; ++i )
        {
            т_фраг f;
            if ( i < maxOccur-1 )
                f = клонируй(фраги.хвост.значение);
            else
                f = фраги.хвост.значение;
            trans_t t = добавьПроход;
            t.predicate.тип = predicate_t.Тип.epsilon;
            f.записи ~= t;
            f.exit_state ~= t;
    ...
    проц constructAltern(Список!(т_фраг) фраги)
    {
        debug(tnfa) Стдвыв.форматнс("constructAltern");

        if ( фраги.пустой || фраги.голова is фраги.хвост )
            throw new ИсклРегВыр("too few аргументы for | at \""~Utf.вТкст(образец[cursor..$])~"\"");

        т_фраг  frag = new т_фраг,
        f1 = фраги.хвост.значение,
        f2 = фраги.хвост.предш.значение;
    ...
    проц constructConcat(Список!(т_фраг) фраги)
    {
        ...
        т_фраг  f1 = фраги.хвост.значение,
                f2 = фраги.хвост.предш.значение;
...
private class TDFA(т_сим)
{
    ...
    **********************************************************************************************/
    class Состояние
    {
        ...
        проц оптимизируй()
        {
            // совмести transitions with equal targets (same состояние индекс и equal commands)
    ...
    **********************************************************************************************/
    this(TNFA!(т_сим) tnfa)
    {
        ...
        foreach ( состояние; states )
        {
            ...
            renumberCommand(cmd);
            // сделай sure поз-commands are executed после reorder-commands и
        ...
        debug(TangoRegex)
        {
            foreach ( ref знач; registers )
            {
                if ( (знач in regNums) !is пусто )
                    знач = regNums[знач];
            }
        }
        ...
        // TODO: метка states that can покинь the finishing поднабор of DFA states or use a greedy переход_в
    ...
    **********************************************************************************************/
    class StateElement
    {
        ...
        ткст вТкст()
        {
            ...
            бул первый = да;
            foreach ( ключ, знач; tags )
            {
                стр ~= Формат.преобразуй("{}m({},{})", первый?"":",", ключ, знач);
                первый = нет;
            }
            стр ~= "}";
            return стр;
    ...
    **********************************************************************************************/
    predicate_t[] disjointPredicates(SubsetState состояние)
    {
        ...
        foreach ( t; состояние )
        {
            ...
            // сн. lb-closure will not расширь for transitions with a superset of the lb-predicate
...
Lmerge:
        foreach ( r; disjoint )
        {
            ...
            {
                // we can совмести r преобр_в the текущ predicate if
    ...
    SubsetState lookbehindClosure(SubsetState из_, predicate_t пред)
    {
        ...
        while ( !стэк.пустой )
        {
            ...
            foreach ( t; se.nfa_state.transitions )
            {
                ...
                if ( врем !is пусто )
                {
                    // if higher prio (smaller значение) есть_ли, do not use this переход_в
...
//                         debug(tdfa) Стдвыв.форматнс("maxPrio({}) {} beats {}", t.мишень.индекс, new_maxPri, врем.maxPriority);
                ...
                if ( t.тэг > 0 )
                {
                    foreach ( ключ, знач; se.tags )
                    new_se.tags[ключ] = знач;
            ...
            if ( !seenLastUsedIndex )
            {
                foreach ( e; рез.elms )
                {
                    foreach ( ref i; e.tags )
                    {
                        ...
                        // после it имеется been detected в_ be a newly использован индекс
                        if ( i == firstFreeIndex )
                            i = -firstFreeIndex;
                    }
                }
...
Louter:
    ...
    **********************************************************************************************/
    проц generateInitializers(SubsetState старт)
    {
        ...
        {
            foreach ( ключ, знач; nds.tags )
            cmds[ключ] = знач;
        }

        foreach ( ключ, знач; cmds )
        {
            Command cmd;
            cmd.приёмн = registerFromTagIndex(ключ,знач);
            cmd.ист = CURRENT_POSITION_REGISTER;
            initializer ~= cmd;
        }
    }
    ...
    проц generateFinishers(SubsetState r)
    {
        ...
        foreach ( se; sorted_elms )
        {
            ...
            if ( se.nfa_state.прими )
            {
                ...
                // nfa состояние в этом наборе with the highest incoming priority (последний in
                ...
                // The NFA->DFA transformation will most likely расширь the automata
    ...
    **********************************************************************************************/
    проц minimizeDFA()
    {
        ...
        scope рознь = new DiffTable(states.length);
        бул new_diff = да;
...
Linner:
                foreach ( j, b; states[i+1 .. $] )
                {
                    ...
                    if ( a.прими ) // b accepts too
                    {
                        ...
                        foreach ( ключ, cmd; a.finishers )
                        {
                            if ( cmd != b.finishers[ключ] )
                            {
                                рознь.установи(i, j+i+1);
                                new_diff = да;
                                continue Linner;
...
Ltrans:
                    foreach ( ta; a.transitions )
                    {
                        foreach ( tb; b.transitions )
                        {
                            ...
                            {
                                if ( рознь(ta.мишень.индекс, tb.мишень.индекс) )
                                {
                                    рознь.установи(i, j+i+1);
                                    new_diff = да;
                                    continue Linner;
                                ...
                                foreach ( ключ, cmd; ta.commands )
                                {
                                    if ( cmd != tb.commands[ключ] )
                                    {
                                        рознь.установи(i, j+i+1);
                                        new_diff = да;
                                        continue Linner;
...
/**************************************************************************************************
    Regular Выражение compiler и interpreter.
**************************************************************************************************/
class RegExpT(т_сим)
{
    ...
    **********************************************************************************************/
    ...
    /** описано ранее */
    this(т_сим[] образец, бул свопMBS, бул без_якоря, бул printNFA=нет)
    {
        pattern_ = образец;
    ...
    **********************************************************************************************/
    ...
    /**********************************************************************************************
        Pick up where последний тест(ввод) or тест() лево off, и ищи again.
        Возвращает: нет for no сверь, да for сверь
    **********************************************************************************************/
    бул тест()
    {
...
Ltrans_loop:
                    ...
                    tp < tp_end; ++tp )
            {
                ...
                {
                // single сим
                case predicate_t.ПРежимСверки.single_char:
                    ...
                    goto Lconsume;
                case predicate_t.ПРежимСверки.single_char_l:
                    debug(TangoRegex) Стдвыв.форматнс("single сим 0x{:x} == 0x{:x}", cast(цел)c, cast(цел)t.predicate.data_chr);
                    if ( c != t.predicate.data_chr )
                        continue Ltrans_loop;
                    goto Lno_consume;
                ...
                // ткст ищи
                case predicate_t.ПРежимСверки.string_search:
                    ...
                    if ( индексУ(t.predicate.data_str.ptr, c, t.predicate.data_str.length) >= t.predicate.data_str.length )
                        continue Ltrans_loop;
                    goto Lconsume;
                case predicate_t.ПРежимСверки.string_search_l:
                    debug(TangoRegex) Стдвыв.форматнс("ткст ищи {} in {}", c, t.predicate.data_str);
                    if ( индексУ(t.predicate.data_str.ptr, c, t.predicate.data_str.length) >= t.predicate.data_str.length )
                        continue Ltrans_loop;
                    goto Lno_consume;
                ...
                // генерный
                case predicate_t.ПРежимСверки.генерный:
                    ...
                    continue Ltrans_loop;
                case predicate_t.ПРежимСверки.generic_l:
                    debug(TangoRegex) Стдвыв.форматнс("генерный {}\n{}", c, t.predicate.вТкст);
                    for ( auto cmp = t.predicate.data_str.ptr,
                            cmpend = cmp + t.predicate.data_str.length;
                            cmp < cmpend; ++cmp )
...
Laccept:
    ...
    **********************************************************************************************/
    т_сим[] сверь(бцел индекс)
    {
        ...
        if ( старт >= 0 && старт < конец && конец <= ввод_.length )
            return ввод_[старт .. конец];
        return пусто;
    }
    ...
    **********************************************************************************************/
    т_сим[] pre()
    {
        ...
        if ( старт < 0 )
            return пусто;
        return ввод_[0 .. last_start_+старт];
    }
    ...
    **********************************************************************************************/
    ...
    /**********************************************************************************************
        Splits the ввод at the совпадает of this regular Выражение преобр_в an Массив of slices.
        Пример:
            ---
            import io.Stdout;
            import text.Regex;
    ...
    **********************************************************************************************/
    ...
    /**********************************************************************************************
        Возвращает копируй of the ввод with все совпадает replaced by замена.
    **********************************************************************************************/
    ...
    /**********************************************************************************************
        Возвращает копируй of the ввод with the последний сверь replaced by замена.
    **********************************************************************************************/
    ...
    /**********************************************************************************************
        Возвращает копируй of the ввод with the первый сверь replaced by замена.
    **********************************************************************************************/
    ...
    **********************************************************************************************/
    ...
    /**********************************************************************************************
        Compiles the regular Выражение в_ D код.
 ...
 *
 *  o  Нельзя искажать исток данного программного обеспечения; либо
