...
/******************************************************************************
        ...
        добавь trailing '0' decimals when установи ~ иначе trailing '0's 
        will be elопрed

******************************************************************************/

T[] форматируй(T, D=ЧисТип, U=цел) (T[] приёмн, D x, U decimals=Dec, U e=Exp, бул пад=Pad)
{return форматируй!(T)(приёмн, x, decimals, e, пад);}

T[] форматируй(T) (T[] приёмн, ЧисТип x, цел decimals=Dec, цел e=Exp, бул пад=Pad)
{
        сим*           конец,
                        стр;
        цел             эксп,
...
ЧисТип разбор(T) (T[] ист, бцел* ate=пусто)
{
        ...
        if (корень != 10)
           {
           дол знач = Целое.разбор (ист, корень, ate); 
...
/******************************************************************************
        ...
        Convert a плав в_ a ткст. This produces pretty good результатs
...
T[] форматируй(T, D=дво, U=бцел) (T[] приёмн, D x, U decimals=Dec, цел e=Exp, бул пад=Pad)
{return форматируй!(T)(приёмн, x, decimals, e, пад);}

T[] форматируй(T) (T[] приёмн, ЧисТип x, бцел decimals=Dec, цел e=Exp, бул пад=Pad)
{
        static T[] inf = "-inf";
        static T[] nan = "-nan";
