...
class РазбоксИскл : Исключение
{
    ...
    this(Бокс объект, ИнфОТипе типВывода, ткст файл = "Неизвестно", т_мера строка = 0)
    {
        ...
        super(stdrus.форматируй("Не удалось разбоксирование из типа %s в тип %s.", объект.тип, типВывода), файл, строка);
...
struct Бокс
{
   ...
   ткст string;
        проц[] арги = new проц[(char[]).sizeof + данные.length];
        ткст format = "%s";
       ...
       проц putc(dchar ch)
        ...
        арги[0..(char[]).sizeof] = (cast(ук) &format)[0..(char[]).sizeof];
        арги[(char[]).sizeof..length] = данные;
        форматДелай(&putc, arguments, арги.ptr);
        delete арги;
