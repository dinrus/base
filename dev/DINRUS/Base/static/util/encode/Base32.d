...
export ткст кодируй(ббайт[] данные, ткст buff, бул пад=да)
in
{
    assert(данные);
...
body
{
    ...
    foreach (ббайт j; данные)
    {
        ...
        {
            остдлина -= 5;
            buff[i++] = _encodeTable[(остаток>>остдлина)&0b11111];
        }
        while (остдлина > 5)
        }
    if (остдлина)
        buff[i++] = _encodeTable[(остаток<<(5-остдлина))&0b11111];
    if (пад)
    {
...
body
{
    ...
    foreach (c; данные)
    {
        ...
        for (остдлина += 5; остдлина >= 8; остдлина -= 8)
            buff[oIndex++] = остаток >> (остдлина-8);
    }
