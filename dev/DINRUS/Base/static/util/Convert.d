/**
 * Этот модуль предоставляет шаблонные функции, выполняющие преобразования
 * с сохранением значений над произвольными типами. Поведение этой функции
 * может быть, при необходимости, расширено пользовательскими типами.
 */
...
version( TangoDoc )
{
     ...
     */
    D в_(D,S)(S значение);
    D в_(D,S)(S значение, D default_); /// описано ранее
}
else
{
    template в_(D)
...
/**
 * This исключение is thrown when the в_ template is unable в_ выполни a
 * conversion at run-время.  This typically occurs when the исток значение cannot
...
T[] ctfe_trimr(T)(T[] исток)
{
    if( исток.length == 0 )
        return пусто;

    foreach_reverse( i,c ; исток )
    if( !ctfe_isSpace(c) )
        return исток[0..i+1];
...
T[] ctfe_trim(T)(T[] исток)
{
    return ctfe_trimr(ctfe_triml(исток));
}
...
{
    static assert(нет, "Неподдерживаемое преобразование: нельзя преобразовать в "
                  ~ctfe_trim(D.stringof)~" из "
...
template convError()
{
    проц throwConvError()
    {
        ...
        {
            throw new ИсклПреобразования("Не удаётся преобразовать значение типа "
                    ~S.stringof~" в тип "~D.stringof~".");
        }
        else
        {
            throw new ИсклПреобразования("Не удаётся преобразовать `"
                    ~в_!(ткст)(значение)~"` типа "
                    ~S.stringof~" в тип "~D.stringof~".");
        }
    }
}
...
{
    auto знач = math.Math.округли(значение);
    if( (cast(реал) D.min) <= знач && знач <= (cast(реал) D.max) )
    {
        return cast(D) знач;
    }
    else
...
D toMapFromMap(D,S)(S значение)
{
    ...
    foreach( ключ,знач ; значение )
    результат[ в_!(Dk)(ключ) ] = в_!(Dv)(знач);
