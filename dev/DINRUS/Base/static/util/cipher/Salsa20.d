...
class Сальса20 : ШифрПоток
{
    protected
    {
        // Constants
        static ббайт[] сигма = cast(ббайт[])"расширь 32-байт ключ",
                       tau = cast(ббайт[])"расширь 16-байт ключ";
    ...
    проц init(бул зашифруй, Параметры парамы)
    {
        ...
        if (!ivParams)
            не_годится(имя()~": init параметры must include an IV. (use ПараметрыIV)");
    ...
    ббайт верниБбайт(ббайт ввод)
    {
        ...
        if (индекс == 0) {
            словоСальса20ВБбайт(состояние, keyПоток);
            состояние[i0]++;
            if (!состояние[i0])
                состояние[i1]++;
    ...
    debug (UnitTest)
    {
        unittest
        {
            ...
            {
                СимметричныйКлюч ключ = new СимметричныйКлюч(БайтКонвертер.раскодируйГекс(test_keys[i]));
                ПараметрыIV парамы = new ПараметрыIV(ключ, БайтКонвертер.раскодируйГекс(test_ivs[i]));
                
                // Encryption
                s20.init(да, парамы);
                s20.обнови(БайтКонвертер.раскодируйГекс(test_plaintexts[i]), буфер);
                ...
                // Decryption
                s20.init(нет, парамы);
                s20.обнови(БайтКонвертер.раскодируйГекс(test_ciphertexts[i]), буфер);
                результат = БайтКонвертер.кодируйГекс(буфер);
