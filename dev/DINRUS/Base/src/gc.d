...
private import exception, stdrus, sync, sys.WinConsts, cidrus, std.intrinsic, osutil, rt.lifetime;
...
//debug = SENTINEL;
//debug = НА_КОНСОЛЬ;
version = СТЭК_РАСТЁТ_ВНИЗ;	// наращивание стэка означает отнимание от указателя стэка
				// (используется для процессоров Intel X86)
				// иначе наращивание стэка означает прибавление к указателю стэка.
...
+/
    ...
    т_слово тестСброс(т_мера инд)
    {
        ...
        { 
            ...
            auto укз = &данные[1 + (инд >> СДВИГ_БИТ)];
            auto  маска = (БИТЫ_1 << (инд & МАСКА_БИТ));
            auto результат = *укз & маска;
            *укз &= ~маска;
            return результат;
        }
    }
...
debug (LOGGING)
{
    struct Журнал
    {
        ...
        проц выведи()
        {
            эхо("    ptr = %x, razmer = %d, roditel' = %x ", укз, размер, родитель);
            if (файл)
            {
                эхо("%s(%u)", файл, строка);
            }
...
export extern (D) проц иниц()
    {
        ...
        смх = cast(СМХ*)cidrus.кразмести(1, СМХ.sizeof);
        if (!смх)
            throw new ВнеПамИскл;
        смх.иниц();
        устНизСтэка(osutil.ртНизСтэка());
		//эхо("СМ initialized");
    }
...
////////////////////////////////// Исключено по неработоспособности из ДЛЛ!!!	
/+
invariant()
    {
	инвариант();
...
////////////////////////////////////////////	
    ...
    {
        смх.началоСборки = начало;
        смх.конецСборки = конец;
    }
	...
	* в зависимости от реализации сборщика мусора,
	 ...
	 * памяти, на которую ссылается укз. Если укз ссылается на пямять, изначально распределенную
	 * не этим мусоросборщиком, то указывает на внутренности блока памяти, или если if укз
     * нулевой, возвращает ноль.
     *
     * Параметры:
     *  укз = Указатель на корень действительного блока памяти или на пусто.
     *
     * Возвращает:
     *  Бит-поле, содержащее какие-либо биты, установленные для блока памяти, на который
	 * указывает  укз, или ноль при ошибке.
     */
    export extern (D) бцел дайАтр( ук укз )
    {
         if (!укз)
        {
            return 0;
        ...
        бцел go()
        {
            Пул* пул = смх.найдиПул(укз);
            бцел  oldb = 0;

            if (пул)
            {
                auto битинд = cast(т_мера)(укз - пул.адрОсновы) >> пул.смещНа;

                oldb = смх.дайБиты(пул, битинд);
            }
            return oldb;
        }
    ...
    /**
     * Устанавливает указаннные биты для обращений к памяти переменной укз.  Если укз обращается
     ...
     *
     * Параметры:
     *  укз = Указатель на корень действительного блока памяти, либо в пусто.
     ...
     */
    export extern (D) бцел устАтр( ук укз, ПАтрБлока маска )
    {
         if (!укз)
        {
            return 0;
    ...
    /**
     * Удаляет указанные биты для обращения к памяти переменной укз.  Если укз обращается
     * к памяти, размещённой изначально не данным сборщиком, указывает внутрь блока памяти
     * либо содержит значение пусто, никакого действия не последует.
     *
     * Параметры:
     ...
     *  Результат вызова дайАтр устанавливается после установки указанного бита.
     */ 
    export extern (D) бцел удалиАтр( ук укз, ПАтрБлока маска )
    {
        if (!укз)
        {
            return 0;
        ...
        бцел go()
        {
            ...
            if (пул)
            {
                ...
                oldb = смх.дайБиты(пул, битинд);
                смх.удалиБиты(пул, битинд, маска);
            }
            return oldb;
        }
     ...
     * от которого ожидается вывод ВнеПамИскл.
     *
     * Параметры:
     ...
     *
     * Выводит:
     *  ВнеПамИскл при неудачном распределении.
     */
    export extern (D) ук празмести(т_мера размер, бцел биты = 0, т_мера *разм_размещ = null)
    {
        ...
        {
		//эхо("Passed to празместиБезСинх\n");
            return празместиБезСинх(размер, биты, разм_размещ);
        }
    }
     ...
     * запуске сборки. Если распределение неудачно, то эта функция вызывает onOutOfMemory,
     * от которого ожидается вывод ВнеПамИскл.
     *
     * Параметры:
     *  разм = Желаемый размер размещения в байтах.
    ...
    export extern (D) ук кразмести(т_мера размер, бцел биты = 0, т_мера *разм_размещ = null)
    {
        ...
        synchronized (смЗамок)
        {
            return кразместиБезСинх(размер, биты, разм_размещ);
        }
    }

    /**
     * Если разм равно нулю, то память, на которую указывает укз, будет освобождена,
	 ...
	 * от которого ожидается вывод исключения ВнеПамИскл. Если же укз
	  ...
	  * а укз указывает на главу действительного, известного блока памяти,
     * тогда все биты, установленные на текущем блоке, будут перенесены на новый блок,
	 * если потребуется перемещение. Если ba не равно нулю, а укз указывает на
     ...
     *  нулю. При неудаче возвращается исходное значение укз.
     *
     * Выводит:
     *  ВнеПамИскл при неудачном размещении.
     */
    export extern (D) ук перемести(ук укз, т_мера размер, бцел биты = 0, т_мера *разм_размещ = null)
    {
        synchronized (смЗамок)
        {
            return переместиБезСинх(укз, размер, биты, разм_размещ);
        }
    }

    /**
     * Запрашивает, чтобы блок управляемой памяти,  на который ссылается укз,
     ...
     * и укз ссылается на память, ранее размещенную другим сборщиком,
	 * либо же укз указывает внутрь блока память,
     ...
     *
     * Возвращает:
     *  Размер в байтах расширенного блока памяти, показываемого укз, или обнули,
     *  если расширения не произошло.
     */
    export extern (D) т_мера расширь(ук укз, т_мера минразм, т_мера максразм)
    {
        if(!нить_нужнаБлокировка()||stdrus.Нить.члонн ==1)
        {
            return расширьБезСинх(укз, минразм, максразм);
        }
        else synchronized (смЗамок)
        {
            return расширьБезСинх(укз, минразм, максразм);
    ...
    export extern (D) т_мера резервируй(т_мера размер)
    {
        ...
        if(!нить_нужнаБлокировка()||stdrus.Нить.члонн ==1)
        {
            return резервируйБезСинх(размер);
        }
        else synchronized (смЗамок)
        {
            return резервируйБезСинх(размер);
    ...
    /**
     * Вымещает память, на которую показывает укз.  Если укз равен нулю, никаких
     * действий не предпринимается. Если укз ссылается на память, ранее размещенную другим
     ...
     *
     * Параметры:
     *  укз = Указатель на корень действующего блока памяти или в пусто.
     */
    export extern (D) проц освободи( ук укз )
    {
        if (!укз)
        {
            return;
    ...
    /**
     * Возвращает адрес базы блока памяти, содержащего укз.  Это значение
     * может использоваться для определения того, является ли укз
	 ...
	 * Если укз ссылается на память, которую изначально выделил не этот
	 * сборщик мусора, если укз = пусто, или сборщик мусора не поддерживает
     * данную операцию, то будет возвращено пусто.
     *
     * Параметры:
     *  укз = Указатель на корень или внутренность валидного блока памяти или в
     *      пусто.
     *
     * Возвращает:
     *  Базовый адрес блока памяти, на который ссылается укз или пусто при ошибке.
     */
    export extern (D) ук адрес_у( ук укз )
    {
        if (!укз)
        {
            return null;
    ...
    /**
     * Возвращает настоящий размер блока памяти, указанного укз.  Это значение
     * представляет максимальный размер в байтах, в который вызовом realloc 
     * можно на месте изменить существующий блок. Если укз ссылается на память,
     * которая не была изначально размещена этим СМ, то указывает на внутренныю часть
     * блока памяти, либо, если укз = пусто, возвращает ноль.
     *
     * Параметры:
     *  укз = указатель на валидный блок памяти или в пусто.
     *
     * Возвращает:
     *  Размер в байтах  блока памяти, на который ссылается укз или ноль при ошибке.
     */
    export extern (D) т_мера размер_у( ук укз )
    {
        if (!укз)
        {
            return 0;
    ...
    /**
     * Возвращает совокупную информацию о блоке памяти, содержащем укз.  Если укз
     * ссылается на память, исходно установленную не этим сборщиком мусора, 
     *или укз имеет значение пусто, либо СМ не поддерживает эту операцию, будет
     * выдано ИнфОБл.init. Как правило, поддержка данной операции зависит от поддержки
     * функции адрес_у.
     *
     * Параметры:
     *  укз = Указатель на корень или внутренность валидного блока памяти
     *   либо на  пусто.
     *
     * Возвращает:
     *  Информацию, касающуюся блока бамяти, на который ссылается укз, или ИнфОБл.init
     *  при ошибке.
     */
    export extern (D) ИнфОБл опроси( ук укз )
    {
        if (!укз)
        {
            ИнфОБл инд;
     ...
     */
    export extern (D) проц проверь(ук укз)
    {
        if (!укз)
        {
            return;
    ...
    /**
     * Добавляет адрес памяти, на который ссылается укз, во внутренний список корней,
     * подлежащих сканированию при сборке.  Если укз = пусто, никакой операции
     * не выполняется.
     *
     * Параметры:
     *  укз = Указатель на действительный адрес памяти или в пусто.
     */
    export extern (D) проц добавьКорень( ук укз )
    {
        if (!укз)
        {
            return;
  ...
  export extern (D)  цел delegate(цел delegate(ref ук)) обходКорня()
    ...
    /**
     * Добавляет блок памяти, указанный укз размера разм во внутренний список
     * сканируемых при сборке диапазонов.  Если укз пусто, то никаких действий
     * не предпринимается.
     *
     * Параметры:
     *  укз  = Указатель на действительный адрес памяти или в пусто.
     *  разм = Размер в байтах добавляемого блока.  Если разм = нуль, то
     *      ничего не произойдёт. Если укз = пусто, то разм должен быть нуль.
     */
////////////////////////////////	 
    export extern (D) проц добавьПространство( ук укз, т_мера разм )
    {
        ...
        if(!нить_нужнаБлокировка()||stdrus.Нить.члонн ==1)
        {
            смх.добавьПространство(укз, укз + разм);
        }
        else synchronized (смЗамок)
        {
            смх.добавьПространство(укз, укз + разм);
...
/////////////////////////////////////////	
	...
	debug(НА_КОНСОЛЬ) скажифнс("+СМ.добавьПространство(Низ = x%x, Верх = x%x)\n", Низ, Верх);
        if(!нить_нужнаБлокировка()||stdrus.Нить.члонн ==1)
        {
	    смх.добавьПространство(Низ, Верх);
        }
        else synchronized (смЗамок)
	{
	    смх.добавьПространство(Низ, Верх);
...
//////////////////////////	
    ...
    /**
     * Удаляет блок памяти, указанный укз, из внутреннего списка "корней" (ветвей),
     * подлежащих сканированию при сборке. Если укз = пусто или не представляет собой
     * ранее переданное add(ук) значение, то никакого действия не происходит.
     *
     *  укз  = Указатель на действительный адрес памяти или в пусто.
     */
	 
    export extern (D) проц удалиКорень( ук укз )
    {
        if (!укз)
        {
            return;
    ...
    /**
     * Удаляет блок памяти, на к-й ссылается укз, из внутреннего списка диапазонов,
     * подлежащих сканированию при сборке мусора. Если укз пусто или не представляет значение,
     * ранее переданное add(ук, т_мера), то никакой операции не выполняется.
     *
     * Параметры:
     *  укз  = Указатель на действительный адрес памяти или в пусто.
     */
    export extern (D) проц удалиПространство( ук укз )
    {
        if (!укз)
        {
            return;
    ...
    export extern (D) ук создайСлабУк( Объект r )
    {
        if (r)
           {
           //должно быть размещено в памяти Си
           //1. ссылка из этого СМ скрывается нами
           //2. этот СМ не сканирует делегатов, добавленных rt_detachDisposeEvent для ссылок
     ...
     */
    export extern (D) проц удалиСлабУк( ук укз )
    {
        if (укз)        
           {
           auto wp = cast(СлабыйУказатель*)укз;
     ...
     */
    export extern (D) Объект дайСлабУк( ук укз )
    {
        if (укз)
           {
           ...
           return блокированный!(Объект)({
                  return (cast(СлабыйУказатель*)укз).ссылка;
                  });
           }
        return null;
    }
    ...
    * Параметры:
    * новдлин = число размещаемых элементов
    * размЭлта = размер одного элемента
    */
    export extern (D) т_мера нарастиДлину (т_мера новдлин, т_мера размЭлта=1)
    {   
        return нарастиДлину (новдлин, размЭлта, 100, 0, 1);
    }
    ...
    */
    export extern (D) т_мера нарастиДлину(т_мера новдлин, т_мера размЭлта, т_мера a, т_мера b=0, т_мера минБитов=1)
    {
        ...
        т_мера newext = 0;
        т_мера newcap = новдлин * размЭлта;
        дол mult = 100 + a*(минБитов + b) / (log2(новдлин) + b);
   ...
   /**
    ...
    export extern (D) проц полныйСбор()
    {
        ...
        version (none)
        {
            ...
            getStats(стат);
            debug(НА_КОНСОЛЬ) скажифнс("размерПула = %x, испРазмер = %x, размСпискаСвобБлоков = %x\n",
...
///////////////////////////////
	export extern (D) проц естьУказатели(ук укз)
    {
	synchronized (смЗамок)
	{
	    смх.ЕстьУказатели(укз);
	}
    }
/////////////////////////
	export extern (D) проц нетУказателей(ук укз)
    {
	if (!смх.консервативный)
	{   synchronized (смЗамок)
	    {
		смх.НетУказателей(укз);
	    }
	}
    }
////////////////////////
    ...
    {
	смх.консервативный = 1;
    }
///////////////////
/**
     * Определить размещённый размер указателя укз.  If укз is an interior pointer
     * or not a gc allocated pointer, return 0.
     */
   export extern (D) т_мера ёмкость(ук укз)
    {
        if (!укз)
        {
            return 0;
...
/**
	...
	export extern (D) проц устФинализатор(ук укз, ФИНАЛИЗАТОР_СМ укНаФункц)
    {
	synchronized (смЗамок)
	{
	    смх.финализатор = укНаФункц;
	    смх.заверши(укз);
	}
    }
...
private:
//////////////////////////////////////////////
ук празместиБезСинх(т_мера размер, бцел биты = 0, т_мера *разм_размещ = null)
//////////////////////////////////////////////
    {
        assert(размер != 0);
        ...
        if (бин < Б_СТР)
        {
           ...
           состояние = смх.отключен ? 1 : 0;
            собрано = false;

            while (!смх.бак[бин] && !смх.разместиСтраницу(бин))
            {
                switch (состояние)
                {
                case 0:
                    т_мера удалённые_страницы = смх.полная_уборка();
                    собрано = true;
                    if (удалённые_страницы < смх.чло_пулов * ((РАЗМЕР_ПУЛА / РАЗМЕР_СТРАНИЦЫ) / 8))
                    {   
                        смх.новыйПул(1, false);
                        состояние = 2;
                    }
                    else
                        состояние = 1;
            ...
            смх.бак[бин] = (cast(Список*)укз).следщ;
            if( !(биты & ПАтрБлока.НеСканировать) )
                cidrus.memset(укз + размер, 0, бинразм[бин] - размер);
            debug(НА_КОНСОЛЬ) скажифнс("\tcidrus.празмести => %x\n", укз);
            debug (MEMSTOMP) cidrus.memset(укз, 0xF0, размер);
        }
        else
        {
            укз = смх.большоеРазмещение(размер, &пул, разм_размещ);
            if (!укз)
                throw new ВнеПамИскл();
        }
        размер -= SENTINEL_EXTRA;
        укз = сентинель_добавь(укз);
        сентинель_иниц(укз, размер);
        смх.журнал_празмести(укз, размер);

        if (биты)
        {
            пул = смх.найдиПул(укз);
            assert(пул);

            смх.устБиты(пул, cast(т_мера)(укз - пул.адрОсновы) >> пул.смещНа, биты);
        }
		debug(НА_КОНСОЛЬ) скажифнс("Возврат из cidrus.празмести %i\n", укз);
        return укз;
    }
/////////////////////////////////////////////	
ук кразместиБезСинх(т_мера размер, бцел биты = 0, т_мера *разм_размещ = null)
/////////////////////////////////////////////
    {
        assert(размер != 0);

        //debug(НА_КОНСОЛЬ) скажифнс("cidrus.кразмести: %x len %d\n", укз, len);
        ук укз = празместиБезСинх(размер, биты, разм_размещ);
        cidrus.memset(укз, 0, размер);
        return укз;
    }
////////////////////////////////////////////////////	
ук переместиБезСинх(ук укз, т_мера размер, бцел биты = 0, т_мера *разм_размещ = null)
////////////////////////////////////////////////////
    {
        if (!размер)
        {   if (укз)
        ...
        {
            укз = празместиБезСинх(размер, биты, разм_размещ);
        }
        else
        {   ук p2;
            т_мера размук;
            ...
            version (SENTINEL)
            {
                сентинель_Инвариант(укз);
                размук = *сентинель_размер(укз);
                if (размук != размер)
                {
                    if (размук)
                    {
                        ...
                        if (пул)
                        {
                            ...
                            if (биты)
                            {
                                смх.удалиБиты(пул, битинд, ПАтрБлока.ВсеБиты);
                                смх.устБиты(пул, битинд, биты);
                            }
                            else
                            {
                                биты = смх.дайБиты(пул, битинд);
                            }
                        }
                    }
                    p2 = празместиБезСинх(размер, биты, разм_размещ);
                    if (размук < размер)
                        размер = размук;
                    cidrus.memcpy(p2, укз, размер);
                    укз = p2;
                }
            }
            else
            {
                размук = смх.найдиРазмер(укз);        // найди allocated размер
                if (размук >= РАЗМЕР_СТРАНИЦЫ && размер >= РАЗМЕР_СТРАНИЦЫ)
                {
                    ...
                    auto пул = смх.найдиПул(укз);
                    auto номерстр = (укз - пул.адрОсновы) / РАЗМЕР_СТРАНИЦЫ;

                    if (newsz < psz)
                    {   // Shrink in place
                        synchronized (смЗамок)
                        {
                            debug (MEMSTOMP) cidrus.memset(укз + размер, 0xF2, размук - размер);
                    ...
                    else if (номерстр + newsz <= пул.члостр)
                    {
                        ...
                        synchronized (смЗамок)
                        {
                            for (т_мера инд = номерстр + psz; 1;)
                            {
                                ...
                                {
                                    //debug (MEMSTOMP) cidrus.memset(укз + размук, 0xF0, размер - размук);
...
//////////////////////////////////////////////////////////
т_мера расширьБезСинх(ук укз, т_мера аргминразм, т_мера аргмаксразм)
//////////////////////////////////////////////////////////
    in
    {
	...
	if (смх.пущен)
        ...
        else if (номерстр + psz + разм == пул.члопереданных)
        {
            ...
            return 0;
        debug (MEMSTOMP) cidrus.memset(укз + размук, 0xF0, (psz + разм) * РАЗМЕР_СТРАНИЦЫ - размук);
        cidrus.memset(пул.таблица_страниц + номерстр + psz, Б_СТРПЛЮС, разм);
		пул.обновиСмещения(номерстр);
        пул.свобстры -= разм;
        if (укз == смх.ключ_кэшированного_размера)
            смх.знач_кэшированного_размера = (psz + разм) * РАЗМЕР_СТРАНИЦЫ;
        if (укз == смх.ключ_кэшированной_инфо)
            смх.знач_кэшированной_инфо.размер = (psz + разм) * РАЗМЕР_СТРАНИЦЫ;
        return (psz + разм) * РАЗМЕР_СТРАНИЦЫ;
    }
///////////////////////////////////	
т_мера резервируйБезСинх(т_мера размер)
///////////////////////////////////
    {
        assert(размер != 0);
        assert(смх);
        ...
        return смх.резервируй(размер);
    }
/////////////////////
проц освободиБезСинх(ук укз)
/////////////////////
    {
        assert (укз);
        ...
        if (!пул)                              // if not one of ours
            return;                             // ignore
        сентинель_Инвариант(укз);
        укз = сентинель_отн(укз);
        номерстр = cast(т_мера)(укз - пул.адрОсновы) / РАЗМЕР_СТРАНИЦЫ;
        битинд = cast(т_мера)(укз - пул.адрОсновы) >> пул.смещНа;
        смх.удалиБиты(пул, битинд, ПАтрБлока.ВсеБиты);
        ...
        if (бин == Б_СТР)              // if large alloc
        {
            ...
            debug (MEMSTOMP) cidrus.memset(укз, 0xF2, члостр * РАЗМЕР_СТРАНИЦЫ);
            пул.освободиСтраницы(номерстр, члостр);
        }
        else
        {   // Add to cidrus.освободи список
            Список *список = cast(Список*)укз;

            debug (MEMSTOMP) cidrus.memset(укз, 0xF2, бинразм[бин]);

            список.следщ = смх.бак[бин];
			список.пул = пул;
            смх.бак[бин] = список;
        }
        смх.журнал_освободи(сентинель_добавь(укз));
    }

/////////////////////	
ук адрес_уБезСинх(ук укз)
/////////////////////
    {
        if (!укз)
        {
        ...
        return смх.найдиОснову(укз);
    }
	
/////////////////////////
т_мера размер_уБезСинх(ук укз)
/////////////////////////
    {
        assert (укз);
        ...
        {
            укз = сентинель_отн(укз);
            т_мера размер = смх.найдиРазмер(укз);
            ...
            if (cast(т_мера)укз & (размер - 1) & (РАЗМЕР_СТРАНИЦЫ - 1))
                размер = 0;
...
////////////////////////
ИнфОБл опросиБезСинх(ук укз)
///////////////////////
    {
        assert(укз);

        return смх.дайИнфо(укз);
    }
	
//////////////////////
проц проверьБезСинх(ук укз)
///////////////////////
    {
        assert(укз);
        ...
        debug (PTRCHECK)
        {
            ...
            assert(бин <= Б_СТР);
            размер = бинразм[бин];
            assert((cast(т_мера)укз & (размер - 1)) == 0);

            debug (PTRCHECK2)
            {
                if (бин < Б_СТР)
                {
                    // Check that укз is not on a cidrus.освободи список
                    Список *список;

                    for (список = смх.бак[бин]; список; список = список.следщ)
                    {
                        assert(cast(ук)список != укз);
                    }
                }
            }
        }
...
/////////////////////////
    {
        ...
        else
        {
            ...
            {
                debug(НА_КОНСОЛЬ) скажифнс("устНизСтэка(%x)\n", укз);
                смх.низСтэка = cast(char*)укз;
            }
        }
    }
////////////////////////
проц дайСтатБезСинх(out СМСтат стат)
///////////////////////////////
    {
        т_мера размук = 0;
        т_мера размфл = 0;
        ...
        for (n = 0; n < смх.чло_пулов; n++)
        {   Пул *пул = смх.таблица_пулов[n];
        ...
        for (n = 0; n < Б_СТР; n++)
        {
            for (Список *список = смх.бак[n]; список; список = список.следщ)
            {
                размфл += бинразм[n];
            }
        }
    ...
    //call блокированный if necessary
    T блокированный(T)(in T delegate() код) 
    {
        if (нить_нужнаБлокировка()||stdrus.Нить.члонн != 1)
            synchronized(смЗамок) return код();
        else
           return код();
    }
    ...
    struct СлабыйУказатель 
    {
        ...
        проц ondestroy(Объект r) 
        {
            ...
            блокированный!(проц)({ ссылка = null; });
        }
    }
...
struct СМХ
{
	    ...
	    debug (THREADINVARIANT)
    {
        ...
        {
            if (этот != эта_нить())
                скажифнс("инвариант_Нити(): смх = %x, this = %x, pthread_self() = %x\n", this, этот, эта_нить());
    ...
    {   цел dummy;
        ...
        (cast(байт*)this)[0 .. СМХ.sizeof] = 0;
        низСтэка = cast(char*)&dummy;
        журнал_иниц();
        debug (THREADINVARIANT)
            этот = эта_нить();
        //скажифнс("смх = %укз, этот = %x\n", this, этот);
        this.готов = 1;
    }
...
if(готов)
{
       // debug скажифнс("СМХ.invariant(): this = %укз\n", this);
            т_мера инд;
...
////////////////////////////////////////////
   ...
   /**
     * Run финализатор on укз when it is cidrus.освободи'd.
     */
    проц заверши(ук укз)
    {
	Пул *пул = найдиПул(укз);
	assert(пул);
	...
	пул.финалы.установи((укз - пул.адрОсновы) / 16);
    }
	
    /**
     * Indicate that block pointed to by укз has possible pointers
     * to СМ allocated memory in it.
     */

	проц ЕстьУказатели(ук укз)
    {
	Пул *пул = найдиПул(укз);
	assert(пул);

	пул.нескан.сбрось((укз - пул.адрОсновы) / 16);
    }
	
    /**
     * Indicate that block pointed to by укз has no possible pointers
     * to СМ allocated memory in it.
     */
    проц НетУказателей(ук укз)
    {
	//скажифнс("НетУказателей(%укз)\n", укз);
	Пул *пул = найдиПул(укз);
	assert(пул);

	пул.нескан.установи((укз - пул.адрОсновы) / 16);
    }
...
//////////////////////////////////////////////////
    ...
    проц добавьПространство(ук Низ, ук Верх)
    {
        ...
        debug(НА_КОНСОЛЬ) скажифнс("%x.СМХ::добавьПространство(%x, %x), члопространств = %d\n", this, Низ, Верх, члопространств);
    ...
    {
        debug(НА_КОНСОЛЬ) скажифнс("Нить %x ", эта_нить());
        debug(НА_КОНСОЛЬ) скажифнс("%x.СМХ.удалиПространство(%x), члопространств = %d\n", this, Низ, члопространств);
    ...
    Пул *найдиПул(ук укз)
    {
        if (укз >= минАдр && укз < максАдр)
        {
            ...
            while (low <= high)
            {
                ...
                auto пул = pt[mid];
                if (укз < пул.адрОсновы)
                    high = mid - 1;
                else if (укз >= пул.адрВерха)
                    low = mid + 1;
                else
                    return пул;
            }
    ...
    ук найдиОснову(ук укз)
    {
        ...
        if (пул)
        {
            т_мера смещение = cast(т_мера)(укз - пул.адрОсновы);
    ...
    т_мера найдиРазмер(ук укз)
    {
        ...
        if (ИСПОЛЬЗОВАТЬ_КЭШ && укз == ключ_кэшированного_размера)
        ...
        if (пул)
        {
            ...
            ключ_кэшированного_размера = укз;
     ...
     *
     */
    ИнфОБл дайИнфо(ук укз)
    {
        Пул*   пул;
        ИнфОБл инфо;
        
        if (ИСПОЛЬЗОВАТЬ_КЭШ && укз == ключ_кэшированной_инфо)
        ...
        if (пул)
        {
            ...
            if (бин <= Б_СТР)
            {
                инфо.основа = cast(ук)((cast(т_мера)укз) & небинразм[бин]);
            }
            else if (бин == Б_СТРПЛЮС)
            {
                auto pageOffset = пул.смещБСтр[pn];
...
//debug скажинс("Экономия пространства выполнена");
    ...
    ук большоеРазмещение(т_мера размер, Пул **poolPtr, т_мера *разм_размещ = null)
    {
        ...
        {
            // This код could use some refinement when repeatedly
      ...
      L1:
        ...
        пул.свобстры -= члостр;		
        укз = пул.адрОсновы + pn * РАЗМЕР_СТРАНИЦЫ;
        cidrus.memset(cast(char *)укз + размер, 0, члостр * РАЗМЕР_СТРАНИЦЫ - размер);
        debug (MEMSTOMP) cidrus.memset(укз, 0xF1, размер);
		 if(разм_размещ)
    ...
    Пул *новыйПул(т_мера члостр, bool объектБольшой)
    {
        ...
        //debug(НА_КОНСОЛЬ) скажифнс("************СМХ::новыйПул(члостр = %d)****************\n", члостр);
    ...
    цел разместиСтраницу(Бины бин)
    {
        ...
        //debug(НА_КОНСОЛЬ) скажифнс("СМХ::разместиСтраницу(бин = %d)\n", бин);
        for (n = 0; n < чло_пулов; n++)
        {
            пул = таблица_пулов[n];
      ...
      L1:
        ...
        укз = пул.адрОсновы + pn * РАЗМЕР_СТРАНИЦЫ;
        верх = укз + РАЗМЕР_СТРАНИЦЫ;
        for (; укз < верх; укз += размер)
        {
            (cast(Список *)укз).следщ = *b;
			(cast(Список *)укз).пул = пул;
            *b = cast(Список *)укз;
        }
        return 1;
    }
	...
	т_мера смещение;
        ...
        //эхо("marking range: %укз -> %укз\n", Низ, Верх);
        for (; p1 < p2; p1++)
        {
            auto укз = cast(байт *)(*p1);

            //if (журнал) debug(НА_КОНСОЛЬ) эхо("\tmark %x\n", укз);
            if (укз >= минАдр && укз < максАдр)
            {
                if ((cast(т_мера)укз & ~cast(т_мера)(РАЗМЕР_СТРАНИЦЫ-1)) == кэш)
 	            continue;

                auto пул = найдиПул(укз);
                if (пул)
                {
                    смещение = cast(т_мера)(укз - пул.адрОсновы);
                    т_мера битинд = void;
                    т_мера pn = смещение / РАЗМЕР_СТРАНИЦЫ;
                    Бины бин = cast(Бины)пул.таблица_страниц[pn];
					ук основа = void;
                    ...
                    else if (бин == Б_СТР)
                    {
                        ...
                        кэш = cast(т_мера)укз & ~cast(т_мера)(РАЗМЕР_СТРАНИЦЫ-1);
                    }
                    else if (бин == Б_СТРПЛЮС)
                    {
                        pn -= пул.смещБСтр[pn];
    ...
    т_мера полная_уборка(ук верхСтэка)
    {
        ...
        debug(COLLECT_PRINTF) эхо("СМХ.fullcollect()\n");
        if (пущен)
        ...
        {   т_мера pn;
            ...
            if(пул.объектБольшой)
            {
                for(pn = 0; pn < члопереданных; pn++)
                {
                    ...
                    {   байт *укз = пул.адрОсновы + pn * РАЗМЕР_СТРАНИЦЫ;

                        сентинель_Инвариант(сентинель_добавь(укз));
                        if (пул.финалы.члобит && пул.финалы.тестСброс(битинд))
                            rt_finalize_gc(сентинель_добавь(укз));
                        удалиБиты(пул, битинд, ПАтрБлока.ВсеБиты ^ ПАтрБлока.Финализовать);

                        debug(COLLECT_PRINTF) эхо("\tcollecting big %укз\n", укз);
                        журнал_освободи(сентинель_добавь(укз));
                        пул.таблица_страниц[pn] = Б_ОСВОБ;
                        if(pn < пул.стартПоиска) пул.стартПоиска = pn;
                        освобождённые_стр++;
                        пул.свобстры++;

                        debug (MEMSTOMP) memset(укз, 0xF3, РАЗМЕР_СТРАНИЦЫ);
                        while (pn + 1 < члопереданных && пул.таблица_страниц[pn + 1] == Б_СТРПЛЮС)
                        {
                            ...
                            debug (MEMSTOMP)
                            {   укз += РАЗМЕР_СТРАНИЦЫ;
                                memset(укз, 0xF3, РАЗМЕР_СТРАНИЦЫ);
                            }
                        }
                    }
                }
            ...
            else
            {
                ...
                for (pn = 0; pn < члопереданных; pn++)
                {
                    ...
                    if (бин < Б_СТР)
                    {
                        ...
                        for (; укз < верх; укз += размер, битинд += битстрайд, индСброса += битстрайд)
                        {
                            if(индСброса > СМБиты.БИТ_НА_СЛОВО - 1)
                            {
                                if(к_сбросу)
                                {
                                    СМХ.УдалиБитыМал(пул, стартСброса, к_сбросу);
                                    к_сбросу = 0;
                                }
                            ...
                            if (!пул.пометь.тестируй(битинд))
                            {
                                ...
                                Список *список = cast(Список *)укз;
                                debug(PRINTF) эхо("\tcollecting %укз\n", список);
                                журнал_освободи(сентинель_добавь(список));

                                debug (MEMSTOMP) memset(укз, 0xF3, размер);
...
/**
    ...
    bool идётУборка(void *укз)
    {
        ...
        if(пул)
        {
            auto смещение = cast(т_мера)(укз - пул.адрОсновы);
            auto pn = смещение / РАЗМЕР_СТРАНИЦЫ;
            auto бины = cast(Бины)пул.таблица_страниц[pn];
            if(бины <= Б_СТР)
            {
                assert(укз == cast(void*)((cast(т_мера)укз) & небинразм[бины]));
...
//            пул.nomove.данные[индексДанных] &= оставить;
    ...
    debug (LOGGING)
    {
        ...
        проц журнал_празмести(ук укз, т_мера размер)
        {
            //debug(НА_КОНСОЛЬ) эхо("+журнал_празмести(укз = %x, размер = %d)\n", укз, размер);
            Журнал журнал;
        ...
        проц журнал_освободи(ук укз)
        {
            //debug(НА_КОНСОЛЬ) эхо("+журнал_освободи(%x)\n", укз);
            т_мера инд;

            инд = текущий.найди(укз);
            if (инд == СБОЙ_ОП)
            {
        ...
        проц журнал_сборка()
        {
            ...
            for (т_мера инд = 0; инд < текущий.разм; инд++)
            {
                ...
                j = предыдущ.найди(текущий.данные[инд].укз);
                if (j == СБОЙ_ОП)
        ...
        проц журнал_родитель(ук укз, ук родитель)
        {
...
struct Пул
{
    ...
    проц иниц(т_мера члостр, бул объектБольшой)
    {
        ...
        // Some of the код depends on page alignment of memory pools
...
version (SENTINEL)
{
    ...
    т_мера* сентинель_размер(ук укз)  { return &(cast(т_мера *)укз)[-2]; }
    т_мера* sentinel_pre(ук укз)   { return &(cast(т_мера *)укз)[-1]; }
    ббайт* sentinel_post(ук укз) { return &(cast(ббайт *)укз)[*сентинель_размер(укз)]; }


    проц сентинель_иниц(ук укз, т_мера размер)
    {
        *сентинель_размер(укз) = размер;
        *sentinel_pre(укз) = SENTINEL_PRE;
        *sentinel_post(укз) = SENTINEL_POST;
    }


    проц сентинель_Инвариант(ук укз)
    {
        assert(*sentinel_pre(укз) == SENTINEL_PRE);
        assert(*sentinel_post(укз) == SENTINEL_POST);
    }


    ук сентинель_добавь(ук укз)
    {
        return укз + 2 * т_мера.sizeof;
    }


    ук сентинель_отн(ук укз)
    {
        return укз - 2 * т_мера.sizeof;
    }
}
