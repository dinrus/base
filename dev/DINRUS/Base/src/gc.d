...
debug (LOGGING)
{
    struct Журнал
    {
        ...
        проц выведи()
        {
            эхо("    укз = %x, razmer = %d, roditel' = %x ", укз, размер, родитель);
            if (файл)
            {
                эхо("%s(%u)", файл, строка);
            }
...
private проц инвариант()
{
	...
	* в зависимости от реализации сборщика мусора,
	 ...
	 * от которого ожидается вывод исключения OutOfMemoryException. Если же укз
     ...
     *
     * Параметры:
     *  укз = A pointer to the корень or the interior of a valid memory block or to
     *      пусто.
     *
     * Возвращает:
     *  The base address of the memory block referenced by укз or пусто on error.
     */
     ...
     *
     * Возвращает:
     *  Размер в байтах of the memory block referenced by укз or ноль on error.
     */
