...
private
{
    ...
    import Аски = text.Ascii;
    import Целое = text.convert.Integer;
    import ШтампВрем = text.convert.TimeStamp;
}
 ...
 *******************************************************************************/
export class ИсклФтп: Исключение
{
    сим[3] кодОтвета_ = "000";
     ...
     ***********************************************************************/
   export  this(ткст сообщение, сим[3] код = "420")
    {
        this.кодОтвета_[] = код;
        super(сообщение);
    }
    ...
    /***********************************************************************
     ...
     r =               the сервер ответ
     ***********************************************************************/
    export this(ОтветФтп r)
    {
        this.кодОтвета_[] = r.код;
        super(r.сообщение);
    }
...
/*******************************************************************************
 ...
 Documentation pending
 *******************************************************************************/
export class СоединениеФтп: Telnet
{

    ЭлтФтп[] поддерживаемыеФичи_ = пусто;
    ДеталиПодключенияФтп инф_;
    т_мера позРестарта_ = 0;
    ткст текФайл_ = "";
    Сокет сокетДанных_;
    ИнтервалВремени таймаут_ ;

    /***********************************************************************
     Добавлено с версии: 0.99.8
     ***********************************************************************/
    export  ИнтервалВремени таймаут()
    {
        return таймаут_;
    }
    ...
    /***********************************************************************
     Добавлено с версии: 0.99.8
     ***********************************************************************/
    export  проц таймаут(ИнтервалВремени t)
    {
        таймаут_ = t;
    }
    ...
    /***********************************************************************
     Добавлено с версии: 0.99.8
     ***********************************************************************/
    export  ИнтервалВремени времяШатдауна()
    {
        return таймаут_ + таймаут_;
    }

    /***********************************************************************
     Добавлено с версии: 0.99.8
     ***********************************************************************/
    export  ЭлтФтп[] поддерживаемыеВозможности()
    {
        if(поддерживаемыеФичи_ !is пусто)
        {
            return поддерживаемыеФичи_;
        }
        дайВозможности();
        return поддерживаемыеФичи_;
    }
     ...
     ***********************************************************************/
    export проц исключение(ткст сообщение)
    {
    ...
    /***********************************************************************
     Изменено с версии: 0.99.8
     ***********************************************************************/
    export проц исключение(ОтветФтп fr)
    {
    ...
    export  this()
    {
        таймаут_ = ИнтервалВремени.изМиллисек(5000);
    }

    export  this(ткст имя_хоста, ткст ник = "anonymous",
    ...
    /***********************************************************************
     Добавлено с версии: 0.99.8
     ***********************************************************************/
    export  проц подключись(АдресФтп fad)
    {
    ...
    /************************************************************************
     Изменено с версии: 0.99.8
     ************************************************************************/
    export  проц подключись(ткст имя_хоста, ткст ник = "anonymous",
                                         ткст пароль = "anonymous@anonymous", бцел порт = 21)
    in
    {
    ...
    export  проц закрой()
    {
        ...
        if(сокетДанных_ !is пусто)
            this.завершиКомандуДанных(сокетДанных_);
        if(сокет_ !is пусто)
        {
            try
            {
            ...
            delete поддерживаемыеФичи_;
            delete сокет_;
        }
    }

    export  проц устПассив()
    {
        инф_.тип = ПТипСоединенияФтп.Пассивное;

        delete инф_.адрес;
        delete инф_.слушай;
    }

    export  проц устАктив(ткст ИП, бкрат порт, ткст ип_прослушки = пусто,
    ...
    body
    {
        инф_.тип = ПТипСоединенияФтп.Активное;
        инф_.адрес = new АдресИПв4(ИП, порт);
        ...
        if(ип_прослушки == пусто)
            инф_.слушай = new АдресИПв4(АдресИПв4.АДР_ЛЮБОЙ, порт_прослушки);
        else
            инф_.слушай = new АдресИПв4(ип_прослушки, порт_прослушки);
    }
    ...
    export  проц chmod(ткст путь, цел режим)
    in
    {
    ...
    export  проц переименуй(ткст old_path, ткст new_path)
    in
    {
   ...
   export  цел есть_ли(ткст файл)
    ...
    export  т_мера размер(ткст путь, ПФорматФтп форматируй = ПФорматФтп.Образ)
    in
    {
        assert(путь.length > 0);
    }
    ...
    export  проц тип(ПФорматФтп форматируй)
    {
    ...
    /***********************************************************************
     Добавлено с версии: 0.99.8
     ***********************************************************************/
    export Время изменён(ткст файл)
    in
    {
    ...
    export  проц дайВозможности()
    {
        ...
        else
        {
            ...
            поддерживаемыеФичи_ = new ЭлтФтп[строки.length - 1];
            поддерживаемыеФичи_[0].команда = "FEAT";

            for(т_мера i = 1; i < строки.length - 1; i++)
            {
                т_мера поз = Текст.местоположение(строки[i], ' ');

                поддерживаемыеФичи_[i].команда = строки[i][0 .. поз];
                if(поз < строки[i].length - 1)
                    поддерживаемыеФичи_[i].парамы = строки[i][поз + 1 .. строки[i].length];
            }
    ...
    export  проц шлиКоманду(ткст команда, ткст[] параметры...)
    {
    ...
    export  ОтветФтп читайОтвет(ткст ожидаемый_код)
    {
        debug(FtpDebug)
        {
    ...
    protected Сокет дайСокетДанных()
    {
        ...
        case ПТипСоединенияФтп.Активное:
            АдресИПв4 data_Addr = cast(АдресИПв4) this.инф_.адрес;

            // Start listening.
            Сокет listener = new Сокет;
            listener.вяжи(this.инф_.слушай);
    ...
    /*******************************************************************************
      ...
      *******************************************************************************/
    export  Сокет подключисьПассивно()
    {
    ...
    export  бул поддерживается(ткст команда)
    in
    {
    ...
    body
    {
        if(this.поддерживаемыеФичи_.length == 0)
            return да;

        // Поиск through the список for the feature.
        foreach(ЭлтФтп feat; this.поддерживаемыеФичи_)
        {
            if(Аски.сравнилюб(feat.команда, команда) == 0)
                return да;
        }
    ...
    export  бул поддерживается_ли(ткст команда)
    {
        if(this.поддерживаемыеФичи_.length == 0)
            return нет;
    ...
    /*****************************************************************************
     Изменено с версии: 0.99.8
     *****************************************************************************/
    export  проц завершиКомандуДанных(Сокет данные)
    {
    ...
    /*****************************************************************************
     Изменено с версии: 0.99.8
     *****************************************************************************/
    export  Сокет обработайКомандуДанных(ткст команда, ткст[] параметры...)
    {
        // Создаёт connection.
    ...
    export  ИнфОФайлеФтп[] ls(ткст путь = "")
    ...
    protected ИнфОФайлеФтп разборСтрокиСписка(ткст строка)
    {
        ...
        case нет:
            ...
            if(ШтампВрем.досвремя(r.сверь(0), инфо.изменён) is 0)
    ...
    protected ИнфОФайлеФтп разборСтрокиМлст(ткст строка)
    {
        ...
        if(filename_pos > 1)
        {
            ...
            foreach(ткст fact; temp_facts)
            {
                ...
                инфо.факты[Аски.вПроп(fact[0 .. поз])] = fact[поз + 1 .. fact.length];
            }
            ...
            if("type" in инфо.факты)
            {
                ...
                switch(Аски.вПроп(инфо.факты["type"]))
                {
                case "file":
                    инфо.тип = ПТипФайлаФтп.Файл;
                    break;
    ...
    export  ИнфОФайлеФтп дайИнфОФайле(ткст путь)
    in
    {
        assert(путь.length > 0);
    }
    ...
    export  проц помести(ткст путь, ткст локальн_файл,
    ...
    body
    {
        ...
        if(this.позРестарта_ > 0)
        {
            файл.сместись(this.позРестарта_);
            this.позРестарта_ = 0;
        }
        else {
     ...
     ********************************************************************************/
    export  проц помести(ткст путь, ИПотокВвода поток = пусто,
                                   ХодФтп ход = пусто, ПФорматФтп форматируй = ПФорматФтп.Образ)
    in
    {
        assert(путь.length > 0);
    ...
    /********************************************************************************
     ...
     форматируй =          что форматируй в_ шли the данные in
     ********************************************************************************/
    export  проц добавь(ткст путь, ИПотокВвода поток,
                                 ХодФтп ход = пусто, ПФорматФтп форматируй = ПФорматФтп.Образ)
    in
    {
        assert(путь.length > 0);
     ...
     **********************************************************************************/
    export  проц рестартСик(т_мера смещение)
    {
        сим[16] врем;
     ...
     ***********************************************************************************/
    export  проц размести(дол байты)
    in
    {
        assert(байты > 0);
    }
     ...
     **********************************************************************************/
    export  проц получи(ткст путь, ткст локальн_файл,
                                 ХодФтп ход = пусто, ПФорматФтп форматируй = ПФорматФтп.Образ)
    in
    {
        assert(путь.length > 0);
    ...
    /**********************************************************************************
     ...
     форматируй =          что форматируй в_ читай the данные in
     ***********************************************************************************/
    export  проц получи(ткст путь, ИПотокВывода поток,
                                 ХодФтп ход = пусто, ПФорматФтп форматируй = ПФорматФтп.Образ)
    in
    {
        assert(путь.length > 0);
    ...
    /*****************************************************************************
     Добавлено с версии: 0.99.8
     *****************************************************************************/
    export  ИПотокВвода ввод(ткст путь)
    {
        тип(ПФорматФтп.Образ);
        сокетДанных_ = this.обработайКомандуДанных("RETR", путь);
        return сокетДанных_;
    }
    ...
    /*****************************************************************************
     Добавлено с версии: 0.99.8
     *****************************************************************************/
    export  ИПотокВывода вывод(ткст путь)
    {
        тип(ПФорматФтп.Образ);
        сокетДанных_ = this.обработайКомандуДанных("STOR", путь);
        return сокетДанных_;
    }
}
