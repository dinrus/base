/*******************************************************************************

        Support for HTTP chunked I/O.

        See http://www.w3.org/Protocols/rfc2616/rfc2616-sec3.html

*******************************************************************************/
module net.http.ChunkStream;

private import  io.stream.Lines;
private import  io.device.Conduit, io.stream.Buffered;
private import  Целое = text.convert.Integer;

/*******************************************************************************

        Prefix each блок of данные with its length (in hex цифры) и добавь
        appropriate \r\n sequences. To подай the поток you'll need в_ use
        the терминируй() function и optionally предоставляет it with a обрвызов
        for writing trailing заголовки

*******************************************************************************/

export class ЧанкВывод : ФильтрВывода
{
    private БуфВывод вывод;

    /***********************************************************************

            Use a буфер belonging в_ our sibling, if one is доступно

    ***********************************************************************/

    export this (ИПотокВывода поток)
    {
        super (вывод = Бвыв.создай(поток));
    }

    /***********************************************************************

            Зап a чанк в_ the вывод, псеп_в_начале и postfixed in a
            manner consistent with the HTTP chunked перемести coding

    ***********************************************************************/

    export final override т_мера пиши (проц[] ист)
    {
        сим[8] врем =void;

        вывод.добавь (Целое.форматируй (врем, ист.length, "x"))
        .добавь ("\r\n")
        .добавь (ист)
        .добавь ("\r\n");
        return ист.length;
    }

    /***********************************************************************

            Зап a zero length чанк, trailing заголовки и a terminating
            blank строка

    ***********************************************************************/

    export final проц терминируй (проц delegate(БуфВывод) заголовки = пусто)
    {
        вывод.добавь ("0\r\n");
        if (заголовки)
            заголовки (вывод);
        вывод.добавь ("\r\n");
    }
}


/*******************************************************************************

        Parse hex цифры, и use the результатant размер в_ modulate requests
        for incoming данные. A размер чанка of 0 terminates the поток, so в_
        читай any trailing заголовки you'll need в_ предоставляет a delegate handler
        for receiving those

*******************************************************************************/

class ЧанкВвод : Строки!(сим)
{
    private alias проц delegate(ткст строка) Заголовки;

    private Заголовки         заголовки;
    private бцел            доступно;

    /***********************************************************************

            Prime the доступно размер чанка by reading и parsing the
            первый доступно строка

    ***********************************************************************/

    this (ИПотокВвода поток, Заголовки заголовки = пусто)
    {
        установи (поток);
        this.заголовки = заголовки;
    }

    /***********************************************************************

            Reset ЧанкВвод в_ a new ИПотокВвода

    ***********************************************************************/

    override ЧанкВвод установи (ИПотокВвода поток)
    {
        super.установи (поток);
        доступно = следщЧанк;
        return this;
    }

    /***********************************************************************

            Чит контент based on a previously разобрано размер чанка

    ***********************************************************************/

    final override т_мера читай (проц[] приёмн)
    {
        if (доступно is 0)
        {
            // terminated 0 - читай заголовки и пустой строка, per rfc2616
            ткст строка;
            while ((строка = super.следщ).length)
                if (заголовки)
                    заголовки (строка);
            return ИПровод.Кф;
        }

        auto размер = приёмн.length > доступно ? доступно : приёмн.length;
        auto читай = super.читай (приёмн [0 .. размер]);

        // проверь for следщ чанк заголовок
        if (читай != ИПровод.Кф && (доступно -= читай) is 0)
        {
            // используй trailing \r\n
            super.ввод.сместись (2);
            доступно = следщЧанк ();
        }

        return читай;
    }

    /***********************************************************************

            Чит и разбор другой размер чанка

    ***********************************************************************/

    private final бцел следщЧанк ()
    {
        ткст врем;

        if ((врем = super.следщ).ptr)
            return cast(бцел) Целое.разбор (врем, 16);
        return 0;
    }
}


/*******************************************************************************

*******************************************************************************/

debug (Chstream)
{
    import io.Console;
    import io.device.Array;

    проц main()
    {
        auto буф = new Массив(40);
        auto чанк = new ЧанкВывод (буф);
        чанк.пиши ("hello world");
        чанк.терминируй;
        auto ввод = new ЧанкВвод (буф);
        Квывод.поток.копируй (ввод);
    }
}
