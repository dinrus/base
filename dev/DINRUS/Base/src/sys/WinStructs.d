...
import tpl.com, sys.com.com, stdrus;
version = Динрус;
version = Dinrus;
version = Unicode;
version = ЛитлЭндиан;
...
extern (Windows)
{
	...
	alias GUID UUID, /*IID, CLSID, */FMTID, uuid_t, т_ууид;
}
 ...
 struct ГУИД 
	...
	export:
	 ...
	 static ГУИД opCall(бцел a, бкрат b, бкрат c, ббайт d, ббайт e, ббайт f, ббайт g, ббайт h, ббайт i, ббайт j, ббайт k) {
	   ...
	   * Возвращает: да, если другой равен этому экземпляру; иначе, false.
	   */
	...
	struct БЕЗОПМАС
	 ...
	 export:
	  ...
	  static БЕЗОПМАС* opCall(T)(T[] массив) {
		...
		static if (is(T : ткст)) alias шткст0 Тип;
		else                       alias T Тип;
	...
	export:
	  ...
	  /// Представляет smallest possible value.
	  static ДЕСЯТОК мин = { 0, 0, ОТРИЦ_ДЕСЯТОК, бцел.max, бцел.max, бцел.max };
	  /// Представляет largest possible value.
	  ...
	  /// Возвращает значение indicating whether this экземпляр and a specified экземпляр represent the same _value.
	...
	проц opAddAssign(ДЕСЯТОК d) { 
	 ...
	 ДЕСЯТОК opSub(ДЕСЯТОК d) {
			ДЕСЯТОК результат; 
			ДесВарвычти(*this, d, результат); 
		  return результат; 
		  } 
		 проц opSubAssign(ДЕСЯТОК d) { 
		   ДесВарвычти(*this, d, *this);
		 }
...
export:
	  ...
	  private static проц инициализуй(T)(ref ВАРИАНТ рез, T значение, бкрат тип = ТипВариант!(T))
	  {
		static if (is(T E == enum))
		{
		  инициализуй(рез, cast(E) значение, тип);
		}
		else
		{
		  рез = cast(ВАРИАНТ) значение;
		  if (тип != рез.вт)
			ИзмениТипВариантаДоп(рез, рез,  ДайЛокальНити(), ПВар.АльфаБул, тип);
		}
	  }
	  ...
	  {
		if (!пуст_ли) сотри();
		if (auto disp = ком_каст!(ИДиспетчер)(значение)) 
		{
		  депЗнач = disp;
	  ...
	  V значение(V)()
	  {
		...
		else static if (is(V == шткст0)) return бстрЗнач;
	  ...
	  ВАРИАНТ opSub(ВАРИАНТ тот) { 
			ВАРИАНТ результат; 
			вычтиВар(*this, тот, результат); 
			return результат; 
		  } 
	   проц opSubAssign(ВАРИАНТ тот) { 
		   if (!пуст_ли) сотри(); 
			вычтиВар(*this, тот, *this);
		  }  
...
///////////////////////////////////
struct ИНФОВЕРСИИОС_А//OSVERSIONINFOA
{
...
/////////////////////////////////
struct ИНФОВЕРСИИОС //OSVERSIONINFOW
{
	бцел      размИнфОВерсииОс;
	бцел      версияМажор;
	бцел      версияМинор;
...
/////////////////////////////////////
struct ИНФОВЕРСИИОСДОП_А //OSVERSIONINFOEXA
{
	бцел     размИнфОВерсииОс;
	бцел     версияМажор;
	бцел     версияМинор;
...
/////////////////////////////////////
struct ИНФОВЕРСИИОСДОП //OSVERSIONINFOEXW
{
	бцел      размИнфОВерсииОс;
	бцел      версияМажор;
	бцел      версияМинор;
...
struct FILE_NOTIFY_INFORMATION {
	...
	шткст0 FileName() { return &_FileName; }
}
...
struct IMAGE_RESOURCE_DIR_STRING_U {
	...
	шткст0 NameString() { return &_NameString; }
}
...
struct REPARSE_DATA_BUFFER {
	...
	union {
		...
		struct _SymbolicLinkReparseBuffer {
			...
			шткст0 PathBuffer() { return &_PathBuffer; }
		}
...
alias СТАТУС_ПИТАНИЯ_СИСТЕМЫ СТАТПИТОС,_SYSTEM_POWER_STATUS, SYSTEM_POWER_STATUS;
