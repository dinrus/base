...
ПУровеньИмперсонацииБезопасности, ПТокен, КЛАСС_ИНФОРМАЦИИ_ТОКЕНА,
 ПСтд, ПШирСим, ПСжатие, ПРежимКонсоли, ППозКурсора, ППозПотока,
  ПРеестр, ПКлючРег, ПФайл, ПСовмИспФайла, ПРежимФайла, ПРежСоздФайла,
   ПФлагКУДоп, ППриоритетНити, ППраваПроцесса, ПРежим_Адресации, ПИдЧП,
    ПКомРез, ПОшибка, ПЗагрФлаг, ПОкПерерис, ПФичаБезопмаса,
     ПТипВарианта, ПВинСокОш, ПТипСок, ППротокол, ПОпцияСокета,
      ПСемействоАдресов, ПЭкстрЗакрытиеСокета, ПФлагиСокета, ПИнАдр,
       ПИницКо, ПИмИнфо, АИ, ПСисМетрика, ПАтрыИнфосим, ПТипСоб,
        ПТипСобМыши, ПСостКнопкиМыши, ПСостКлУпр, ПСтильПера, ПЗвук,
         ППамять, ПФлагСооб, ПВиртКл, ППрокр, ПВидОкна, ПЦветУпрЭлта,
          ПСтильКласса, ПСлотПочты, ПСооб, ПФорматСооб, ПЯзык,
           ППодъяз, ПКодСтр, ПКонтекст, ПОкСооб, ПСозданиеПроцесса,
            ПКонтекстВып, ППолитикаИсключений, ПДиспачФлаг, ПСредство,
             ПВар, ПАспектЦУ, ТИМЕД, ПВидТипа, ПКонвВызова,
              ПВидФунк, ПВидВызова, ПВидПерем, ПФлагТипа, ПФлагФунк,
               ПФлагиПерем, ПВидДескр, ПВидСистемы, ПВидРег, ПВидИзм,
...
enum ПСозданиеПроцесса : бцел {
	Отладка               = 0x00000001,//
	ОтладкаТолькоЭтого     = 0x00000002,//
	СоздатьПодвешенным            = 0x00000004,//
	Откреплённый            = 0x00000008,//
	НоваяКонсоль          = 0x00000010,//
	НормКлассПриоритета       = 0x00000020,//
	БездействКлассПриоритета         = 0x00000040,//
	ВысшийКлассПриоритета         = 0x00000080,//
	РеальноВремКлассПриоритета     = 0x00000100,//
	НоваяГруппаПроцессов    = 0x00000200,//
	СредаЮникод  = 0x00000400,//
	Отдельный_WOW_VDM     = 0x00000800,//
	Совместный_WOW_VDM       = 0x00001000,//
	ФорсДос             = 0x00002000,//
	НижеНормКлассПриоритета = 0x00004000,//
	ВышеНормКлассПриоритета = 0x00008000,//
	Брейкэвэй_От_Джоб   = 0x01000000,//
	СПользоватПрофилем     = 0x02000000,//
	ДефолтнРежимОшибок   = 0x04000000,//
	БезОкна            = 0x08000000,//
	ПрофильПользователь                = 0x10000000,//
	ПрофильЯдро              = 0x20000000,//
	ПрофильСервер              = 0x40000000//
}
...
enum ПКлассКонтекста //CLSCTX
 {
	...
	ОбработчикВнутриПроцесса  =  0x2,//CLSCTX_INPROC_HANDLER
    ...
    ВнутриПроцесса           = СерверВнутриПроцесса | ОбработчикВнутриПроцесса,
    Сервер           = СерверВнутриПроцесса | МестныйСервер | УдалённыйСервер,
	СерверКонтекстаКласса = СерверВнутриПроцесса|МестныйСервер|УдалённыйСервер, //CLSCTX_SERVER
    Все              = СерверВнутриПроцесса | ОбработчикВнутриПроцесса | МестныйСервер | УдалённыйСервер
}
alias  ПКлассКонтекста ПКонтекстВып;