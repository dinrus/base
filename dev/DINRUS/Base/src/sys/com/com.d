...
export extern (C)
{
	 ...
	 ткст прогИдИзКлсид(ГУИД клсид) {
	  return ПрогИДИзКЛСИД(клсид);
	}

	ГУИД клсидИзПрогИд(ткст прогИд){  
	  return КЛСИДИзПрогИД_(прогИд);
	}

	 Исключение исклКомРез(цел кодОшибки) {
	  switch (кодОшибки) {
		case ПКомРез.Нереализовано:
		  return new Исключение("КОМ: Нереализованная функция");
		case ПКомРез.НеИнтерфейс:
		  return new Исключение("КОМ: Неудачное преобразование типа");
		case ПКомРез.Ук:
		  return new Исключение("КОМ: Ссылка на пустое");
		case ПКомРез.НетДоступа:
		 return new Исключение("КОМ: Недостаточно право для доступа");
		case ПКомРез.ВнеПамяти:
		   return new ВнеПамИскл();
		  case ПКомРез.НевернАрг:
		  return new Исключение("КОМ: Использован неверный параметр");
		default:
	  }
...
export extern(C){
	 ...
	 */
	 ткст бткстВТкст(шим* s, бул высвободить = true)
	 {
	  ...
	  ткст рез = toUTF8(s[0 .. длин]);
	  /*цел кб = WideCharToMultiByte(CP_UTF8, 0, s, длин, null, 0, null, null);
	  сим[] рез = new сим[кб];
	  WideCharToMultiByte(CP_UTF8, 0, s, длин, рез.ptr, кб, null, null);*/
	...
	Инкогнито создайЭкземплярКо(ГУИД клсид,Инкогнито внешний, ПКонтекстВып контекст,ГУИД иид) {
	 Инкогнито рез;
	  if (УД(СоздайЭкземплярКо(клсид, внешний, cast(бцел)контекст, иид, tpl.com.возврзнач(рез))))
		return рез;
	  return null;
	}
 ...
 */
export extern(D) class КомОбъект
 {
