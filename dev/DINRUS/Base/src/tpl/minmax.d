...
/**
   Вычисляет, приблизительно равен ли $(D lhs)  $(D правткт),
   учитывая максимальную относительную разность $(D максОтнРазность) и
   максимальную абсолютную разность $(D максАбсРазность).
 */
бул приблизитРавны(T, U, V)(T lhs, U правткт, V максОтнРазность, V максАбсРазность = 0)
{
    static if (массив_ли!(T)) {
        final n = lhs.length;
        static if (массив_ли!(U)) {
            // Two arrays
            assert(n == правткт.length);
            for (бцел i = 0; i != n; ++i) {
                if (!приблизитРавны(lhs[i], правткт[i], максОтнРазность, максАбсРазность))
                    return нет;
            }
        } else {
            // lhs is массив, правткт is number
            for (бцел i = 0; i != n; ++i) {
                if (!приблизитРавны(lhs[i], правткт, максОтнРазность, максАбсРазность))
                    return нет;
            }
        }
        return да;
        ...
        static if (массив_ли!(U)) {
            // lhs is number, правткт is массив
            return приблизитРавны(правткт, lhs, максОтнРазность);
        } else {
            // two numbers
            ...
            return fabs((lhs - правткт) / правткт) <= максОтнРазность
                || максАбсРазность != 0 && fabs(lhs - правткт) < максАбсРазность;
        }
    }
}

/**
   Returns $(D приблизитРавны(lhs, правткт, 0.01)).
 */
бул приблизитРавны(T, U)(T lhs, U правткт) {
    return приблизитРавны(lhs, правткт, 0.01);
}
+/