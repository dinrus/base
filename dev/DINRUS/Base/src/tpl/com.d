...
template ком_каст_реализ(T, ППолитикаИсключений политика)
 {

  T ком_каст_реализ(U)(U объ) {
      ...
      static if (тип != ПТипВарианта.Проц) {
        ...
        if (УД(ИзмениТипВариантаДоп(temp, объ,  ДайЛокальНити(), ПВар.АльфаБул, тип))) {
          ...
          with (temp) {
            ...
            else static if (тип == ПТипВарианта.Инкогнито) return ком_каст_реализ(объ.инкЗнач);
            else static if (тип == ПТипВарианта.Диспетчер) return ком_каст_реализ(объ.депЗнач);
            else return T.init;
          }
        }
        static if (политика == ППолитикаИсключений.Выводить)
...
template ком_каст(T) { 
 ...
 alias ком_каст_реализ!(T, ППолитикаИсключений.НеВыводить) ком_каст;}
...
template ком_каст_безоп(T) {  alias ком_каст_реализ!(T, ППолитикаИсключений.Выводить) ком_каст_безоп;}
...
export extern(D) проц выполниТерминатор(Объект объ) 
{
  if (объ) 
  {
