...
alias проц  function( ткст файл, т_мера строка, ткст msg = null ) TипПроверОбр;
alias ИнфоОтслежИскл function( ук ptr = null ) TипСледОбр;

  TипПроверОбр   проверОбр   = null;
  TипСледОбр    трОбр    = null;
...
export extern (C)
{
	проц ошибка(ткст сооб, ткст файл = ткст.init, дол  строка = дол.init)
		{
		ткст soob = сооб~"\nФайл: "~файл~"\nСтрока:"~std.string.toString(строка);
		ОкноСооб(null, soob, cast(ткст)"Исключение Динрус :", ПСооб.Ошибка|ПСооб.Поверх);
