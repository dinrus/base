...
/*******************************************************************************
    ...
    encodes данные и returns как ASCII hex ткст.
...
body
{
    ...
    foreach (ббайт j; данные) {
        буф[i++] = _encodeTable[j >> 4];
        буф[i++] = _encodeTable[j & 0b0000_1111];
    }
...
body
{
    ...
    foreach (c; данные) {
        ...
        if (знач & 0b1000_0000)
            continue;
        if (even) {
            буф[i] = знач << 4; // Store знач in high for биты
        } else {
            буф[i] |= знач;     // OR-in low 4 биты,
