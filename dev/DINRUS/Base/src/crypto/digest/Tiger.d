...
export class Тигр : МерклеДамгард
{
        ...
        ***********************************************************************/
        ...
        export override бцел размерДайджеста() {return 24;}
        ...
        ***********************************************************************/
        ...
        export  override проц трансформируй(ббайт[] ввод)
        {
                ...
                for(i = 0; i < npass; i++) {
                        ...
                        пароль(a,b,c,x,(i==0)?5:(i==1)?7:9);
                        tmpa = a; a = c; c = b; b = tmpa;
                }

                контекст[0] = a ^ контекст[0];
                контекст[1] = b - контекст[1];
                контекст[2] = c + контекст[2];
        ...
        ***********************************************************************/
        ...
        {
                c ^= x;
                a -= t1[получиБайт(c,0)] ^ t2[получиБайт(c,2)] ^ t3[получиБайт(c,4)] ^ t4[получиБайт(c,4,2)];
                b += t4[получиБайт(c,1)] ^ t3[получиБайт(c,3)] ^ t2[получиБайт(c,4,1)] ^ t1[получиБайт(c,4,3)];
                b *= mul;
        }
        ...
        ***********************************************************************/
        ...
        {
                округли(a,b,c,x[0],mul);
                округли(b,c,a,x[1],mul);
                округли(c,a,b,x[2],mul);
                округли(a,b,c,x[3],mul);
                округли(b,c,a,x[4],mul);
                округли(c,a,b,x[5],mul);
                округли(a,b,c,x[6],mul);
                округли(b,c,a,x[7],mul);
        }
        ...
        ***********************************************************************/
        ...
        private static бдол[] t1() { return таблица[0..256]; }
        private static бдол[] t2() { return таблица[256..512]; }
        private static бдол[] t3() { return таблица[512..768]; }
        private static бдол[] t4() { return таблица[768..1024]; }
}
...
debug(UnitTest)
{
        ...
        static ткст[] strings = [
                ...
                "Тигр - A Быстрый Нов Хэш Function, by Ross Anderson и Eli Biham",
                "Тигр - A Быстрый Нов Хэш Function, by Ross Anderson и Eli Biham, proceedings of Быстрый Software Encryption 3, Camмост.",
