...
export class Сальса20 : ШифрПоток
{
    protected
    {
        // Constants
        static ббайт[] сигма = cast(ббайт[])"расширь 32-байт ключ",
                       тау = cast(ббайт[])"расширь 16-байт ключ";
        
        // Counter indexes (добавьed for ЧаЧа)            
        бцел i0, i1;
   ...
   export  this()
    {
        ...
        ключПоток = new ббайт[64];
        
        i0 = 8;
        i1 = 9;
    }
    
    export проц иниц(бул зашифруй, Параметры парамы)
    {
        ПараметрыIV ivParams = cast(ПараметрыIV)парамы;
        
        if (!ivParams)
            не_годится(имя()~": В параметры инициализации должен входить IV. (используйте ПараметрыIV)");
                    
        СимметричныйКлюч парамыКлюча = cast(СимметричныйКлюч)ivParams.параметры;
                    
        ббайт[] iv = ivParams.iv,
                ключ = парамыКлюча.ключ;
            
        if (ключ)
        {
            if (ключ.length != 16 && ключ.length != 32)
                не_годится(имя()~": Неверная длина ключа. (требуется 16 или 32 байт)");
        ...
        if (!рабочийКлюч)
            не_годится(имя()~": Ключ не установлен.");
   ...
   export  ббайт верниБбайт(ббайт ввод)
    {
        ...
        if (индекс == 0) {
            словоСальса20ВБбайт(состояние, ключПоток);
            состояние[i0]++;
            if (!состояние[i0])
                состояние[i1]++;
            // As in djb's, changing the IV после 2^70 байты is the пользователь's responsibility
            // lol glwt
        }
        
        ббайт результат = (ключПоток[индекс]^ввод);
        индекс = (индекс + 1) & 0x3f;
        
        return результат;
    }
    
   export  бцел обнови(проц[] ввод_, проц[] вывод_)
    {
        if (!_инициализован)
            не_годится(имя()~": Шифр не инициализован");
        ...
        for (цел i = 0; i < ввод.length; i++)
        {
            ...
            вывод[i] = (ключПоток[индекс]^ввод[i]);
            индекс = (индекс + 1) & 0x3f; 
        }
    ...
    protected проц установиКлюч()
    {
        ...
        состояние[1] = БайтКонвертер.ЛитлЭндиан.в_!(бцел)(рабочийКлюч[0..4]);
        состояние[2] = БайтКонвертер.ЛитлЭндиан.в_!(бцел)(рабочийКлюч[4..8]);
        состояние[3] = БайтКонвертер.ЛитлЭндиан.в_!(бцел)(рабочийКлюч[8..12]);
        состояние[4] = БайтКонвертер.ЛитлЭндиан.в_!(бцел)(рабочийКлюч[12..16]);
        
        if (рабочийКлюч.length == 32)
        {
            константы = сигма;
            смещение = 16;
        } else
        ...
        состояние[11] = БайтКонвертер.ЛитлЭндиан.в_!(бцел)(рабочийКлюч[смещение..смещение+4]);
        состояние[12] = БайтКонвертер.ЛитлЭндиан.в_!(бцел)(рабочийКлюч[смещение+4..смещение+8]);
        состояние[13] = БайтКонвертер.ЛитлЭндиан.в_!(бцел)(рабочийКлюч[смещение+8..смещение+12]);
        состояние[14] = БайтКонвертер.ЛитлЭндиан.в_!(бцел)(рабочийКлюч[смещение+12..смещение+16]);
        состояние[ 0] = БайтКонвертер.ЛитлЭндиан.в_!(бцел)(константы[0..4]);
        состояние[ 5] = БайтКонвертер.ЛитлЭндиан.в_!(бцел)(константы[4..8]);
        состояние[10] = БайтКонвертер.ЛитлЭндиан.в_!(бцел)(константы[8..12]);
        состояние[15] = БайтКонвертер.ЛитлЭндиан.в_!(бцел)(константы[12..16]);
    }
    
    protected проц установиIV()
    {
        состояние[6] = БайтКонвертер.ЛитлЭндиан.в_!(бцел)(рабочийIV[0..4]);
        состояние[7] = БайтКонвертер.ЛитлЭндиан.в_!(бцел)(рабочийIV[4..8]);
    ...
    debug (UnitTest)
    {
        unittest
        {
            ...
            Сальса20 s20 = new Сальса20();
            ббайт[] буфер = new ббайт[64];
            ткст результат;
            for (цел i = 0; i < ключи_теста.length; i++)
            {
                СимметричныйКлюч ключ = new СимметричныйКлюч(БайтКонвертер.раскодируйГекс(ключи_теста[i]));
                ПараметрыIV парамы = new ПараметрыIV(ключ, БайтКонвертер.раскодируйГекс(test_ivs[i]));
                
                // Encryption
                s20.иниц(да, парамы);
                s20.обнови(БайтКонвертер.раскодируйГекс(простыеТексты_теста[i]), буфер);
                результат = БайтКонвертер.кодируйГекс(буфер);
                assert(результат == тексты_тестаШифра[i],
                        s20.имя()~": ("~результат~") != ("~тексты_тестаШифра[i]~")");           
                
                // Decryption
                s20.иниц(нет, парамы);
                s20.обнови(БайтКонвертер.раскодируйГекс(тексты_тестаШифра[i]), буфер);
                результат = БайтКонвертер.кодируйГекс(буфер);
                assert(результат == простыеТексты_теста[i],
                        s20.имя()~": ("~результат~") != ("~простыеТексты_теста[i]~")");
            }   
        }
    }
}
