...
/** Implementation of ЧаЧа designed by Daniel J. Bernstein. */
export class ЧаЧа : Сальса20
{
   export ткст имя()
    {
        return "ЧаЧа";
   ...
   export this()
    ...
    protected проц установиКлюч()
    {
        ...
        состояние[4] = БайтКонвертер.ЛитлЭндиан.в_!(бцел)(рабочийКлюч[0..4]);
        состояние[5] = БайтКонвертер.ЛитлЭндиан.в_!(бцел)(рабочийКлюч[4..8]);
        состояние[6] = БайтКонвертер.ЛитлЭндиан.в_!(бцел)(рабочийКлюч[8..12]);
        состояние[7] = БайтКонвертер.ЛитлЭндиан.в_!(бцел)(рабочийКлюч[12..16]);
        ...
        состояние[ 8] = БайтКонвертер.ЛитлЭндиан.в_!(бцел)(рабочийКлюч[смещение..смещение+4]);
        состояние[ 9] = БайтКонвертер.ЛитлЭндиан.в_!(бцел)(рабочийКлюч[смещение+4..смещение+8]);
        состояние[10] = БайтКонвертер.ЛитлЭндиан.в_!(бцел)(рабочийКлюч[смещение+8..смещение+12]);
        состояние[11] = БайтКонвертер.ЛитлЭндиан.в_!(бцел)(рабочийКлюч[смещение+12..смещение+16]);
        состояние[ 0] = БайтКонвертер.ЛитлЭндиан.в_!(бцел)(константы[0..4]);
        состояние[ 1] = БайтКонвертер.ЛитлЭндиан.в_!(бцел)(константы[4..8]);
        состояние[ 2] = БайтКонвертер.ЛитлЭндиан.в_!(бцел)(константы[8..12]);
        состояние[ 3] = БайтКонвертер.ЛитлЭндиан.в_!(бцел)(константы[12..16]);
    }
    
    protected проц установиIV()
    {
        состояние[12] = состояние[13] = 0;
        состояние[14] = БайтКонвертер.ЛитлЭндиан.в_!(бцел)(рабочийIV[0..4]);
        состояние[15] = БайтКонвертер.ЛитлЭндиан.в_!(бцел)(рабочийIV[4..8]);
    }
    ...
    debug (UnitTest)
    {
        unittest
        {
            ...
            for (цел i = 0; i < ключи_теста.length; i++)
            {
                ...
                // Encryption
                cc.init(да, парамы);
                cc.обнови(БайтКонвертер.раскодируйГекс(простыеТексты_теста[i]), буфер);
                результат = БайтКонвертер.кодируйГекс(буфер);
                assert(результат == тексты_тестаШифра[i],
                        cc.имя()~": ("~результат~") != ("~тексты_тестаШифра[i]~")");           
                
                // Decryption
                cc.init(нет, парамы);
                cc.обнови(БайтКонвертер.раскодируйГекс(тексты_тестаШифра[i]), буфер);
                результат = БайтКонвертер.кодируйГекс(буфер);
                assert(результат == простыеТексты_теста[i],
                        cc.имя()~": ("~результат~") != ("~простыеТексты_теста[i]~")");
            }   
        }
    }
}
