...
export class Камбала : ШифрБлок
{
    ...
    export проц иниц(бул зашифруй, СимметричныйКлюч парамыКлюча)
    {
        ...
        if (длин < MIN_KEY_SIZE || длин > MAX_KEY_SIZE)
            не_годится(имя()~": Неверная длина ключа (требует 4-56 байты)");
    ...
    export override бцел обнови(проц[] ввод_, проц[] вывод_)
    {
        ...
        if (ввод.length < BLOCK_SIZE)
            не_годится (имя()~": Вводный буфер слишком краток");

        if (вывод.length < BLOCK_SIZE)
            не_годится (имя()~": Выводной буфер слишком краток");
...
/** Some Камбала тест vectors из_ Schneier's site. */
debug (UnitTest)
{
    unittest
    {
