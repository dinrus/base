...
export struct МассивБит
{
     ...
     */
    export static МассивБит opCall( бул[] биты )
    {
        МассивБит врем;
   ...
   export  МассивБит dup()
     ...
     *  assert(ba2[0] == нет);
     */
    export МассивБит opSliceAssign(МассивБит правткт)
    in
    {
        assert(правткт.длин == длин);
    }
    body
    {
        т_мера mDim=длин/32;
        ptr[0..mDim] = правткт.ptr[0..mDim];
        цел rest=cast(цел)(длин & cast(т_мера)31u);
        if (rest)
        {
            бцел маска=(~0u)<<rest;
            ptr[mDim]=(правткт.ptr[mDim] & (~маска))|(ptr[mDim] & маска);
        }
        return *this;
    }
     ...
     */
    export проц иниц( проц[] мишень, т_мера члобит )
    in
    {
...
Ldone:
     ...
     */
    export цел opApply( цел delegate(ref бул) дг )
    {
        цел результат;
    ...
    /** описано ранее */
   export  цел opApply( цел delegate(ref т_мера, ref бул) дг )
    {
        цел результат;
     ...
     */
   export  цел opEquals( МассивБит правткт )
    {
        if( this.длина != правткт.длина )
            return 0; // not equal
        бцел* p1 = this.ptr;
        бцел* p2 = правткт.ptr;
     ...
     */
    export цел opCmp( МассивБит правткт )
    {
        auto длин = this.длина;
        if( правткт.длина < длин )
            длин = правткт.длина;
        бцел* p1 = this.ptr;
        бцел* p2 = правткт.ptr;
        т_мера n = длин / 32;
        ...
        return ((this.длина<правткт.длина)?-1:((this.длина==правткт.длина)?0:1));
    }
     ...
     *
     * Параметры:
     *  поз = The требуется индекс позиция.
     *
     * In:
     ...
     *
     * Параметры:
     *  правткт = The Массив with which в_ выполни the bitwise and operation.
     *
     * In:
     *  правткт.длина must equal the длина of this Массив.
     *
     * Возвращает:
     *  A new Массив which is the результат of a bitwise and with this Массив and
     *  the supplied Массив.
     */
   export  МассивБит opAnd( МассивБит правткт )
    in
    {
        assert( длин == правткт.длина );
    }
    body
    {
        auto цразм = this.цразм();
        ...
        результат.длина = длин;
        for( т_мера i = 0; i < цразм; ++i )
            результат.ptr[i] = this.ptr[i] & правткт.ptr[i];
        return результат;
    }
     ...
     *
     * Параметры:
     *  правткт = The Массив with which в_ выполни the bitwise or operation.
     *
     * In:
     *  правткт.длина must equal the длина of this Массив.
     *
     ...
     *  the supplied Массив.
     */
    export МассивБит opOr( МассивБит правткт )
    in
    {
        assert( длин == правткт.длина );
    }
     ...
     *
     * Параметры:
     *  правткт = The Массив with which в_ выполни the bitwise xor operation.
     *
     * In:
     *  правткт.длина must equal the длина of this Массив.
     *
     ...
     *  the supplied Массив.
     */
   export  МассивБит opXor( МассивБит правткт )
    in
    {
        assert( длин == правткт.длина );
    }
     ...
     *
     * Параметры:
     *  правткт = The Массив with which в_ выполни the subtraction operation.
     *
     * In:
     *  правткт.длина must equal the длина of this Массив.
     *
     ...
     */
    export МассивБит opSub( МассивБит правткт )
    in
    {
        assert( длин == правткт.длина );
    }
     ...
     *
     * Параметры:
     *  правткт = The Массив with which в_ выполни the concatenation operation.
     *
     * Возвращает:
    ...
    МассивБит opCat( бул правткт )
    {
        ...
        результат[длин] = правткт;
        return результат;
    }


    /** описано ранее */
    export МассивБит opCat_r( бул lhs )
    {
        МассивБит результат;
    ...
    /** описано ранее */
   export  МассивБит opCat( МассивБит правткт )
    {
        МассивБит результат;
     ...
     */
    export бул opIndexAssign( бул b, т_мера поз )
    in
    {
        assert( поз < длин );
    }
     ...
     *  A shallow копируй of this Массив.
     */
    export МассивБит opAndAssign( МассивБит правткт )
    in
    {
        assert( длин == правткт.длина );
    }
     ...
     *  A shallow копируй of this Массив.
     */
    export МассивБит opOrAssign( МассивБит правткт )
    in
    {
        assert( длин == правткт.длина );
    }
     ...
     *  A shallow копируй of this Массив.
     */
   export  МассивБит opXorAssign( МассивБит правткт )
    in
    {
        assert( длин == правткт.длина );
    }
     ...
     *  A shallow копируй of this Массив.
     */
    export МассивБит opSubAssign( МассивБит правткт )
    in
    {
        assert( длин == правткт.длина );
    }
    ...
    /** описано ранее */
    export МассивБит opCatAssign( МассивБит правткт )
    {
        auto istart = длин;
        длина = длин + правткт.длина;
        for( auto i = istart; i < длин; ++i )
            (*this)[i] = правткт[i - istart];
        return *this;
    }
