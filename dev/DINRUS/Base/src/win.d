...
export extern (C) void консЦелое (ulong i)
{
        char[25] tmp = void;

        консТкст (ulongToUtf8 (tmp, i));
}
...
export extern (C) void консТкст (char[] s)
{
        version (Windows)
                {
                int  mode, count;
...
export extern (C) void консТкстОш (char[] s)
{
        version (Windows)
                {
                int  mode, count;
...
export extern (C) struct БКонсоль
{
export :
        alias новстр opCall;
        alias выдай    opCall;
    
        /// выдать строку utf8 на консоль
        ...
        /// выдать беззначное целое на консоль
        БКонсоль выдай (бдол i)
        {
                консЦелое (i);
                return *this;
        }

        /// выдать новстр на консоль
        БКонсоль новстр ()
        {
                version (Windows)
                         const КС = "\r\n";
...
public static БКонсоль консоль;

export extern(C)
{
  проц ошибнс(ткст ткт){консоль.ош(ткт~"\r\n");}
	проц скажибд(бдол ткт){консоль(ткт);}
	проц скажинсбд(бдол ткт){консоль(ткт).нс;}
...
enum Цвет : цел
{
    ...
    Белый         = Яркий | СветлоСерый,
      ...
      export extern(C)  проц устЦветКонсоли(Цвет цвет)
        {
            const бцел БелыйПП = cast(бцел)( ПТекстКонсоли.КрасныйПП | ПТекстКонсоли.ЗелёныйПП | ПТекстКонсоли.СинийПП);
            ПТекстКонсоли атр = инфОбКонсБуф.атрибуты;
            auto атры = (атр & ~(БелыйПП | ПТекстКонсоли.ИнтПП)) |
                   ((цвет & Цвет.Красный)    ? ПТекстКонсоли.КрасныйПП   : 0) |
                   ((цвет & Цвет.Зелёный)  ? ПТекстКонсоли.ЗелёныйПП  : 0) |
                   ((цвет & Цвет.Синий)   ? ПТекстКонсоли.СинийПП  : 0) |
                   ((цвет & Цвет.Яркий) ? ПТекстКонсоли.ИнтПП : 0);
            УстановиАтрибутыТекстаКонсоли(КОНСВЫВОД, cast(ПТекстКонсоли) атры);
        }