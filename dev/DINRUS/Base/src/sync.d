 ...
 version(D_InlineAsm_X86) {
    ...
    in {
        ...
        static if( T.sizeof == byte.sizeof ) {
            ...
            static assert( false, "Назначен неверный тип шаблона, 8 байтов в 32 битном режиме: "~T.stringof );
        }
        else
        {
            static assert( false, "Назначен неверный тип шаблона: "~T.stringof );
        }
    }
} else version (D_InlineAsm_X86_64){
    T атомнаяПерестановка( T )( inout T знач, T новзнач )
...
version(D_InlineAsm_X86) {
    ...
    in {
        ...
        {
            static assert( false, "Указан неверный тип шаблона: "~T.stringof );
        }
    }
} else version (D_InlineAsm_X86_64){
    T atomicCAS( T )( ref   T знач, T новзнач, T равно )
...
проц атомноеСохранение(T)(ref   T знач, T новЗнач)
in {
        assert( атомныеЗначенияПравильноРазмещены!(T)( cast(т_мера) &знач ), "неправильная раскладка" );
        static assert(УкНаКласс!(T).sizeof<=т_мера.sizeof,"неверный размер для "~T.stringof);
} body {
    volatile знач=новЗнач;
}
    ...
    T атомнаяПрибавка(T,U=T)(ref   T знач, U incV_){
        ...
        static if (целыйТип_ли!(T)||указательИлиКласс_ли!(T)){
            ...
            static if (T.sizeof==1){
                ...
                static assert(0,"Неподдерживаемый размер типа:"~T.stringof);
            }
            return рез;
        } else {
            return атомнаяОп(знач,delegate T(T a){ return a+incV; });
...
T флагУст(T)(ref   T флаг,T новЗнач){
    барьерПамяти!(false,strictFences,false,true)();
    return атомнаяПерестановка(флаг,новЗнач);
}
...
export:
    ...
    this( Мютекс m )
    {
             m_blockLock = СоздайСемафорА( null, 1, 1, null );
            if( m_blockLock == m_blockLock.init )
                throw new ИсключениеСинх( "Не удаётся инициализировать условие" );
            scope(failure) ЗакройДескр( m_blockLock );

            m_blockQueue = СоздайСемафорА( null, 0, цел.max, null );
            if( m_blockQueue == m_blockQueue.init )
                throw new ИсключениеСинх( "Не удаётся инициализировать условие" );
            scope(failure) ЗакройДескр( m_blockQueue );
    ...
    ~this()
    {
            BOOL rc = ЗакройДескр( m_blockLock );
            assert( rc, "Не могу удалить условие" );
            rc = ЗакройДескр( m_blockQueue );
            assert( rc, "Не могу удалить условие" );
...
private:
        ...
        bool ждатьПоВремени( DWORD timeout )
        {
            ...
            rc = ЖдиОдинОбъект( m_blockLock, БЕСК );
            assert( rc == ЖДИ_ОБЪЕКТ_0 );
            ...
            rc = ЖдиОдинОбъект( m_blockQueue, timeout );
            assert( rc == ЖДИ_ОБЪЕКТ_0 || rc == ЖДИ_ТАЙМАУТ );
            bool timedOut = (rc == ЖДИ_ТАЙМАУТ);
            ...
            if( numSignalsLeft == 1 )
            {
                ...
                {
                    rc = ЖдиОдинОбъект( m_blockQueue, БЕСК );
                    assert( rc == ЖДИ_ОБЪЕКТ_0 );
                }
                // open the gate
                rc = ReleaseSemaphore( m_blockLock, 1, null );
        ...
        КРИТСЕКЦ    m_unblockLock;  // internal mutex/CS
...
export:
    ...
    this( бцел счёт = 0 )
    {
            ...
            m_hndl = СоздайСемафорА( null, счёт, цел.max, null );
    ...
    проц жди()
    {
            ...
            DWORD rc = ЖдиОдинОбъект( m_hndl, БЕСК );
            if( rc != ЖДИ_ОБЪЕКТ_0 )
    ...
    body
    {
            ...
            if( период > MAX_WAIT_MILLIS )
                период = MAX_WAIT_MILLIS;
            switch( ЖдиОдинОбъект( m_hndl, cast(бцел) период ) )
            {
            case ЖДИ_ОБЪЕКТ_0:
                return true;
            case ЖДИ_ТАЙМАУТ:
...
begin
  ...
  if WaitResult = ЖДИ_ТАЙМАУТ then
  ...
  begin
    ...
    if (WaitResult >= WAIT_ABANDONED_0) and (WaitResult < WAIT_ABANDONED_0 + cHandles) then
      ...
      lpdwIndex := WaitResult - ЖДИ_ОБЪЕКТ_0;
  end;
end;
