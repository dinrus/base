...
проц создайПроцесс( )
{
    ...
    if( !СоздайПроцесс( пусто,   // Без названия модуля (использовать комстроку). 
        ...
        &пи )             // Указатель на структуру PROCESS_INFORMATION.
    ) 
    {
    ...
    ЖдиОдинОбъект( пи.процесс, БЕСК );

    // Закрытие хендлов процесса и нити. 
    ЗакройДескр( пи.процесс );
    ЗакройДескр( пи.нить );
}
+/
/**
...
extern (C) void gc_term()
 ...
 void new_finalizer(ук укз, bool dummy)
	 ...
	 extern (C) void _d_callinterfacefinalizer(ук укз)
	{
		...
		if (укз)
		{
		Interface *пи = **cast(Interface ***)укз;
		Object o = cast(Object)(укз - пи.offset);
