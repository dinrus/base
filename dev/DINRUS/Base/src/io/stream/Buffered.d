...
public alias БуфВвод  Бввод;        /// краткий псевдоним
...
export:
       ...
       protected проц инвариант()
        ...
        static БуфВвод создай (ИПотокВвода поток)
        {
		
		debug эхо("БуфВвод.создай\n");
                auto исток = поток;
                ...
                return создайБВвод (поток, провод.размерБуфера);
				debug эхо("БуфВвод 8\n");
        }
...
export extern(C) БуфВвод создайБВвод(ИПотокВвода п, т_мера ёмкость = 8192){return new БуфВвод(п, ёмкость);}
       ...
       static БуфВывод создай (ИПотокВывода поток)
        {
                ...
                return создайБВывод (поток, провод.размерБуфера);
        }
...
export extern(C) БуфВывод создайБВывод(ИПотокВывода п, т_мера ёмкость = 8192){return new БуфВывод(п,ёмкость); }
