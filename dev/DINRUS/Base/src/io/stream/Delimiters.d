...
/*******************************************************************************
        ...
        Чтобы усилить желаемый аспект только чтения, этот код должен бы
        включать повторное копирование или же компилятор должен поддерживать
        массивы только для чтения.

        Смотрите Строки, Образцы, Кавычки.
...
debug(UnitTest)
{
        ...
        unittest
        {
                auto p = new Разграничители!(сим) (", ", new io.device.Array.Массив("blah"));
        }
}
