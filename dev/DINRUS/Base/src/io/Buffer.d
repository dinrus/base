...
+/
    ...
    export this (ИПотокВвода поток, т_мера ёмкость)
    {
        this (ёмкость);
        ввод = поток;
    }

    export this (ИПотокВывода поток, т_мера ёмкость)
    {
        this (ёмкость);
        вывод = поток;
    }
    ...
    export this (т_мера ёмкость = 0)
    {
	ббайт[] парам = ббайтмас(ёмкость);
        устКонтент (парам, 0);
    }


    export this (проц[] данные)
    {
        устКонтент (данные, данные.length);
    }


    export this (проц[] данные, т_мера читаемо)
    {
        устКонтент (данные, читаемо);
    }


    export static ИБуфер совместно (ИПотокВвода поток, т_мера размер = т_мера.max)
    {
    ...
    export static ИБуфер совместно (ИПотокВывода поток, т_мера размер = т_мера.max)
    {
        auto b = cast(ИБуферированный) поток;
        if (b)
            return b.буфер;
    ...
    export ИБуфер устКонтент (проц[] данные)
    {
        return устКонтент (данные, данные.length);
    }

    export ИБуфер устКонтент (проц[] данные, т_мера читаемо)
    {
        this.данные = данные;
    ...
    export final проц[] opSlice (т_мера старт, т_мера конец)
    {
        assert (старт <= протяженность && конец <= протяженность && старт <= конец);
        return данные [старт .. конец];
    }

    export проц[] срез (т_мера размер, бул съешь = да)
    {
        if (размер > читаемо)
        {
            if (бввод is пусто)
    ...
    export т_мера заполни (проц[] приёмн)
    {
        т_мера длин = 0;
    ...
    export проц[]  читайРовно (ук приёмн, т_мера байты)
    {
    ...
    export ИБуфер добавь (проц[] ист)
    {
        return добавь (ист.ptr, ист.length);
    }


    export ИБуфер добавь (ук ист, т_мера длина)
    {
    ...
    export ИБуфер добавь (ИБуфер другой)
    {
        return добавь (другой.срез);
    }


    export проц потреби (проц[] x)
    {
        добавь (x);
    }


    export бул пропусти (цел размер)
    {
        if (размер < 0)
        {
            размер = -размер;
    ...
    export дол сместись (дол смещение, Якорь старт = Якорь.Нач)
    {
        if (старт is Якорь.Тек)
        {
            ...
            if (bpos >= 0 && bpos < this.предел)
            {
                ...
                return 0;
                //return провод.позиция - ввод.читаемо;
            }
    ...
    export бул следщ (т_мера delegate (проц[]) скан)
    {
        while (читатель(скан) is ИПровод.Кф)
            ...
            if (бввод)
            {
                if (позиция && можноСжать)
                    ...
                    if (записываемо is 0 && расширь(0) is 0)
    ...
    export т_мера читаемо ()
    {
        return протяженность - индекс;
    }


    export т_мера записываемо ()
    {
        return дименсия - протяженность;
    }


    export final т_мера резервируй (т_мера пространство)
    {
    ...
    export т_мера писатель (т_мера delegate (проц[]) дг)
    {
    ...
    export т_мера читатель (т_мера delegate (проц[]) дг)
    {
    ...
    export т_мера заполни (ИПотокВвода ист)
    {
        if (ист is пусто)
            return ИПровод.Кф;
        /+
        // Сброс здесь не нужен, так как мы только заполняем!
        if (читаемо is 0 && можноСжать)
            индекс = протяженность = 0;  // то же что и очисть(), бев цепи вызовов
        else if (записываемо is 0)
            return 0;
        +/
        return писатель (&ист.читай);
    }

    export final т_мера дренируй (ИПотокВывода приёмн)
    {
        if (приёмн is пусто)
            return ИПровод.Кф;
    ...
    export бул упрости (т_мера length)
    {
    ...
    export т_мера предел ()
    {
        return протяженность;
    }


    export т_мера ёмкость ()
    {
        return дименсия;
    }


    export т_мера позиция ()
    {
        return индекс;
    }


    export ИБуфер устПровод (ИПровод провод)
    {
        бвывод = провод;
        бввод = провод;
        return this;
    ...
    export final ИБуфер вывод (ИПотокВывода бвывод)
    {
        this.бвывод = бвывод;
        return this;
    }

    export final ИБуфер ввод (ИПотокВвода бввод)
    {
        this.бввод = бввод;
        return this;
    }

    export проц[] дайКонтент ()
    {
        return данные;
    }

    export проц копируй (проц *ист, т_мера размер)
    {
    ...
    export т_мера расширь (т_мера размер)
    {
        return записываемо;
    }
    ...
    export ИБуферВвода бвхо ()
    {
        return пусто;
    }

    export ИБуферВывода бвых ()
    {
        return пусто;
    }
    ...
    export final проц ошибка (ткст сооб)
    {
        throw new ВВИскл (сооб);
    }

    export override ИПотокВывода слей ()
    {
        if (бвывод)
        {
            while (читаемо() > 0)
    ...
    export override ИПотокВвода очисть ()
    {
    ...
    export override ИПотокВывода копируй (ИПотокВвода ист, т_мера max=-1)
    {
    ...
    export проц[] загрузи (т_мера max=-1)
    {
        return срез;
    }
    ...
    export final т_мера размерБуфера ()
    {
        return 32 * 1024;
    }

    export final бул жив_ли ()
    {
        return да;
    }


    export final ИПотокВывода вывод ()
    {
        return бвывод;
    }


    export final ИПотокВвода ввод ()
    {
        return бввод;
    }

    export final проц открепи ()
    {
    }
...
export Буфер объБуфер (ИПровод провод)
{
    return new Буфер(провод);
}


export Буфер объБуфер (ИПотокВвода поток, т_мера ёмкость)
{
    return new Буфер(поток, ёмкость);
}

export Буфер объБуфер(ИПотокВывода поток, т_мера ёмкость)
{
    return new Буфер(поток, ёмкость);
}


export Буфер объБуфер(т_мера ёмкость = 0)
{
    return new Буфер(ёмкость);
}


export Буфер объБуфер (проц[] данные)
{
    return new Буфер(данные);
}


export Буфер объБуфер (проц[] данные, т_мера читаемо)
{
    return new Буфер(данные, читаемо);
}

export class БуферРоста : Буфер
{
...
export БуферРоста объБуферРоста (т_мера размер = 1024, т_мера инкремент = 1024)
{
    return new БуферРоста(размер, инкремент);
}


export БуферРоста объБуферРоста (ИПровод провод, т_мера размер = 1024)
{
