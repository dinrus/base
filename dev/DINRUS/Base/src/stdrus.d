...
private  ук укМодуль_;
	...
	проц закрой()
	{
		if(null !is укМодуль_)
        {
		if(!ОсвободиБиблиотеку(укМодуль_))
		    throw new ИсклВнешнМодуля(ДайПоследнююОшибку());
		}
	}
	...
	body
	{
	ук символ = cast(ук) ДайАдресПроц(укМодуль_, симв);
	    if(null is символ)
	    {
		throw new ИсклВнешнМодуля(ДайПоследнююОшибку());
	    }
	...
	{
	return адаптВыхУкз(дайСимволИМодуля(укМодуль_, симв));
	}
	
	ук Ук()
	{
	return адаптВыхУкз(укМодуль_);
	}
	
	ткст Путь()
	{
	assert(null != укМодуль_);
...
export extern (D)
{
	цел найди(рткст текст, ткст образец, ткст атрибуты )//Возврат -1=совпадений нет, иначе=индекс совпадения
		{
...
export:
  ...
  // Открывает existing хэндл; use with care!
...
export extern (D) class ФильтрПоток : Поток
 {
	...
	export:
	 ...
	 бул закрытьИсток(){return закрытьГнездо;}
	  ...
	  /// Конструирует ФильтрПоток для данного источника.
		this(Поток исток) {
		this.п = исток;
		закрытьИсток(да);
		if (п !is null) {
	  ...
	  /***
	   * Получить текущий поток исток.
	   */
	   Поток исток(){return this.п;}

	  /***
	   * Установить текущий поток исток.
	   *
	   * Установка потока исток закрывает этот поток прежде, чем прикрепит новый
	   * исток. Прикрепление открытого потока переоткрывает этот поток и 
	   * переустанавливает его состояние. 
	   */
	  проц исток(Поток п) {
		закрой();
		this.п = п;
...
export:
	  ...
	  проц устБуфер(ббайт[] буф){this.буфер = буф;}
	  ббайт[] дайБуфер(){return this.буфер;}
  
	  проц устТекБуфПоз(бцел тбп){this.текБуфПоз = тбп;}
	  бцел дайТекБуфПоз(){return this.текБуфПоз;}
  
  
	  проц устДлинуБуф(бцел дб){this.длинаБуф = дб;}
	  бцел дайДлинуБуф(){return this.длинаБуф;}
  

	  проц устЧерновой(бул чб){this.черновойБуф = чб;}
	  бул дайЧерновойБуф(){return this.черновойБуф;}
	  
  
	  проц устПозИстокаБуф(бцел пиб){this.позИстокаБуф = пиб;}
	  бцел дайПозИстокаБуф(){return this.позИстокаБуф;}
	  
	
	  проц устПозПотока(бдол пп){this.позПотока = пп;}
	  бдол дайПозПотока(){return this.позПотока;}
/+
  invariant() {
...
+/
  static const бцел дефРазмБуфера = 8192;

  /***
   * Создаёт буферированный поток для потока исток с размером буфера
   * размБуф.
   */
  this(Поток исток, бцел размБуф = дефРазмБуфера) {
  
   super(исток);
   assert(super.исток() == исток);
   assert(this.п == исток);
  ...
  override т_мера читайБлок(ук результат, т_мера длин) {
    ...
    ббайт* буфвыв = cast(ббайт*)адаптВхоУкз(результат);
    т_мера readsize = 0;

    if (текБуфПоз + длин < длинаБуф) {
      // буфер has all the данные so copy it
      буфвыв[0 .. длин] = буфер[текБуфПоз .. текБуфПоз+длин];
      текБуфПоз += длин;
      readsize = длин;
      goto ExitRead;
    }
    ...
    if (readsize > 0) {
      ...
      буфвыв[0 .. readsize] = буфер[текБуфПоз .. длинаБуф];
      буфвыв += readsize;
      текБуфПоз += readsize;
      длин -= readsize;
    }
    ...
    if (длин >= буфер.length) {
      ...
      т_мера siz = super.читайБлок(буфвыв, длин);
      readsize += siz;
      позПотока += siz;
    } else {
...
export extern (D) class БуфФайл: БуфПоток {
 ...
 protected Поток поток; 

 alias БуфПоток.дефРазмБуфера дефРазмБуфера;
...
//  win.скажинс("Вход в конструктор БуфФайла");	
	 super(new Файл); 
	 this.поток = super.исток();
	 assert(this.поток == super.исток());
  ...
  /// Открывает файл в требуемом режиме с указанным размером буфера
  ...
  /// Открывает файл для чтения с требуемым размером буфера
  this(Файл файл, бцел размБуф = дефРазмБуфера) {
    super(файл,размБуф);
	this.поток = super.исток();
  }

  /// Открывает существующий хэндл; использовать осторожно!
  this(ук  файлУк, ПРежимФайла режим, бцел размбуфа) {
    super(new Файл(адаптВхоУкз(файлУк),режим),размбуфа);
	this.поток = super.исток();
  }

  /// Открывает файл в требуемом режим
  проц открой(ткст имяф, ПРежимФайла режим = cast(ПРежимФайла) 1) {
    Файл sf = cast(Файл)поток;
	this.записываемый(поток.записываемый());
    сканируемый(да);
    читаемый(cast(бул)(режим & ПРежимФайла.Ввод));
	записываемый(cast(бул)(режим & ПРежимФайла.Вывод));	
  ...
  /// Создаёт файл в требуемом режим
			...
			т_мера окно = 0)
    {
		...
		version (Win32)
		{
			...
			if (имяф)
			{
				...
				{
					hFile =cast(ук) СоздайФайлА(имяф,
							dwDesiredAccess2,
							dwShareMode,
							null,
							dwCreationDisposition,
