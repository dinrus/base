...
// атан(y0(x)/j0(x)) = x - пи/4 + x P(x**2)/Q(x**2)
...
export реал cylBessel_y0(реал x)
{
     ...
     * R(x) is employed в_ compute y0(x)  = R(x) + 2/пи * лог(x) * j0(x).
     *
...
// y0(x) = 2/пи * лог(x) * j0(x) + P(z**2)/Q(z**2), 0 <= x <= 5
...
export реал cylBessel_j1(реал x)
{
     ...
     * The approximation в_ j1 is M1 * кос(x -  3 пи/4 + 1/x P5(1/x^2)/Q6(1/x^2)).
     */
...
// y1(x) = 2/пи * (лог(x) * j1(x) - 1/x) + R(x^2) z P(z**2)/Q(z**2)
// 0 <= x <= 4.5, z(x) = x
