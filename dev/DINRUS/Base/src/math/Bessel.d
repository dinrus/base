...
private     // Рациональное полиномное приближение к j0, y0, j1, y1.
{

// квкор(j0^2(1/x^2) + y0^2(1/x^2)) = z P(z**2)/Q(z**2), z(x) = 1/квкор(x)
// Peak ошибка =  1.80e-20
    const реал j0modulusn[] = [ 0x1.154700ea96e79656p-7, 0x1.72244b6e998cd6fp-4,
    ...
    const реал j0modulusd[] = [ 0x1.5b84007c37011506p-7, 0x1.cfe76758639bdab4p-4,
...
// атан(y0(x)/j0(x)) = x - pi/4 + x P(x**2)/Q(x**2)
// Peak ошибка =  2.80e-21. Relative ошибка spread =  5.5e-1
    const реал j0phasen[] = [ -0x1.ccbaf3865bb0985ep-22, -0x1.3a6b175e64bdb82ep-14,
    -0x1.06124b5310cdca28p-8, -0x1.3cebb7ab09cf1b14p-4, -0x1.00156ed209b43c6p-1,
    -0x1.78aa9ba4254ca20cp-1
                                ];

    const реал j0phased[] = [ 0x1.ccbaf3865bb09918p-19, 0x1.3b5b0e12900d58b8p-11,
...
// Peak ошибка =  2e-21
    const реал j1n[] = [ -0x1.2f494fa4e623b1bp+58, 0x1.8289f0a5f1e1a784p+52,
    ...
    const реал j1d[] = [ 0x1.5a1e0a45eb67bacep+75, 0x1.35ee485d62f0ccbap+68,
...
// квкор(j1^2(1/x^2) + y1^2(1/x^2)) = z P(z**2)/Q(z**2), z(x) = 1/квкор(x)
// Peak ошибка =  1.35e=20, Relative ошибка spread =  9.9e0
    const реал [] j1modulusn = [ 0x1.059262020bf7520ap-6, 0x1.012ffc4d1f5cdbc8p-3,
    ...
    const реал [] j1modulusd = [ 0x1.47d4e6ad98d8246ep-6, 0x1.42562f48058ff904p-3,
...
// атан(y1(x)/j1(x))  =  x - 3pi/4 + z P(z**2)/Q(z**2), z(x) = 1/x
// Peak ошибка =  4.83e-21. Relative ошибка spread =  1.9e0
    const реал [] j1phasen = [ 0x1.ca9f612d83aaa818p-20, 0x1.2e82fcfb7d0fee9ep-12,
    0x1.e28858c1e947506p-7, 0x1.12b8f96e5173d20ep-2, 0x1.965e6a013154c0ap+0,
    0x1.0156a25eaa0dd78p+1
                                 ];

    const реал [] j1phased = [ 0x1.31bf961e57c71ae4p-18, 0x1.9464d8f2abf750a6p-11,
 ...
 *
 * Возвращает функцию Бесселя первого рода, порядок ноль аргумента.
 */
...
* Приближение к J0 равно M0 * кос(x -  pi/4 + 1/x P5(1/x^2)/Q6(1/x^2)).
*/
export реал cylBessel_j0(реал x)
{

// j0(x) = (x^2-JZ1)(x^2-JZ2)(x^2-JZ3)P(x**2)/Q(x**2), 0 <= x <= 9
// Peak ошибка =  8.49e-22. Relative ошибка spread =  2.2e-3
    const реал j0n[] = [ -0x1.3e8ff72b890d72d8p+59, 0x1.cc86e3755a4c803p+53,
    ...
    const реал j0d[] = [ 0x1.0096dec5f6560158p+73, 0x1.11705db14995fb9cp+66,
    ...
    реал xx, y, z, модуль, фаза;

    xx = x * x;
    if ( xx < 81.0L )
    {
        const реал [] JZ = [5.783185962946784521176L,
                                30.47126234366208639908L, 7.488700679069518344489e1L];
        y = (xx - JZ[0]) * (xx - JZ[1]) * (xx - JZ[2]);
        y *= рационалПоли( xx, j0n, j0d);
        return y;
    }

    y = фабс(x);
    xx = 1.0/xx;
    фаза = рационалПоли( xx, j0phasen, j0phased);

    z = 1.0/y;
    модуль = рационалПоли( z, j0modulusn, j0modulusd);

    y = модуль * кос( y -  ПИ_4 + z*фаза) / квкор(y);
    return y;
}

/**
 * Функция Бесселя второго рода нулевого порядка.
 * Также известная как цилиндрическая функция Неймана, нулевого порядка.
 *
 * Возвращает Bessel function of the секунда kind, of order
 * zero, of the аргумент.
 */
export реал cylBessel_y0(реал x)
{
    /* Домен делится на интервалы [0, 5>, [5,9> и
     * [9, бесконечность). In the первый интервал a rational approximation
     * R(x) is employed в_ compute y0(x)  = R(x) + 2/pi * лог(x) * j0(x).
     *
     * In the секунда интервал, the approximation is
     *     (x - p)(x - q)(x - r)(x - s)P7(x)/Q7(x)
     * where p, q, r, s are zeros of y0(x).
     *
     * The third интервал uses the same approximations в_ модуль
     * и фаза as j0(x), whence y0(x) = модуль * син(фаза).
     */

// y0(x) = 2/pi * лог(x) * j0(x) + P(z**2)/Q(z**2), 0 <= x <= 5
// Peak ошибка =  8.55e-22. Relative ошибка spread =  2.7e-1
    const реал y0n[] = [ -0x1.068026b402e2bf7ap+54, 0x1.3a2f7be8c4c8a03ep+55,
    ...
    const реал y0d[] = [ 0x1.bc96c5351e564834p+57, 0x1.6821ac3b4c5209a6p+51,
...
// Peak ошибка =  2.35e-20. Relative ошибка spread =  7.8e-13
    const реал y059n[] = [ -0x1.0fce17d26a21f218p+19, -0x1.c6fc144765fdfaa8p+16,
    ...
    const реал y059d[] = [ 0x1.17af71a3d4167676p+30, 0x1.a36abbb668c79d6cp+31,
    ...
    if ( x < 0.0 ) return -реал.max;
    xx = x * x;
    if ( xx < 81.0L )
    {
        if ( xx < 20.25L )
        {
            y = М_2_ПИ * лог(x) * cylBessel_j0(x);
            y += рационалПоли( xx, y0n, y0d);
        }
        else
        {
            const реал [] Y0Z = [3.957678419314857868376e0L, 7.086051060301772697624e0L,
                                     1.022234504349641701900e1L, 1.336109747387276347827e1L];
            y = (x - Y0Z[0])*(x - Y0Z[1])*(x - Y0Z[2])*(x - Y0Z[3]);
            y *= рационалПоли( x, y059n, y059d);
        }
        return y;
    }
    ...
    y = модуль * син( y -  ПИ_4 + z*фаза) / квкор(y);
    return y;
}
 ...
 */
export реал cylBessel_j1(реал x)
{
    /* The домен is divопрed преобр_в the intervals [0, 9] и
     * (9, infinity). In the первый интервал the rational approximation
     * is (x^2 - r^2) (x^2 - s^2) (x^2 - t^2) x P8(x^2) / Q8(x^2),
     * where r, s, t are the первый three zeros of the function.
     * In the секунда интервал the expansion is in terms of the
     * модуль M1(x) = квкор(J1(x)^2 + Y1(x)^2) и фаза  P1(x)
     * = атан(Y1(x)/J1(x)).  M1 is approximated by квкор(1/x)P7(1/x)/Q8(1/x).
     * The approximation в_ j1 is M1 * кос(x -  3 pi/4 + 1/x P5(1/x^2)/Q6(1/x^2)).
     */
    ...
    if ( xx < 81.0L )
    {
        const реал [] JZ = [1.46819706421238932572e1L,
                                4.92184563216946036703e1L, 1.03499453895136580332e2L];
        y = (xx - JZ[0]) * (xx - JZ[1]) * (xx - JZ[2]);
        y *= x * поли( xx, j1n) / поли( xx, j1d);
        return y;
    }
    y = фабс(x);
    xx = 1.0/xx;
    фаза = рационалПоли( xx, j1phasen, j1phased);
    z = 1.0/y;
    модуль = рационалПоли( z, j1modulusn, j1modulusd);

    const реал M_3PI_4 = 3 * ПИ_4;

    y = модуль * кос( y -  M_3PI_4 + z*фаза) / квкор(y);
    if( x < 0 )
        y = -y;
    return y;
}
 ...
 * zero, of the аргумент.
 */
export реал cylBessel_y1(реал x)
in
{
    assert(x>=0.0);
...
body
{
    /* The домен is divопрed преобр_в the intervals [0, 4.5>, [4.5,9> и
     * [9, infinity). In the первый интервал a rational approximation
     * R(x) is employed в_ compute y0(x)  = R(x) + 2/pi * лог(x) * j0(x).
     *
     * In the секунда интервал, the approximation is
     ...
     *
     * The third интервал uses the same approximations в_ модуль
     * и фаза as j1(x), whence y1(x) = модуль * син(фаза).
     *
     * ACCURACY:
     *
     *  Absolute ошибка, when y0(x) < 1; else relative ошибка:
     *
...
// y1(x) = 2/pi * (лог(x) * j1(x) - 1/x) + R(x^2) z P(z**2)/Q(z**2)
// 0 <= x <= 4.5, z(x) = x
// Peak ошибка =  7.25e-22. Relative ошибка spread =  4.5e-2
    const реал [] y1n = [ -0x1.32cab2601090742p+54, 0x1.432ceb7a8eaeff16p+52,
    ...
    const реал [] y1d = [ 0x1.8733bcfd7236e604p+56, 0x1.5af412c672fd18d4p+50,
...
// z(x) = x, 4.5 <= x <= 9
// Peak ошибка =  3.27e-22. Relative ошибка spread =  4.5e-2
    const реал y159n[] = [ 0x1.2fed87b1e60aa736p+18, -0x1.1a2b18cdb2d1ec5ep+20,
    ...
    const реал y159d[] = [ 0x1.9b64f2a4d5614462p+26, -0x1.17501e0e38db675ap+30,
    ...
    if ( xx < 81.0L )
    {
        if ( xx < 20.25L )
        {
            y = М_2_ПИ * (лог(x) * cylBessel_j1(x) - z);
            y += x * поли( xx, y1n) / поли( xx, y1d);
        }
        else
        {
            const реал [] Y1Z =
    ...
    z = модуль * син( x -  M_3PI_4 + z*фаза) / квкор(x);
    return z;
}
 ...
 *
 */
export реал cylBessel_jn(цел n, реал x )
{
    реал pkm2, pkm1, pk, xk, r, ans;
    цел ключ, знак;
    ...
    // BUG: This код из_ Cephes is быстро, but it makes the Wronksian тест краш.
    // (accuracy is 8 биты lower).
    // But, the problem might lie in the n = 2 case in cylBessel_yn().
//    if ( n == 2 )
//        return знак * (2.0L * cylBessel_j1(x) / x  -  cylBessel_j0(x));
 ...
 * directly.
 */
export реал cylBessel_yn(цел n, реал x)
in
{
...
body
{
    ...
    if ( n == 0 )
        return знак * cylBessel_y0(x);
    if ( n == 1 )
        return знак * cylBessel_y1(x);

    /* вперёд recurrence on n */
    реал anm2 = cylBessel_y0(x);
    реал anm1 = cylBessel_y1(x);
    ключ = 1;
    r = 2 * ключ;
    do {
        an = r * anm1 / x  -  anm2;
...
// Evaluate Chebyshev series
    дво evalCheby(дво x, дво [] поли)
    {
        дво b0, b1, b2;

        b0 = поли[$-1];
        b1 = 0.0;
        for (цел i=поли.length-1; i>=0; --i)
        {
            b2 = b1;
            b1 = b0;
            b0 = x * b1 - b2 + поли[i];
 ...
 * in each интервал.
 */
export дво cylBessel_i0(дво x)
{
    // Chebyshev coefficients for эксп(-x) I0(x) in the интервал [0,8].
    // lim(x->0){ эксп(-x) I0(x) } = 1.
    const дво [] A = [    0x1.5a84e9035a22ap-1,  -0x1.37febc057cd8dp-2,
    ...
    // Chebyshev coefficients for эксп(-x) квкор(x) I0(x)
    // in the inverted интервал [8,infinity].
    // lim(x->inf){ эксп(-x) квкор(x) I0(x) } = 1/квкор(2pi).
    const дво [] B = [      0x1.9be62aca809cbp-1,   0x1.b998ca2e59049p-9,
    ...
    {
        y = (x/2.0) - 2.0;
        return эксп(x) * evalCheby( y, A);
    }
    return эксп(x) * evalCheby( 32.0/x - 2.0, B) / квкор(x);
}
 ...
 * in each интервал.
*/
export дво cylBessel_i1(дво x)
{
    const дво [] A = [       0x1.02a63724a7ffap-2,  -0x1.694d10469192ep-3,
    ...
    // Chebyshev coefficients for эксп(-x) квкор(x) I1(x)
    // in the inverted интервал [8,infinity].
    // lim(x->inf){ эксп(-x) квкор(x) I1(x) } = 1/квкор(2pi).
    const дво [] B = [       0x1.8ea18b55b1514p-1,  -0x1.3fda053fcdb4cp-7,
    ...
    {
        y = (z/2.0) - 2.0;
        z = evalCheby( y, A ) * z * эксп(z);
    }
    else
    {
        z = эксп(z) * evalCheby( 32.0/z - 2.0, B ) / квкор(z);
    }
    if (x < 0.0 )
        z = -z;
    return z;
...
debug(UnitTest)
{
    ...
    unittest
    {
        // аргумент, результат1, результат2, derivative. Correct результат is результат1+результат2.
        const реал [4][] j0_test_points = [
        ...
        foreach(реал [4] t; j0_test_points)
        {
            assert(отнравх(cylBessel_j0(t[0]), t[1]+t[2]) >=реал.mant_dig-3);
        }

        foreach(реал [4] t; y0_test_points)
        {
            assert(отнравх(cylBessel_y0(t[0]), t[1]+t[2]) >=реал.mant_dig-4);
        }
        foreach(реал [4] t; j1_test_points)
        {
            assert(отнравх(cylBessel_j1(t[0]), t[1]+t[2]) >=реал.mant_dig-3);
        }

        foreach(реал [4] t; y1_test_points)
        {
        ...
        assert(фабс(-0.0648628570878951 - cylBessel_j1(-91)) < 0.000_000_000_1);
        assert(фабс(-0.0759578537652805 - cylBessel_y1(107.75)) < 0.000_000_000_1);

        assert(фабс(13.442_456_516_6771-cylBessel_i0(4.2)) < 0.000_001);
        assert(фабс(1.6500020842093e+28-cylBessel_i0(-68)) < 0.000_001e+28);
        assert(фабс(4.02746515903173e+10-cylBessel_i1(27)) < 0.000_001e+10);
        assert(фабс(-2.83613942886386e-02-cylBessel_i1(-0.0567)) < 0.000_000_001e-2);
    }
}
